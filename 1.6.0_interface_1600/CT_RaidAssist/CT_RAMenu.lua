...
function CT_RAMenu_UpdateMenu()
	...
	for i = 1, 5, 1 do
		getglobal("CT_RAMenuFrameGeneralTargetsTarget" .. i .. "CB"):SetChecked(CT_RAMenu_Options["temp"]["ShowMTs"][i]);
	end
	...
	CT_RAMenuFrameGeneralMiscHideShortCB:SetChecked(CT_RAMenu_Options["temp"]["HideShort"]);
	...
	CT_RAMenuFrameMiscMaintainTargetCB:SetChecked(CT_RAMenu_Options["temp"]["MaintainTarget"]);
...
function CT_RAMenuTarget_Toggle()
	CT_RAMenu_Options["temp"]["ShowMTs"][this:GetID()] = this:GetChecked();
	CT_RA_UpdateMTs();
end
...
function CT_RAMenu_General_ResetWindows()
	...
	CT_RA_EmergencyFrame:ClearAllPoints();
	...
	CT_RA_EmergencyFrame:SetPoint("CENTER", "UIParent", "CENTER", 0, 0);
...
function CT_RAMenuMisc_Slider_OnChange()
	...
	CT_RAMenu_Options["temp"]["ClassHealings"][CT_RA_GetLocale()][UnitClass("player")][this:GetID()][3] = this:GetValue()/100;
...
function CT_RAMenuMisc_OnUpdate(elapsed)
	if ( this.scaleupdate ) then
		...
		if ( this.scaleupdate <= 0 ) then
			...
			local pos = CT_RAMenu_Options["temp"]["WindowPositions"]["CT_RA_EmergencyFrame"];
			if ( pos and CT_RAMenu_Options["temp"]["EMScaling"] ) then
				CT_RA_EmergencyFrame:SetScale(CT_RAMenu_Options["temp"]["EMScaling"]*UIParent:GetScale());
				
				local diff = UIParent:GetScale()/CT_RA_EmergencyFrame:GetScale();
				CT_RA_EmergencyFrame:ClearAllPoints();
				CT_RA_EmergencyFrame:SetPoint("TOPLEFT", "UIParent", "TOPLEFT", pos[1]*diff, pos[2]*diff);
			end
		end
	end
...
function CT_RAMenuAdditional_EM_OnValueChanged()
	if ( this:GetID() == 1 ) then
		...
		if ( not CT_RAMenu_Options["temp"]["WindowPositions"] or not CT_RAMenu_Options["temp"]["WindowPositions"]["CT_RA_EmergencyFrame"] ) then
			if ( not CT_RAMenu_Options["temp"]["WindowPositions"] ) then
				CT_RAMenu_Options["temp"]["WindowPositions"] = { };
			end
			CT_RAMenu_Options["temp"]["WindowPositions"]["CT_RA_EmergencyFrame"] = { CT_RA_EmergencyFrame:GetLeft(), CT_RA_EmergencyFrame:GetTop()-UIParent:GetTop() };
		end
...
function CT_RAMenu_Misc_MaintainTarget()
	CT_RAMenu_Options["temp"]["MaintainTarget"] = this:GetChecked();
end
...
function CT_RAMenu_SaveWindowPositions()
	...
	left, top, uitop = CT_RA_EmergencyFrame:GetLeft(), CT_RA_EmergencyFrame:GetTop(), UIParent:GetTop();
	if ( left and top and uitop ) then
		CT_RAMenu_Options["temp"]["WindowPositions"]["CT_RA_EmergencyFrame"] = { left, top-uitop };
	end
end
