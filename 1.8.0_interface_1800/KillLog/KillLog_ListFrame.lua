...
function KillLog_ListFrame_Update()
	...
	for index=1, KILLLOG_LIST_CREEPS_DISPLAYED, 1 do
		...
		creepNormalText    = getglobal("KillLog_List_Title"..index.."NormalText");
		creepHighlightText = getglobal("KillLog_List_Title"..index.."HighlightText");
		creepDisabledText  = getglobal("KillLog_List_Title"..index.."DisabledText");
		if ( creepID > numEntries ) then
			...
			if ( creepData.isHeader ) then
				if ( creepData.name ) then
					creepTitle:SetText(creepData.name);
				else
					creepTitle:SetText("");
				end
			...
			if ( tag ) then
				...
				creepNormalText:SetWidth(275 - 5 - creepTitleTag:GetWidth());
				creepHighlightText:SetWidth(275 - 5 - creepTitleTag:GetWidth());
				creepDisabledText:SetWidth(275 - 5 - creepTitleTag:GetWidth());
			else
				creepTitleTag:SetText("");
				-- Reset to max text width
				creepNormalText:SetWidth(275);
				creepHighlightText:SetWidth(275);
				creepDisabledText:SetWidth(275);
			end
...
function KillLog_ListFrame_UpdateCreepDetails()
	...
	if ( creepData.name ) then
		...
		if ( not creepInfo ) then
			...
			if ( creepInfo.class == "worldboss" or creepInfo.class == "rareelite" or creepInfo.class == "elite" ) then
			...
			if ( creepInfo.zone and creepInfo.x and creepInfo.y ) then
				location = format("%s (%d, %d)", creepInfo.zone, creepInfo.x, creepInfo.y);
			end
...
function KillLog_ListFrame_ExpandHeader(creepID)
	...
	if ( creepID == 0 ) then
		...
		for index, creepData in KillLog_ListFrame.display do
			if ( creepData.isHeader ) then
...
function KillLog_ListFrame_Tab_OnClick()
	...
	if ( tabID ~= 1 and KillLog_ListFrame_SortDropDown.selectedID == 1 ) then
...
function KillLog_ListFrame_CreateSessionDisplay()
	...
	if ( KillLog_SessionData ) then
		...
		local index, creepData;
		for index, creepData in KillLog_SessionData do
...
function KillLog_ListFrame_SortDropDown_Initialize()
	...
	local index, data;
	for index, data in KILLLOG_LIST_SORT do
		info.text = data.name;
...
function KillLog_ListFrame_LevelDropDown_OnShow()
	...
	table.setn(KillLog_ListFrame.levelValues, 0);
	local index, data;
	if ( KillLog_AllCharacterData and KillLog_AllCharacterData["level"] ) then
		for index, data in KillLog_AllCharacterData["level"] do
...
function KillLog_ListFrame_LevelDropDown_Initialize()
	...
	local index, data;
	for index, data in KillLog_ListFrame.levelValues do
		info.text = data;
...
function KillLog_ListFrame_ChangeCreepFamily(creepID, newFamily)
	local creepData = KillLog_ListFrame_GetCreepName(creepID);
	local creepInfo = KillLog_CreepInfo[creepData.name];
	if ( not creepInfo ) then
...
function KillLog_ListFrame_GetSortInfo_Type(creepName)
	...
	if ( KillLog_CreepInfo[creepName] ) then
		...
		--if ( KillLog_ListFrame.display[creepName].kill ) then
		--	tag = KillLog_ListFrame.display[creepName].kill;
		--end
