...
--	
--	$Id: Khaos.lua 1553 2005-05-20 22:32:36Z AlexYoshi $
--	$Rev: 1553 $
--	$LastChangedBy: AlexYoshi $
--	$Date: 2005-05-21 00:32:36 +0200 (Sat, 21 May 2005) $
--
--	Syntax: 
...
-- Mapping of locale->server->player for configurations
...
Khaos = {
	...
	--
	-- unregisterOptionSet( folder, setid ) 
	-- 	unregisters an option set from the folder specified
	--
	-- args:
	...
	validateOption = function ( option, defaultDifficulty ) 
		...
		elseif ( option.type == K_SLIDER ) then 
			...
			if ( type ( option.setup ) ~= "table" ) then 
				...
				if ( type ( option.setup.sliderDisplayFunc ) ~= "function" ) then
	...
	getSetEnabled = function (setid)	
		...
		if ( enabled == nil ) then 
			Khaos.setSetKey("sets", setid, KhaosData.configurationSets[setid].default);
			enabled = KhaosData.configurationSets[setid].default;
		end
...
KhaosCore = {
	...
	runCallbacks = function ( cfg, setlist )
		...
		for setid,set in sets do
			...
			if ( set.callback ) then 
				set.callback( setDisabled );
			end
			for k,option in set.options do
				local optionDisabled = KhaosConfig_IsOptionDisabled( option, setid, true, cfg);
	...
	getSetKeyFromConfig = function ( config, set, key )
		...
		if ( config and config.configuration ) then 
			if ( config.configuration[set] and config.configuration[set][key] ) then 
				...
				if ( KhaosData.configurationSets[set] ) then 
					for k,v in KhaosData.configurationSets[set].options do 
						if ( v.key == key ) then 
							return Sea.table.copy(v.default);
						end
					end
				end
			end
			...
			if ( type(parseTree[i].key) == "string" ) then 
				if ( type(parseTree[i].stringMap) == "table" ) then 
					...
					if ( parseTree[i].stringMap[currentWord] ) then
						for k,v in parseTree[i].stringMap[currentWord] do 
							if ( type(v) == "string" ) then 
								v = KhaosCore.processEncodedSlashString(v, words, set);
							end
							Khaos.setSetKeyParameter(set, parseTree[i].key, k, v);
						end
					elseif ( parseTree[i].stringMap.default ) then 						
						for k,v in parseTree[i].stringMap.default do 
							if ( type(v) == "string" ) then 
								v = KhaosCore.processEncodedSlashString(v, words, set);
							end
							Khaos.setSetKeyParameter(set, parseTree[i].key, k, v);
...
function KhaosConfig_GetConfigurationTree(id)
	...
	for i=1,table.getn(Khaos_Configurations) do
		for k,v in keywords do
			...
			if ( v == "all" or KhaosCore.getKeywordByID(i, v ) ) then 
	...
	for k,v in configs do 
		...
		table.insert(tree[1].children, child);
	end
...
function KhaosConfig_UpdateOptionPane()
	...
	KhaosConfig_OptionPane_UpdateFauxScrollFrameRange(KhaosConfig_GetOptionCountBySetId(currentSet));
...
function KhaosConfig_Option_LoadOption(id, option)
	...
	elseif ( option.type == K_SLIDER ) then 
		...
		if ( key.slider ) then 
			...
			slider:SetValue(key.default.slider);
		end
		slider:Show();		
	...
	elseif ( option.type == K_PULLDOWN ) then 
		...
		if ( option.setup.multiSelect == false and key.value ) then 
			getglobal(pulldown:GetName().."Text"):SetText(key.value);
		else
...
function KhaosConfig_OptionPane_UpdateFauxScrollFrameRange(range, offset)
	...
	FauxScrollFrame_Update(KhaosFrameOptionSetupPaneContainerScrollFrame, range+1, 10, KHAOS_CONFIG_HEIGHT );
end;
	...
	elseif ( actionEvent == "LEAVE" ) then 
		...
		if ( actionObject == "SLIDER" ) then 
			if ( actionEvent == "CHANGED") then 
				...
				Chronos.scheduleByName("KhaosUpdate",.8,Khaos.refresh, false, false, true);
			end
		end
		...
		if ( actionObject == "PULLDOWN" ) then 
			if ( actionEvent == "CLICK" ) then 
				...
				for k,v in options do
					...
					if ( Sea.list.isInList(key.value, v) ) then
						item.checked = true;
					end
...
function KhaosConfig_Pulldown_UpdateValue(checked, value, set, option)
	...
	if ( option.setup.multiSelect ) then 
		if ( checked ) then 
			if ( not Sea.list.isInList(key.value, value) ) then 
				table.insert(key.value, value);
			end
		else			
			for k,v in key.value do 
				if ( v == value ) then
					table.remove(key.value, k);
				end
			end
		end

		Khaos.setSetKeyParameter(set, option.key, "value", key.value);
		option.callback(key);
		-- Run the callbacks
		KhaosCore.runCallbacks(nil, {currentSet});
		Khaos.refresh(nil, nil, true);
...
function KhaosConfig_Configuration_Tooltip(value)
	...
	if ( Khaos_Configurations[cid] ) then 
		...
		tooltip = tooltip..Khaos_Configurations[cid].name.."\n";
		...
		if ( Khaos_Configurations[cid].keywords ) then 
			...
			if ( Khaos_Configurations[cid].keywords.class ) then 
				tooltip = tooltip..KHAOS_CONFIG_TOOLTIP_CLASSES.."\n";
			...
			if ( Khaos_Configurations[cid].keywords.realm ) then 
				tooltip = tooltip..KHAOS_CONFIG_TOOLTIP_REALMS.."\n";
			...
			if ( Khaos_Configurations[cid].keywords.player ) then 
				tooltip = tooltip..KHAOS_CONFIG_TOOLTIP_CLASSES.."\n";

				local count = 0;
				for k,v in Khaos_Configurations[cid].keywords.player do
					count = count + 1;
...
function KhaosConfig_OnMouseWheel(delta)
	...
	KhaosData.currentOffset = KhaosData.currentOffset - delta;

	if ( KhaosData.currentOffset < 0 ) then 
		KhaosData.currentOffset = 0;
	elseif ( KhaosData.currentOffset > max - 10 ) then 
		KhaosData.currentOffset = max - 10;
	end

	-- Update the scroll frame
	KhaosConfig_OptionPane_UpdateFauxScrollFrameRange(max);
...
function Khaos_Login_Frame_OnShow()
	...
	local configs = KhaosCore.matchKeywords ( { player = character; server = server; class = class } ); 
...
function Khaos_Login_SyncSelected(testId)
	...
	if ( Khaos_Configuration_Locks[locale] ) then 
		if ( Khaos_Configuration_Locks[locale][server] ) then 
			if ( Khaos_Configuration_Locks[locale][server][character] ) then 
				local id = Khaos_Configuration_Locks[locale][server][character];
...
function Khaos_Login_SetConfigurationLock(id)
	...
	if ( not Khaos_Configuration_Locks[locale][server] ) then 
		Khaos_Configuration_Locks[locale][server] = {};
	end
	
	Khaos_Configuration_Locks[locale][server][character] = id;
end
...
function Khaos_Login_ShowConfigurationsMenu()
	...
	configs = KhaosCore.matchKeywords ( { player = character; server = server; class = class } ); 
	...
	EarthMenu_MenuOpen(menu, this:GetName(), 0, 0, nil, 3);
end
...
function KhaosConfig_Set_OpenMenu(value) 
	...
	item.func = function(checked, value) if ( checked == 1 ) then checked = true; end  Khaos.setSetEnabled(value, checked); Khaos.refresh() end;
	menu[1] = item;

	item = {};
	item.text = KHAOS_SET_MENU_RESET; 
	item.value = value;
	item.func = function (checked, value) KhaosCore.resetSet(value); Khaos.refresh(); end;
	menu[2] = item;
...
function Khaos_Frame_OnLoad()
	...
	KhaosFrameConfigurationSelectPaneContainerTree.tooltipPlacement = "button";
	...
	Chronos.afterInit(KhaosCore.runCallbacks);
end
...
function KhaosConfig_RegisterInit()
	...
	if ( Cosmos_RegisterButton ) then
		Cosmos_RegisterButton (
			TEXT(KHAOS_TITLE_TEXT), 
			TEXT(KHAOS_BUTTON_SUB),
			TEXT(KHAOS_BUTTON_TIP),
