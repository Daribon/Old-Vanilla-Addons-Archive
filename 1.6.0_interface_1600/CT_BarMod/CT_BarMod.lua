...
function CT_Sidebar_ChangeAxis(bar, force)
	local i, barstring, types;
	if ( CT_BarModOptions_Options and CT_BarModOptions_Options["remove"] ) then
		types = {
			{ "LEFT", "RIGHT", 3, 0 },
			{ "TOP", "BOTTOM", 0, -3 }
		};
...
function CT_updateActionButtons()
	for i = 1, 12, 1 do
		...
		if ( not HasAction(ActionButton_GetPagedID(ab)) and ab.showgrid == 0 ) then
			if ( CT_ShowGrid == 0 or ( not CT_BottomBar_Enabled or not CT_BottomBar_Enabled[UnitName("player")] or ( not CT_BottomBar_RotatedFrames[UnitName("player")]["Bars"] and not CT_BottomBar_HiddenFrames[UnitName("player")]["BarHotbarBackgroundLeft"] ) ) ) then 
				ab.showgrid = 0;
				ab:Hide();
			else
				ab.showgrid = 1;
...
function CT_newActionButton_Update()
	...
	if ( not HasAction(ActionButton_GetPagedID(this)) and this.showgrid == 0 ) then
		if ( CT_ShowGrid == 0 or ( not CT_BottomBar_Enabled or not CT_BottomBar_Enabled[UnitName("player")] or ( not CT_BottomBar_RotatedFrames[UnitName("player")]["Bars"] and not CT_BottomBar_HiddenFrames[UnitName("player")]["BarHotbarBackgroundLeft"] ) ) ) then 
			this.showgrid = 0;
			this:Hide();
		else
