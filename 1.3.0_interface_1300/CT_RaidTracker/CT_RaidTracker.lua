...
function CT_RaidTracker_Update()
	...
	-- ScrollFrame update
	FauxScrollFrame_Update(CT_RaidTrackerListScrollFrame, numEntries, 6, 16, CT_RaidTrackerHighlightFrame, 293, 316 );
...
function CT_RaidTracker_UpdateItems()
	for k, v in CT_RaidTracker_RaidLog do
		for key, val in v["Loot"] do
			if ( not val["type"] ) then
				val["item"]["type"] = CT_RaidTracker_GetItemType("item:"..val["item"]["id"]);
			end
		end
	end
end
