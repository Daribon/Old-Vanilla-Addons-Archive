...
function CT_IsQuestRecorded(title, money)
	local datatitle, datamoney;
	if ( CT_Data_Quests[title.."|"..money] ) then
		return "true";
	else
		return "false";
	end
...
function CT_RecordQuest(title, money)
	RegisterCVar("WPQuest1", title.."|"..money);
end
