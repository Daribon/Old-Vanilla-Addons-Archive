...
--	
--	$Id: Libram.lua 2025 2005-07-02 23:51:34Z Sinaloit $
--	$Rev: 2025 $
...
LibramData = {
	...
	updateHistory = function()
		...
		-- Update Arcana if required (Deleted/Added Quests)
		if (forceArcana) then
		...
		for i=1,total do 
			...
			if ( not isHeader ) then 
				if ( LibramData.arcana[title] ) then
					Sea.io.dprint(LM_DEBUG_D, "Researchers are investigating ", title, " ", level, " id: ", i);
					...
					if ( LibramData.arcana[title][level] ) then
						...
						Sea.io.dprint( LM_DEBUG, "Libram: Researchers do not recognize ", title, " and ask for its tome." );
	...
	getTome = function ( title, level, id ) 
		...
		if ( not LibramData.arcana[title][level] or
			...
			if ( research ) then 
				...
				Sea.io.dprint(LM_DEBUG, "Libram: Archivists are unable to find ", title, ". ", id);
			end
		end
