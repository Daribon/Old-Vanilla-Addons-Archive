--	FastQuest 2.10 By Vashen ( Vashen@msn.com )
--	http://www.curse-gaming.com/mod.php?addid=812

hQuestLog_Update = QuestLog_Update;
hQuestWatch_Update = QuestWatch_Update;

BINDING_HEADER_FQHEADER = "|cff00ffffFastQuest";
BINDING_NAME_FQNAME = "Toggle Quest Tracker";
...
function FastQuest_SlashCmd(msg)
	if (msg) then
		...
		if( cmd == "tag" ) then
			...
			FastQuestData[player].Tag=FastQuest_ToggleBoolean(FastQuestData[player].Tag,info);
			QuestLog_Update();
			QuestWatch_Update();
			return;
		elseif( cmd == "autoadd" ) then
			info = (info.."Automatic addition of changed quests to QuestTracker has been ");
			FastQuestData.AutoAdd=FastQuest_ToggleBoolean(FastQuestData.AutoAdd,info);
			return;
		elseif( cmd == "autonotify" ) then
			info = (info.."Automatic notification of party members regarding your quest progress has been ");
			FastQuestData.AutoNotify=FastQuest_ToggleBoolean(FastQuestData.AutoNotify,info);
			return;
		elseif( cmd == "autocomplete" ) then
			info = (info.."Automatic quest completion has been ");
			FastQuestData.AutoComplete=FastQuest_ToggleBoolean(FastQuestData.AutoComplete,info);
			return;
		elseif( cmd == "lock" and FastQuestData.NoDrag==false)then
			qOut(info.." Movable components have been Locked");
			FastQuestData.Lock = true;
			FastQuest_LockMovableParts();
			return;
 		elseif( cmd == "unlock" and FastQuestData.NoDrag==false)then
			qOut(info.." Movable components have been Unlocked");
			FastQuestData.Lock = false;
			FastQuest_LockMovableParts();
 			return;
		elseif( cmd == "nodrag") then
			info = (info.." Dragging is now ");
			FastQuestData.NoDrag=FastQuest_ToggleBoolean(FastQuestData.NoDrag,info);
			if (FastQuestData.NoDrag == false ) then FastQuestData.Lock = true;end
			qOut("You must reload UI for this change to apply. Type /console reloadui");
			return;
		elseif( cmd == "reset" and FastQuestData.NoDrag==false)then
		...
		elseif( cmd == "format" )then
			if (FastQuestData[player].Format==nil) then FastQuestData[player].Format=1;end
			if (FastQuestData[player].Format==(nFormats-1)) then
				FastQuestData[player].Format=0;
			else
				FastQuestData[player].Format=FastQuestData[player].Format+1;
			end
			qOut(info.."Toggle beetween output formats ");
			qOut("Selected format: "..Formats[FastQuestData[player].Format]);
			return;
		elseif( cmd == "clear" ) then
			qOut(info.."All quest tracker quests have been removed ");
			for i = GetNumQuestWatches(), 1 , -1 do
				local qID = GetQuestIndexForWatch(i)
				FastQuestData[player].tQuests[i] = " ";
				RemoveQuestWatch(qID);
			end
			FastQuestData[player].nQuests = 0;
			QuestWatch_Update();
			return;
		end
...
function FastQuest_OnEvent(event, message)
	if	((event == "QUEST_PROGRESS") and (FastQuestData.AutoComplete==true)) then
			CompleteQuest();	
	elseif	((event == "QUEST_COMPLETE") and (FastQuestData.AutoComplete==true)) then
		if (GetNumQuestChoices() == 0) then
			GetQuestReward(QuestFrameRewardPanel.itemChoice);
		end
	elseif(event == "VARIABLES_LOADED") then
		QUEST_FADING_ENABLE = nil;
		if (FastQuestData == nil) then FastQuestData = {};end
		if (FastQuestData.Lock == nil) then FastQuestData.Lock=false; end
		if (FastQuestData.NoDrag == nil) then FastQuestData.NoDrag = false; end;
		if (FastQuestData.AutoAdd == nil) then FastQuestData.AutoAdd  = true; end
		if (FastQuestData.AutoNotify == nil) then FastQuestData.AutoNotify  = false; end;
		if (FastQuestData.AutoComplete == nil) then FastQuestData.AutoComplete  = false; end;
		UpdatePlayer();
	...
	elseif (event == "UI_INFO_MESSAGE" and message) then
		...
		if ( uQuestText ~= message) then
			if (FastQuestData.AutoNotify == true) then 
				FastQuest_CheckDefaultChat(false);
				SendChatMessage("FastQuest progress: "..message, DEFAULT_CHAT_FRAME.editBox.chatType);
			end
			if (FastQuestData.AutoAdd==true and GetNumQuestWatches()<MAX_WATCHABLE_QUESTS) then
...
function QuestLog_Update()
	if (player == "DEFAULT" or FastQuestData[player].tQuests == nil) then UpdatePlayer();end;
	FastQuest_LockMovableParts();
	local nEntries, nQuests = GetNumQuestLogEntries();
	if (GetNumQuestWatches() < 1 and FastQuestData[player].nQuests > 0) then
		for i=1, nEntries do
			SelectQuestLogEntry(i);
 			local qTitle, qLevel, qTag, isHeader, isCollapsed = GetQuestLogTitle(i + FauxScrollFrame_GetOffset(QuestLogListScrollFrame));
			for j = 1, FastQuestData[player].nQuests do
				if (qTitle == FastQuestData[player].tQuests[j] and not IsQuestWatched(i)) then
					AddQuestWatch(i); 
				end
			end
		end
...
function QuestWatch_Update()
	...
	FastQuestData[player].nQuests = GetNumQuestWatches();
	for i = 1, GetNumQuestWatches() do
		FastQuestData[player].tQuests[i] = " ";
		local qID = GetQuestIndexForWatch(i);
		if (qID) then
			local qTitle, qLevel, qTag, isHeader, isCollapsed = GetQuestLogTitle(qID);
			FastQuestData[player].tQuests[i]= qTitle;
...
function FastQuest_ChangeTitle(qLogTitle, qTitle, qLevel, qTag, isHeader, Watch)
	...
	if (qTitle and not isHeader) then
		if (qTag and FastQuestData[player].Tag==true) then DifTag = (" ("..qTag..")  ");end
		if (Watch) then
...
function FastQuest_LinkFrame(dButton, pFrame)
	if (FastQuestData.NoDrag == false) then
...
function FastQuest_LockMovableParts()
	if (FastQuestData[player].nQuests > 0 and QuestWatchFrame:IsVisible() and FastQuestData.Lock==false and FastQuestData.NoDrag==false) then
...
function UpdatePlayer()
	...
	if (player == nil or player == UNKNOWNBEING or player == UKNOWNBEING or player == UNKNOWNOBJECT) then
		player = "DEFAULT";end
	if ( FastQuestData[player] == nil or FastQuestData[player].tQuests == nil ) then 
		FastQuestData[player] =	{
				  ["Format"]	= 1;
				  ["Tag"]	= false;
				  ["nQuests"]	= 0;
				  ["tQuests"]	= { };
