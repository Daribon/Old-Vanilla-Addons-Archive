...
OneHitWonder_ShouldShowQueue = 0;
OneHitWonder_ShouldShowQueueFull = false;
...
OneHitWonder_ShouldRemoveDebuffs = 1;
...
OneHitWonder_Options = {
	...
	["OneHitWonder_ShouldRemoveDebuffs"] = OneHitWonder_ShouldRemoveDebuffs,
...
function OneHitWonder_DoGenericActionAdding(addFuncName, ...)
	local counterId = -1;
	local abilityName = "";
	local timeout = 3;
	local target = nil;
	...
	if ( func ) then
		...
		if ( arg ) then
			r1, r2, r3, r4, r5 = func(unpack(arg));
		else
			r1, r2, r3, r4, r5 = func();
		end
		--[[
...
function OneHitWonder_HandleImmune(event, message)
	for ability, target in string.gfind(message, ONEHITWONDER_IMMUNE_CHAT_MESSAGE) do
		if ( UnitName("target") == target ) and ( not UnitIsPlayer("target") ) then
...
function OneHitWonder_Chat_OnEvent(event)
	if ( OneHitWonder_IsEnabled() ) then
		if ( strsub(event, 1, 8) == "CHAT_MSG" ) then
			OneHitWonder_HandleImmune(event, arg1);
...
function OneHitWonder_HandleActionQueue()
	...
	elseif ( actionId == ONEHITWONDER_ACTIONID_ACTION_SELF ) then
		...
		if ( type(actionParameter) == "table" ) then
			actionId = actionParameter[1];
			spellId = actionParameter[2];
		else
			actionId = actionParameter;
		end
...
function OneHitWonder_Init()
	...
	local arr = getglobal(format("OneHitWonder_Options_%s", currentClass));
	if ( arr ) and (type(arr) == "table" ) then
		for k, v in arr do
			OneHitWonder_Options[v] = getglobal(v);
		end
	end
...
function OneHitWonder_LoadOptions()
	OneHitWonder_Print("OneHitWonder - loading options.");
...
function OneHitWonder_SetShouldRemoveDebuffs(toggle)
	if ( OneHitWonder_ShouldRemoveDebuffs ~= toggle ) then
		OneHitWonder_ShouldRemoveDebuffs = toggle;
		OneHitWonder_SetOptions();
	end
end
...
function OneHitWonder_AddStuffToCosmos()
	if ( Cosmos_RegisterConfiguration ) and ( Cosmos_UpdateValue ) then
		...
		Cosmos_RegisterConfiguration(
			"COS_ONEHITWONDER_REMOVE_DEBUFFS",
			"CHECKBOX",
			TEXT(ONEHITWONDER_REMOVE_DEBUFFS),
			TEXT(ONEHITWONDER_REMOVE_DEBUFFS_INFO),
			OneHitWonder_SetShouldRemoveDebuffs,
			OneHitWonder_ShouldRemoveDebuffs
		);
...
function OneHitWonder_OnEvent(event)
	...
	if ( event == "CHAT_MSG_MONSTER_EMOTE" ) then
		local creatureName = string.gfind(arg1, ONEHITWONDER_CHAT_UNIT_STARTS_TO_FLEE);
...
-- /script OneHitWonder_SetupStuffContinuously(); OneHitWonder_SetupOverrideBinding();
function OneHitWonder_SetupOverrideBinding()
	if ( OneHitWonder_ShouldOverrideBindings == 1 ) then
		...
		for k, v in OneHitWonder_Override_Binding_Pointers do
			if ( OneHitWonder_Saved_Binding_Pointers[v] ) then
				setglobal(v, OneHitWonder_Saved_Binding_Pointers[v]);
				OneHitWonder_Saved_Binding_Pointers[v] = nil;
			end
		end
	end
end
...
function OneHitWonder_PitchUpStart(...)
	OneHitWonder_Saved_Binding_Pointers["PitchUpStart"](unpack(arg));
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_PitchUpStop(...)
	OneHitWonder_Saved_Binding_Pointers["PitchUpStop"](unpack(arg));
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_PitchDownStart(...)
	OneHitWonder_Saved_Binding_Pointers["PitchDownStart"](unpack(arg));
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_PitchDownStop(...)
	OneHitWonder_Saved_Binding_Pointers["PitchDownStop"](unpack(arg));
	OneHitWonder_DoStuffContinuously();
end
...
function OneHitWonder_CastBuff(castBuffName, v, unit, destroyTarget, specialOptions)
	...
	if ( ok ) then
		if ( v ) then
			...
			if ( ( v.doNotBuffClass ) and ( not specialOptions.overrideClass ) ) then
			...
			if ( ( v.onlyBuffClass ) and ( not specialOptions.overrideClass ) ) then
				local class = UnitClass(unit);
			...
			if ( ( v.notInCombat == true ) and ( not specialOptions.ignoreInCombat ) ) then
				if ( PlayerFrame.inCombat == 1 ) then
					ok = false;
					OneHitWonder_AddBuffReason("Did not use buff because we were in combat");
				end
			end
			if ( ( v.notWhileHated == true ) and ( not specialOptions.ignoreWhileHated ) ) then
				if ( PlayerFrame.onHateList == 1 ) then
					ok = false;
					OneHitWonder_AddBuffReason("Did not use buff because we were on hate list");
				end
			end
			if ( ( v.onlyWhileHated == true ) and ( not specialOptions.ignoreWhileHated ) ) then
				if ( PlayerFrame.onHateList ~= 1 ) then
					ok = false;
					OneHitWonder_AddBuffReason("Did not use buff because we were on hate list");
				end
			end
			if ( ( v.requiresCombat == true ) and ( not specialOptions.ignoreRequiresCombat ) ) then
				if ( not PlayerFrame.inCombat ) then
					ok = false;
					OneHitWonder_AddBuffReason("Did not use buff because we were not in combat");
				end
			end
			if ( ( v.validTarget ) and ( not specialOptions.ignoreValidTarget ) ) then
			...
			if ( ( not specialOptions.ignoreMana ) and ( not v.useAtAnyMana ) and ( ( v.useWhenHigherManaPercentage ) or ( v.useWhenLowerManaPercentage) ) ) then
			...
			if ( ( not specialOptions.ignoreHealth  ) and ( ( v.useWhenHigherHealthPercentage ) or ( v.useWhenLowerHealthPercentage) ) ) then
			...
			if ( ( v.invalidTarget ) and ( not specialOptions.ignoreInvalidTarget ) ) then
			...
			if ( ( v.powerType ) and ( not specialOptions.ignorePowerType ) ) then
			...
			if ( ( v.notPowerType ) and ( not specialOptions.ignoreNotPowerType ) ) then
		...
		local actionId = OneHitWonder_GetActionIdFromSpellId(buffId);
		local canOverrideTarget = ( ( unit == "player" ) and ( actionId > -1 ) );
		if ( ( ok ) and ( hasTarget ) and ( not canOverrideTarget ) and ( unit ~= "target" ) and ( not UnitCanAttack("target", "player") ) ) then
			local targetName = UnitName("target");
			local unitName = UnitName(unit);
			if ( targetName ~= unitName ) then
				if ( not destroyTarget ) then
					ok = false;
					OneHitWonder_AddBuffReason("Did not use buff because we had a target and could not destroy target.");
				else
...
function OneHitWonder_ShouldTryToCastABuff(ignoreCloakEffects)
	...
	if ( OneHitWonder_NoBuffWhileFishing == 1 ) then
		local itemInfo = DynamicData.item.getEquippedSlotInfo(16);
		if ( itemInfo ) and ( itemInfo.name ) 
...
function OneHitWonder_BuffUnit(unit, destroyTarget, specialOptions)
	...
	if ( getglobal(
		format("OneHitWonder_SetupSequenceContinuously_%s", 
		OneHitWonder_GetPlayerClass() ) ) ) then
		local tmpBuff = nil;
...
ONEHITWONDER_TIME_BETWEEN_DO_STUFF_CONTINUOUSLY = 1.0;
...
function OneHitWonder_DoStuffContinuously()
	...
	if ( ( OneHitWonder_GetPlayerRace() == ONEHITWONDER_RACE_TROLL ) 
		and ( shouldTryToCastABuff ) ) then
	...
	if ( shouldTryToCastABuff ) and ( OneHitWonder_ShouldKeepBuffsUp == 1 )  then
		local trackBuffName = OneHitWonder_GetPlayerTrackingBuffName();
		
		if ( not trackBuffName ) then
			local spellId = -1;
			local shouldTrack = true;
			if ( OneHitWonder_PreferredTrackingSpell ) then
	...
	if ( ( PlayerFrame.inCombat ~= 1 ) and ( OneHitWonder_ShouldKeepBuffsUpParty == 1 ) and ( false ) ) then
		if ( not OneHitWonder_HasTarget() ) then
			...
			local partyName = nil;
			for i = 1, 4 do
				partyName = UnitName("party"..i);
				if ( partyName ) and ( targetName == partyName ) then
					if ( OneHitWonder_DoBuffs(nil, nil, { "party"..i } ) ) then
						return true;
					end
				end
			end
...
function OneHitWonder_MeleeAttack()
	...
	if ( (OneHitWonder_Attacking == false) or 
		--( PlayerFrame.inCombat ~= 1 ) 
		( false ) 
		) then
		AttackTarget();
...
function OneHitWonder_CheckEveryoneEffects()
	OneHitWonder_CheckEffect("player");
	OneHitWonder_CheckEffect("pet");
	for i = 1, 4 do 
		local unitName = "party"..i;
		if (UnitExists(unitName)) then
			OneHitWonder_CheckEffect(unitName);
		end
	end
	OneHitWonder_CheckEffect("target");
end
...
function OneHitWonder_CheckFriendlies()
	...
	for i = 1, 4 do 
		local unitName = "party"..i;
		if ( not OneHitWonder_IsUnitOffline(unitName) ) then
			OneHitWonder_CheckEffect(unitName);
		end
	end
end
...
function OneHitWonder_CheckEffect(unit)
	...
	local curTime = GetTime();
	if ( OneHitWonder_CheckUnit_Time[unit] ) and ( ( curTime - OneHitWonder_CheckUnit_Time[unit] ) < 2 ) then
		OneHitWonder_ScheduleByName(format("OneHitWonder_CheckEffect_%s", unit), 1, OneHitWonder_CheckEffect, unit);
		return;
	end
	OneHitWonder_CheckUnit_Time[unit] = curTime;
	return func(unit);
end
...
function OneHitWonder_GetDistanceInYards(unit)
	...
	if ( OneHitWonder_UnitDistanceTable[unit] ) then
		...
		if ( ( unitData.time + 0.2 ) >= curTime ) then
...
function OneHitWonder_GetHashForUnit(unit)
	...
	return OneHitWonder_GetHashForString(creature)..OneHitWonder_GetHashForString(UnitCreatureType(unit));
end
...
function OneHitWonder_IsUnitFacingMe(unit)
	...
	local unitFacing = UnitFacing(unit);
end
...
function OneHitWonder_Schedule(when, func, ... )
	if ( not when ) or ( not func ) then
		return;
	end
	if ( Cosmos_Schedule ) then
		Cosmos_Schedule(when, func, unpack(arg));
	elseif ( Chronos ) and ( Chronos.scheduleByName ) then
		Chronos.schedule(when, func, unpack(arg));
	end
end

function OneHitWonder_ScheduleByName(name, when, func, ... )
	if ( not name ) or ( not when ) or ( not func ) then
		return;
	end
	if ( not arg ) then
		arg = {};
	...
	if ( Cosmos_ScheduleByName ) then
		return Cosmos_ScheduleByName(name, when, func, unpack(arg));
	elseif ( Chronos ) and ( Chronos.scheduleByName ) then
		return Chronos.scheduleByName(name, when, func, unpack(arg));
	else
...
function OneHitWonder_HasItemEffect(effectNames, slotId)
	local curTime = GetTime();
