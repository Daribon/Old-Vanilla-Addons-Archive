--[[
	...
	$Id: Eclipse.lua 2680 2005-10-22 01:20:15Z mugendai $
	$Rev: 2680 $
	...
	if (reglist and ( type(reglist) == "table" ) and reglist.name and (reglist.name ~= "") ) then
		...
		if ( not Eclipse.Frames[reglist.name] ) then
			Eclipse.Frames[reglist.name] = {
				...
				forceHide = 0;		--If 1 will cause the frame to hide, despite all others
			...
			if (regFrame) then
				if ( regFrame.Show and regFrame.Hide ) then
					--Get the initial visibility state of the frame
					if (regFrame.IsShown) then
						--IsShown is available use it
						Eclipse.Frames[reglist.name].state.show = regFrame:IsShown();
					else
						--Otherwise use IsVisible
				...
				if ( regFrame.SetAlpha and regFrame.GetAlpha ) then
...
Eclipse.UpdateFrames = function ()
	...
	for frameName in Eclipse.Frames do
		...
		if ( Eclipse.Frames[frameName].controltrans ) then
			...
			if (uiFrame and uiFrame.GetAlpha and uiFrame.SetAlpha) then
