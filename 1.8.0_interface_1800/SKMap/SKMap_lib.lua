...
function SkM_RecordPlayerDeath()
	...
	local sKiller, KillerType = SkM_PlayerDeathResp();
...
function SkM_CleanZoneCreatureKill(idx_c, idx_z)
	...
	for i=iZoneNoteCount, 1, -1 do
		...
		if (Note[_SKM._storedInfo][_SKM._type] == _SKM._creatureKill_Target) or (Note[_SKM._storedInfo][_SKM._type] == _SKM._creatureKill_Xp) then
...
function SkM_UpdateEnemy_IncrPlayerKill(sName, iValue)
	SKM_Data[_RealmName][_PlayerName].EnemyHistory[sName][_SKM._playerKill] = ifnil(SKM_Data[_RealmName][_PlayerName].EnemyHistory[sName][_SKM._playerKill], 0) + iValue;
	...
	if ((sGuildName ~= nil) and (sGuildName ~= "")) then
		SkM_UpdateGuild_IncrPlayerKill(sGuildName, iValue);
	end
...
function SkM_UpdateEnemy_IncrPlayerFullKill(sName, iValue)
	SKM_Data[_RealmName][_PlayerName].EnemyHistory[sName][_SKM._playerFullKill] = ifnil(SKM_Data[_RealmName][_PlayerName].EnemyHistory[sName][_SKM._playerFullKill], 0) + iValue;
	...
	-- if ((sGuildName ~= nil) and (sGuildName ~= "not in a guild")) then
	if ((sGuildName ~= nil) and (sGuildName ~= "")) then
		SkM_UpdateGuild_IncrPlayerFullKill(sGuildName, iValue);
	end
	-- 09/04/2005 16:46:43 End of modification
end
...
function SkM_UpdateEnemy_IncrPlayerAssistKill(sName, iValue)
	SKM_Data[_RealmName][_PlayerName].EnemyHistory[sName][_SKM._playerAssistKill] = ifnil(SKM_Data[_RealmName][_PlayerName].EnemyHistory[sName][_SKM._playerAssistKill], 0) + iValue;
	...
	-- if ((sGuildName ~= nil) and (sGuildName ~= "not in a guild")) then
	if ((sGuildName ~= nil) and (sGuildName ~= "")) then
		SkM_UpdateGuild_IncrPlayerAssistKill(sGuildName, iValue);
	end
	-- 09/04/2005 16:46:43 End of modification
end
...
function SkM_UpdateEnemy_IncrKillPlayer(sName, iValue, bBattleground)
	if (bBattleground) then
		SKM_Data[_RealmName][_PlayerName].EnemyHistory[sName][_SKM._enemyKillBG] = ifnil(SKM_Data[_RealmName][_PlayerName].EnemyHistory[sName][_SKM._enemyKillBG], 0) + iValue;
	else
		SKM_Data[_RealmName][_PlayerName].EnemyHistory[sName][_SKM._enemyKillPlayer] = ifnil(SKM_Data[_RealmName][_PlayerName].EnemyHistory[sName][_SKM._enemyKillPlayer], 0) + iValue;
	end
	...
	-- if ((sGuildName ~= nil) and (sGuildName ~= "not in a guild")) then
	if ((sGuildName ~= nil) and (sGuildName ~= "")) then
		SkM_UpdateGuild_IncrKillPlayer(sGuildName, iValue, bBattleground);
	end
	-- 09/04/2005 16:46:43 End of modification
end
...
function SkM_UpdateEnemy_IncrMeet(sName, iValue)
	SKM_Data[_RealmName][_PlayerName].EnemyHistory[sName][_SKM._meetCount] = ifnil(SKM_Data[_RealmName][_PlayerName].EnemyHistory[sName][_SKM._meetCount], 0) + iValue;
end
...
-- 09/04/2005 16:29:27 PIng: Add Guild support
...
function SkM_UpdateGuild_IncrPlayerKill(sGuildName, iValue)
	SKM_Data[_RealmName][_PlayerName].GuildHistory[sGuildName][_SKM._playerKill] = ifnil(SKM_Data[_RealmName][_PlayerName].GuildHistory[sGuildName][_SKM._playerKill], 0) + iValue;
end

function SkM_UpdateGuild_IncrPlayerFullKill(sGuildName, iValue)
	SKM_Data[_RealmName][_PlayerName].GuildHistory[sGuildName][_SKM._playerFullKill] = ifnil(SKM_Data[_RealmName][_PlayerName].GuildHistory[sGuildName][_SKM._playerFullKill], 0) + iValue;
end

function SkM_UpdateGuild_IncrPlayerAssistKill(sGuildName, iValue)
	SKM_Data[_RealmName][_PlayerName].GuildHistory[sGuildName][_SKM._playerAssistKill] = ifnil(SKM_Data[_RealmName][_PlayerName].GuildHistory[sGuildName][_SKM._playerAssistKill], 0) + iValue;
end

function SkM_UpdateGuild_IncrKillPlayer(sGuildName, iValue, bBattleground)
	if (bBattleground) then
		SKM_Data[_RealmName][_PlayerName].GuildHistory[sGuildName][_SKM._enemyKillBG] = ifnil(SKM_Data[_RealmName][_PlayerName].GuildHistory[sGuildName][_SKM._enemyKillBG], 0) + iValue;
	else
		SKM_Data[_RealmName][_PlayerName].GuildHistory[sGuildName][_SKM._enemyKillPlayer] = ifnil(SKM_Data[_RealmName][_PlayerName].GuildHistory[sGuildName][_SKM._enemyKillPlayer], 0) + iValue;
	end
end

function SkM_UpdateGuild_IncrMeet(sGuildName, iValue)
	SKM_Data[_RealmName][_PlayerName].GuildHistory[sGuildName][_SKM._meetCount] = ifnil(SKM_Data[_RealmName][_PlayerName].GuildHistory[sGuildName][_SKM._meetCount], 0) + iValue;
end
...
-- 09/04/2005 16:32:32 End of modification
...
function SkM_PvpEnemyDeath(sName, bHonorKill, sRank)
	...
	if (KillType ~= nil) then
		...
		-- 0.08.2 Begin of modification Add guild support
		local sGuildName = SKM_Data[_RealmName][_PlayerName].EnemyHistory[sName][_SKM._guild];
		if ((sGuildName ~= nil) and (sGuildName ~= "")) then
...
function SkM_PlayerDeathResp()
	...
	table.sort(SKM_Context.PlayerCombat, SkM_SortPlayerCombat_ByHate);

	local idx, val;
	for idx, val in SKM_Context.PlayerCombat do

		-- reset hate list now that we're dead !
		SKM_Context.PlayerCombat = { };

		-- only top enemy in hate list matters
		SkM_Trace(FName, 2, "Most hated = "..idx..", type = "..snil(val[_SKM._enemyType]));
		return idx, val[_SKM._enemyType];
	end
end
...
-- --------------------------------------------------------------------------------------
function SkM_CountGuildMembers(sGuild)
...
function SkM_NormalizeString(sName)
	...
	-- well, that's far too costly it seems :/
	-- comment this out and maybe think about another way of do it

	--local idx, val, i;
	--for idx, val in SKM_ToStandardCase do
	--	for i=1, table.getn(val), 1 do
	--		sString = string.gsub(sString, val[i], idx);
	--	end
	--end
...
-- OBSOLETE
function SkM_MapShift_DoLocalShifts()
	local FName = "SkM_MapShift_DoLocalShifts";

	local iFirstShift;
	local iNbLocalShift = 0;
	local iNbShift = 0;

	if (SKM_LocalShift) then
		iNbLocalShift = table.getn(SKM_LocalShift);
	end

	if (SKM_Settings.LastLocalShift == nil) then
		iFirstShift = 1;
	else
		iFirstShift = SKM_Settings.LastLocalShift + 1;
	end

	SkM_Trace(FName, 3, "First local shift to do = "..iFirstShift);
	SkM_Trace(FName, 3, "Local shifts defined = "..iNbLocalShift);

	if (iFirstShift > iNbLocalShift) then
		-- no new local shift to perform
		return iNbShift;
	end

	local i;
	for i=iFirstShift,iNbLocalShift, 1 do

		SkM_Trace(FName, 3, "Performing local shift "..i);

		--SkM_DoMapShift(SKM_LocalShift[i].Source, SKM_LocalShift[i].Dest, SKM_LocalShift[i].ShiftTable, nil, SKM_LocalShift[i].DateShift)
		SkM_DoMapShift(SKM_LocalShift[i].Source, SKM_LocalShift[i].Dest,
			SkM_GetShiftTable(SKM_LocalShift[i].Source, SKM_LocalShift[i].Dest),
			nil, SKM_LocalShift[i].DateShift)

		iNbShift = iNbShift + 1;

		SKM_Settings.LastLocalShift = i;
		SKM_Settings.ZoneShiftSource = SKM_LocalShift[i].Source;
		SKM_Settings.ZoneShiftDest = SKM_LocalShift[i].Dest;
	end
...
function SkM_MapShiftMigration()
	...
	if (bNewLocalShift) then
		...
		if (SKM_Locale[SKM_Settings.LastLocale].CurrentShift) then
			...
			SkM_Trace(FName, 3, "Perform final shift");
			...
			SkM_DoMapShift(Source, Dest, SkM_GetShiftTable(Source, Dest), nil, nil);
...
function SkM_MapShiftMigration_OLD()
	local FName = "SkM_MapShiftMigration";
	...
	if (SKM_Settings.LastLocalShift == nil) then
		...
		SkM_Trace(FName, 3, "LastLocalShift = nil => perform all defined local shifts");

		-- no zone shift has ever been performed, we need to do them
		-- one by one (if there are any !)
		local iNbShift = SkM_MapShift_DoLocalShifts();
		if (iNbShift > 0) then
			bShift = true;
		end

		-- apply current zone shift
		if (SKM_ZoneShift) then
			SkM_Trace(FName, 3, "Perform final shift");

			SkM_DoMapShift(SKM_CurrentShift.Source, SKM_CurrentShift.Dest, SKM_ZoneShift, nil, nil);
			bShift = true;

			SKM_Settings.ZoneShiftSource = SKM_CurrentShift.Source;
			SKM_Settings.ZoneShiftDest = SKM_CurrentShift.Dest;
		end

	else
		SkM_Trace(FName, 3, "LastLocalShift != nil, check if new shifts should be applied");

		local iNbLocalShift = 0;
		if (SKM_LocalShift) then
			iNbLocalShift = table.getn(SKM_LocalShift);
		end
		if (iNbLocalShift > SKM_Settings.LastLocalShift) then
			-- new shifts are defined since last one we performed

			local iNextShift = SKM_Settings.LastLocalShift + 1;

			SkM_Trace(FName, 3, "There are new shifts. First apply reverse shift");

			-- apply reverse shift to correct erroneous shift applied since patch

			SkM_DoMapShift(
				SKM_Settings.ZoneShiftDest, SKM_Settings.ZoneShiftSource,
				SkM_GetShiftTable(SKM_Settings.ZoneShiftDest, SKM_Settings.ZoneShiftSource),
				SKM_LocalShift[iNextShift].DateShift, nil);
			...
			SkM_Trace(FName, 3, "Now do new shifts");

			-- then apply new shifts
			SkM_MapShift_DoLocalShifts();
	...
	-- check current localization against source zone shift
	-- if they differ, user has changed language, so we must take it into account
	if (SKM_Settings.ZoneShiftSource) then
		local LocaleVer = string.upper(string.sub(GetLocale(), 1, 2));
		--LocaleVer = "FR"; -- for tests !
		local ShiftVer = string.upper(string.sub(SKM_Settings.ZoneShiftSource, 1, 2));

		if (LocaleVer ~= ShiftVer) then
			SkM_Trace(FName, 1, "Locale version switch detected : current = "..LocaleVer..", last known = "..ShiftVer);

			SKM_Settings.ZoneShiftSource = LocaleVer..string.sub(SKM_Settings.ZoneShiftSource, 3, string.len(SKM_Settings.ZoneShiftSource));

			SkM_Trace(FName, 3, "New ZoneShiftSource = "..SKM_Settings.ZoneShiftSource);
		end
	end
	...
	if (bShift) then
		local endTime = GetTime();
		local elapsedTime = math.floor(100 * (endTime - curTime)) / 100;
		SkM_ChatMessageCol("Zone order shifted. Current shift : "..SKM_Settings.ZoneShiftDest);
	end
