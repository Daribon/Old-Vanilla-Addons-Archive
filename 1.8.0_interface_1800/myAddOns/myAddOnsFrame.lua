...
local MYADDONS_NAME = "myAddOns";
local MYADDONS_VERSION = "2.4";
local MYADDONS_AUTHOR = "Scheid";
local MYADDONS_EMAIL = "scheid@free.fr";
local MYADDONS_WEBSITE = "http://scheid.free.fr";
local myAddOnsTree = {};
...
function myAddOnsFrame_OnEvent()
	...
	if (event == "ADDON_LOADED" and arg1 == MYADDONS_NAME) then
...
function myAddOnsLoadOptionsCharactersListFrameButton_OnClick()
	...
	-- Get the index and character of the button that was clicked
	local index = this:GetID() + FauxScrollFrame_GetOffset(myAddOnsLoadOptionsCharactersListFrameScrollFrame);
	local character = myAddOnsCharactersTree[index];
	
	-- Check if the button is a character
	if (character) then
		-- Set the selected character
		myAddOnsCharactersTree.selectedIndex = index;
...
function myAddOnsFrame_Initialize()
	...
	myAddOnsCategories[MYADDONS_CATEGORY_AUDIO] = {expanded = 1, addonsCount = 0};
	myAddOnsCategories[MYADDONS_CATEGORY_BARS] = {expanded = 1, addonsCount = 0};
	myAddOnsCategories[MYADDONS_CATEGORY_BATTLEGROUNDS] = {expanded = 1, addonsCount = 0};
	...
	myAddOnsCategories[MYADDONS_CATEGORY_DEVELOPMENT] = {expanded = 1, addonsCount = 0};
	...
	myAddOnsCategories[MYADDONS_CATEGORY_PLUGINS] = {expanded = 1, addonsCount = 0};
	...
	myAddOnsCategories[MYADDONS_CATEGORY_UNKNOWN] = {expanded = 1, addonsCount = 0};
	...
	local myAddOnsDetails = {
		name = MYADDONS_NAME,
		version = MYADDONS_VERSION,
		releaseDate = MYADDONS_RELEASE_DATE,
		author = MYADDONS_AUTHOR,
		email = MYADDONS_EMAIL,
		website = MYADDONS_WEBSITE,
		category = MYADDONS_CATEGORY_OTHERS
	};
	
	-- Initialize the addon's help
	local myAddOnsHelp = {};
	myAddOnsHelp[1] = MYADDONS_HELP_PAGE1;
	
	-- Register the addon in myAddOns
	myAddOnsFrame_Register(myAddOnsDetails, myAddOnsHelp);
...
function myAddOnsLoadOptionsCharactersListFrame_UpdateHighlight()
	...
	-- Get the selected index
	local selectedIndex = myAddOnsCharactersTree.selectedIndex;
...
-- Update the addons list
function myAddOnsFrame_UpdateList()

	-- Browse the old registration addons list
...
function myAddOnsFrame_Register(addonDetails, addonHelp)
	...
	if (addon) then
		...
		if (myAddOnsCategories[addon.category]) then
...
function myAddOnsFrame_BuildTree()
	...
	for categoryIndex in tempCategories do
		...
		myAddOnsCategories[category].addonsCount = 0;
		...
		for addonName in myAddOnsAddOns do
			...
			if (addon.category == category) then
				...
				if (enabled and loadable) then
					...
					tempAddOns[addonIndex] = addonName;
				end
			end
		end
		...
		if (table.getn(tempAddOns) > 0) then
			...
			index = index + 1;
			myAddOnsTree[index] = category;
			-- Browse the temporary addons list
			for addonIndex in tempAddOns do
				index = index + 1;
				myAddOnsCategories[category].addonsCount = myAddOnsCategories[category].addonsCount + 1;
				myAddOnsTree[index] = tempAddOns[addonIndex];
			end
		end
	end
...
function myAddOnsFrame_GetNumVisibleItems()
	...
	for index in myAddOnsTree do
		...
		if (category) then
			...
			if (category.addonsCount > 0) then
				numVisibleItems = numVisibleItems + 1;
			end
			-- Check if the category is expanded
			if (category.expanded) then
				numVisibleItems = numVisibleItems + category.addonsCount;
			end
		end
	end
...
function myAddOnsFrame_GetSelectedLine()
	...
	for index = 1, myAddOnsTree.selectedIndex, 1 do
		...
		elseif (category) then
			-- Check if the category is visible
			if (myAddOnsCategories[item].addonsCount > 0) then
				selectedLine = selectedLine + 1;
			end
		end
...
function myAddOnsFrame_BuildCharactersTree()
	...
	for realmName in realms do
		for character in realms[realmName] do
			index = index + 1;
			myAddOnsCharactersTree[index] = realmName.." - "..character;
		end
	end
