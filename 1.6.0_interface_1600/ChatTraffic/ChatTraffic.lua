...
function ChatTraffic_Register_Cosmos()
	if ( ( Cosmos_UpdateValue ) and ( Cosmos_RegisterConfiguration ) and ( ChatTraffic_Cosmos_Registered == 0 ) ) then
...
function ChatTraffic_DoNothingAndFail()
	return false;
end

function ChatTraffic_SetUpScheduling()
	local func = ChatTraffic_DoNothingAndFail;
	...
	setglobal("ChatTraffic_ScheduleByName", func);
	func = ChatTraffic_DoNothingAndFail;
	if ( Chronos ) and ( Chronos.schedule ) then
		func = Chronos.schedule;
	elseif ( Cosmos_Schedule ) then
		func = Cosmos_Schedule;
	end
	setglobal("ChatTraffic_Schedule", func);
end

-- Does things with the hooked function
function ChatTraffic_SendChatMessage(message, p1, p2, p3, p4, p5, p6, p7, p8, p9)
	if ( ( ChatTraffic_Enabled == 1 ) and ( not ChatTraffic_ExemptFromTrafficLimitation(message) ) ) then
		...
		if ( messageLen >= maxTraffic ) then
			if ( currentTrafficLeft >= (maxTraffic*CHATTRAFFIC_TRAFFIC_BIG_MESSAGE_CHUNK) ) then
				ChatTraffic_Saved_SendChatMessage(message, p1, p2, p3, p4, p5, p6, p7, p8, p9);
				ChatTraffic_AddTrafficSent(message);
				return;
			end
		end
		if ( currentTrafficLeft < messageLen ) then
			ChatTraffic_Schedule(CHATTRAFFIC_TRAFFIC_DELAY, ChatTraffic_SendChatMessage, message, p1, p2, p3, p4, p5, p6, p7, p8, p9);
			return;
		else
			ChatTraffic_AddTrafficSent(message);
		end
...
function ChatTraffic_Generic_Toggle(toggle, variableName, CVarName, enableMessage, disableMessage, CosmosVarName)
	...
	if ( ChatTraffic_Cosmos_Registered == 1 ) then 
