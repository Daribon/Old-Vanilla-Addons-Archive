...
if ( GetLocale() == "deDE" ) then
	...
	CLOCK_TIME_DAY				= "%d Tag(e)";
	CLOCK_TIME_HOUR				= "%d Stunde(n)";
	CLOCK_TIME_MINUTE			= "%d Minute(n)";
	CLOCK_TIME_SECOND			= "%d Sekunde(n)";
	EXP_PER_HOUR_LEVEL			= "Erfahrung pro Stunde dieses Level: %.2f";
	EXP_PER_HOUR_SESSION			= "Erfahrung pro Stunde diese Sitzung: %.2f";
	EXP_TO_LEVEL				= "Erfahrung dieses Level: %d (noch %.2f%% bis Levelaufstieg)";
	TIME_TO_LEVEL_LEVEL			= "Zeit bis Levelaufstieg: %s\n     (bei gleichbleibender XP-Rate des aktuellen Levels)";
	TIME_TO_LEVEL_SESSION			= "Zeit bis Levelaufstieg: %s\n     (bei gleichbleibender XP-Rate dieser Sitzung)";
	TIME_INFINITE				= "unendlich";
	HEALTH_PER_SECOND			= "Gesundheits-Regeneration pro Sekunde : %d";
	MANA_PER_SECOND				= "Mana-Regeneration pro Tick: %d";
	NONCOMBAT_TRAVEL_PERCENTAGE		= "Verbrachte Zeit auf Reisen diese Sitzung: %.2f%%";
	NONCOMBAT_TRAVEL_DISTANCE		= "Zur\195\188ckgelegte Enfernung diese Sitzung: %.2f";
	TRAVEL_SPEED				= "Reisegeschwindigkeit (in Prozent der Laufgeschwindigkeit): %.2f%%";

	CLOCK_MONEY				= "Geld eingenommen pro Stunde diese Sitzung: %s";
