...
Additional Note: Main idea: CTmod, requested on ultimateuiui.org
		...
		Sinaloit - Converted from UltimateUI_RegisterChatCommand to Sky.registerSlashCommand, updated default slash commands
				fixed thottbot_replace, Sky users can now send notes to non Sky users (cannot receive though).


]]

MAPNOTES_VERSION = "1.5c";
...
MapNotes_Const[MAPNOTES_WARSONGGULCH] = { scale = 0.035, xoffset = 0.41757282062541, 
                         yoffset = 0.33126468682991, xscale = 12897.3, yscale = 8638.1 };
MapNotes_Const[MAPNOTES_ALTERACVALLEY] = { scale = 0.13, xoffset = 0.41757282062541, 
                         yoffset = 0.33126468682991, xscale = 12897.3, yscale = 8638.1 };

function MapNotes_ConvertOldFrenchNotes()
   local temp = { };

   if( not MapNotes_ZoneShiftOld ) then
       MapNotes_StatusPrint("This only works for the French client!");
       return;
    end
   ...
   for z=1, 2, 1 do
      for index, value in MapNotes_Data[z] do
         local zone;
         for index2, value2 in MapNotes_ZoneShiftOld[z] do
            if( value2 == index ) then
	       zone = index2;
	    end
	 end

         temp[MapNotes_ZoneShift[z][zone]] = value;
      end
      MapNotes_Data[z] = { };
      for index, value in temp do
         MapNotes_Data[z][index] = value;
      end
      temp = { };
   end
   MapNotes_StatusPrint("Conversion complete. Please check your notes. Don't run this function again!");
end


function MapNotes_ConvertOldNotes()
   local temp = { };

   for z=1, 2, 1 do
      for index, value in MapNotes_Data[z] do
         temp[MapNotes_ZoneShift[z][index]] = value;
      end
      MapNotes_Data[z] = { };
      for index, value in temp do
         MapNotes_Data[z][index] = value;
...
local function CurrentZoneFix_SetupFix()
	...
	-- email: legorol@ultimateuiui.org
...
function MapNotes_OnLoad()
	...
	if (Sky) then
		...
		Sky.registerSlashCommand(
			{
				...
				commands = MAPNOTES_ONENOTE_COMMANDS;
		...
		for i = 1, table.getn(MAPNOTES_ONENOTE_COMMANDS) do setglobal("SLASH_ONENOTE"..i, MAPNOTES_ONENOTE_COMMANDS[i]); end
		...
		-- hooking the chat when Sky is not installed
...
function MapNotes_Quicknote(msg)
	...
	local zone;
        ...
        if( continent == -1 ) then
	   if( not MapNotes_Data[GetZoneText()] ) then
	      MapNotes_Data[GetZoneText()] = { };
	   end
	   ...
	   if( not MapNotes_Data[continent][zone] ) then
	      MapNotes_Data[continent][zone] = { };
	   end
	...
	elseif ((x == 0 and y == 0) or continent == 0) then
		...
		if (MapNotes_SetNextAsMiniNote ~= 2) then
			...
			if (i < MapNotes_NotesPerZone) then
				...
				MapNotes_StatusPrint(format(MAPNOTES_QUICKNOTE_TOOMANY, GetZoneText()));
			end
		end
		if (MapNotes_SetNextAsMiniNote ~= 0) then
			MapNotes_MiniNote_Data.xPos = x;
	...
	elseif (event == "VARIABLES_LOADED") then
		...
		MapNotes_ImportCartoPlus(); -- for testin only, doesn't work correctly, because of the word wraps...
                ...
                if(myAddOnsFrame) then
                     myAddOnsList.MapNotes = {
                      name = "Map Notes",
                      description = MAPNOTES_ADDONDESCRIPTION,
                      version = MAPNOTES_VERSION,
                      category = MYADDONS_CATEGORY_MAP,
                      frame = "MapNotesOptionsMenuFrame"
                   };
       end
...
function MapNotes_MiniNote_OnUpdate(arg1)
   ...
   if ( MapNotes_MiniNote_Data.xPos ~= nil ) then
	 ...
	 local zoneText;
         ...
         if( MapNotes_MiniNote_Data.continent > 0 and 
	     (continent ~= MapNotes_MiniNote_Data.continent or  zone ~= MapNotes_MiniNote_Data.zone)) then
	              MiniNotePOI:Hide();
	              return;
	end
	 ...
	 if( continent < 1 ) then
	    zoneText = GetZoneText();
            ...
            zone = MapNotes_ZoneShift[continent][zone];
         end
	   ...
	   return;
        ...
        local currentConst = nil;

        if( continent == 0 ) then
           currentConst = MapNotes_Const[zoneText];
	elseif( continent > 0 and MapNotes_MiniNote_Data.continent > 0) then
	   currentConst = MapNotes_Const[MapNotes_MiniNote_Data.continent]
	                     [      MapNotes_ZoneShift[ MapNotes_MiniNote_Data.continent]
			                                [MapNotes_MiniNote_Data.zone]
			    ];
        else
	   MiniNotePOI:Hide();
	   return;
...
function MapNotes_ShowSpecialActionsFrame()
	...
	elseif (MapNotes_TempData_Id == -1) then
		...
		ax = currentZone[MapNotes_TempData_Id].xPos;
		ay = currentZone[MapNotes_TempData_Id].yPos;
	end
	MapNotesPOIMenuFrame:Hide();
	if (ax*1002 >= (1002 - 195)) then xOffset = -176; else xOffset = 0;	end
...
function MapNotes_WorldMapButton_OnClick(mouseButton, button)
	...
	if ( mouseButton == "LeftButton" ) then
		...
		if (MapNotes_BlockingFrame()) then
			...
			if( (continent == 2 and zone == MapNotes_ZoneShift[continent][12]) or        -- Ironforge
			    (continent == 2 and zone == MapNotes_ZoneShift[continent][22]) or        -- Undercity
			    (continent == 1 and zone == MapNotes_ZoneShift[continent][19]) ) then    -- Thunder Bluff
                               callOriginal = false; 
			else
			   callOriginal = true;
			end
...
function MapNotes_WorldMapButton_OnUpdate()
   ...
   elseif( continent > 0 ) then
	...
	if( WorldMapPlayer:IsVisible() ) then
   	   WorldMapMapNotesPlayer:SetPoint("CENTER", "WorldMapPlayer", "CENTER", 0, 0);
           WorldMapMapNotesPlayer:SetFrameLevel(WorldMapPlayer:GetFrameLevel()+1);
