...
function OneHitWonder_Priest_Cosmos()
	if ( Cosmos_RegisterConfiguration ) and ( Cosmos_UpdateValue ) then
		...
		Cosmos_RegisterConfiguration(
			...
			OneHitWonder_Priest_ShouldShieldCritical,  -- checked
			OneHitWonder_Priest_ShieldCriticalBelowHP, -- default value
			1, -- min value
			40, -- max value
...
function OneHitWonder_Priest_SWP()
	...
	if ( OneHitWonder_Priest_AggressiveWonder == 1 ) then
		...
		spellId = OneHitWonder_GetSpellId(ONEHITWONDER_SPELL_HOLY_FIRE_NAME);
...
function OneHitWonder_Priest(removeDefense)
	...
	if ( OneHitWonder_Priest_AggressiveWonder == 1 ) then
		...
		if ( ( PlayerFrame.inCombat == 1 ) or ( OneHitWonder_IsInPartyOrRaid() ) ) and ( OneHitWonder_Priest_SWP() ) then
			return true;
		end
	end
	...
	if ( not OneHitWonder_DoBuffs() ) then
		if ( OneHitWonder_Priest_AggressiveWonder == 1 ) then
			...
			if ( OneHitWonder_Priest_UseMindBlastInGroups == 1 ) or ( not OneHitWonder_IsInPartyOrRaid() ) then
...
function OneHitWonder_Priest_ShieldCriticalInCombat()
	if ( PlayerFrame.inCombat == 1 ) and ( OneHitWonder_Priest_ShieldCritical() ) then
		return true;
	else
		return false;
	end
...
function OneHitWonder_DoStuffContinuously_Priest()
	...
	if ( OneHitWonder_Priest_AutoHoTInCombat == 1 ) then
		if ( OneHitWonder_HoTInCombat() ) then
			return true;
		end
	end
