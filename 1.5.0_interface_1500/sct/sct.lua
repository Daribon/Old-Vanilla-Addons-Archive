...
function SCT_Initialize()
	...
	if(myAddOnsList) then
		myAddOnsList.SCT = {
			...
			description = "Scroll Self Damage and Events above your character",
			version = SCT_Version,
...
function SCT_Display(msg, color, iscrit)
	...
	if (iscrit == 1) then
		...
		if (SCT_Get("STICKYCRIT") == 1) then
			...
			if (critcount > 1) then
				local totaloffset = SCT_CRIT_X_OFFSET * (critcount/2);
				--alternate left to right
				if (mod(critcount, 2) == 1) then
					adat.posX = adat.posX - totaloffset;
				else
					adat.posX = totaloffset;
				end
			end
		end
	else
