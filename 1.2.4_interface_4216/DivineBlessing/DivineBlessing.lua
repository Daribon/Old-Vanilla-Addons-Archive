...
function DivineBlessing_Reset()
	...
	if ( UnitClass("player") == DIVINEBLESSING_CLASS_PALADIN ) then 
--		DivineBlessing_ButtonMap[1].wait = 0;
		local blessingNames = {
			DIVINEBLESSING_BLESSING_OF_MIGHT_SPELL_NAME,
		};
		local blessingArray = {};
		for id=1,180,1 do 
			spellName, subSpellName = GetSpellName(id, "spell");
			
			for k, v in blessingNames do
				if ( v == spellName ) then
					blessingArray[v] = id;
				end
			end
		end
		local id = blessingArray[blessingNames[1]];
		if ( id ) then
...
function DivineBlessing_OnLoad()
	...
	if ( header ) then
		-- helptext:SetText(DIVINEBLESSING_INSTRUCTION);
		header:SetText("|cFFee9966"..DIVINEBLESSING_BUTTON_TITLE.."|r");
	end
...
--			);
		...
		Cosmos_RegisterButton (
			DIVINEBLESSING_BUTTON_TITLE,
			DIVINEBLESSING_BUTTON_SUBTITLE,
			DIVINEBLESSING_BUTTON_TIP,
