...
function OneHitWonder_Hunter_ShouldWingClipUnit(unit)
	if ( ( ( UnitHealth(unit) / UnitHealthMax(unit) ) * 100 ) 
		<= OneHitWonder_Hunter_WingClipHealthPercentage ) then
		return true;
	else
		return false;
	end
...
function OneHitWonder_Hunter(removeDefense)
	...
	local inMarkRange = OneHitWonder_CheckIfInRangeSpellId(OneHitWonder_GetSpellId(ONEHITWONDER_SPELL_HUNTERS_MARK_EFFECT));
	...
	if ( inAutoShotRange ) then
		...
		local spellId = OneHitWonder_GetSpellId(ONEHITWONDER_SPELL_AIMED_SHOT_NAME);
		if ( OneHitWonder_IsSpellAvailable(spellId ) ) 
		and ( OneHitWonder_CastSpellOnTarget(spellId) ) then
			return true;
		end
		if ( not OneHitWonder_HasUnitEffect("target", nil, OneHitWonder_Hunter_Concussive_Shot_Effects) ) then
	...
	elseif ( inMeleeRange ) then
		...
		if ( OneHitWonder_Hunter_ShouldWingClipUnit("target") ) and ( not OneHitWonder_HasUnitEffect("target", nil, OneHitWonder_Hunter_Wing_Clip_Effects) ) then
