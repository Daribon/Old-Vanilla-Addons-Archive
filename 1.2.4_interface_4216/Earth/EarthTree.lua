...
function EarthTree_LoadTable(frame, data, funcTable, keyOrder)
	...
	if ( type(keyOrder) ~= "table" ) then 
		for k,v in data do 
			local entry = EarthTree_CreateEntry(k,v,funcTable,nil);
		...
		for k,v in keyOrder do 
			local entry = EarthTree_CreateEntry(v,data[v],funcTable,nil);
...
function EarthTree_UpdateFrame(frame)
	...
	-- ScrollFrame update
	FauxScrollFrame_Update(getglobal(frame:GetName().."ListScrollFrame"), table.getn(flat), frame.titleCount, EARTHTREE_TITLE_HEIGHT, getglobal(frame:GetName().."HighlightFrame"), getglobal(frame:GetName().."HighlightFrame"):GetWidth(), getglobal(frame:GetName().."HighlightFrame"):GetHeight() );
...
--
--]]
function EarthTree_GetTooltipText(frame, id)
		...
		if ( item ) then
			...
			if ( item.check ) then 
				...
				if ( item.checked ) then 
					getglobal(frame:GetName().."Title"..i.."Check"):SetChecked(item.checked);
				else
					getglobal(frame:GetName().."Title"..i.."Check"):SetChecked(false);						
				end
			...
			-- Indent the text
			getglobal(frame:GetName().."Title"..i.."Button"):SetPoint("TOPLEFT",frame:GetName().."Title"..i,"TOPLEFT",EARTHTREE_EXPAND_INDENT*(ldepth), 0 );
			getglobal(frame:GetName().."Title"..i.."Button"):SetWidth( getglobal(frame:GetName().."Title"..i):GetWidth() - ldepth * EARTHTREE_EXPAND_INDENT );

			-- Set the text width
			if ( item.right ) then 
				getglobal(frame:GetName().."Title"..i.."ButtonNormalText"):SetWidth( getglobal(frame:GetName().."Title"..i):GetWidth() - getglobal(frame:GetName().."Title"..i.."Tag"):GetStringWidth() - ldepth * EARTHTREE_EXPAND_INDENT - 2 );
				getglobal(frame:GetName().."Title"..i.."ButtonHighlightText"):SetWidth( getglobal(frame:GetName().."Title"..i):GetWidth() - getglobal(frame:GetName().."Title"..i.."Tag"):GetStringWidth() - ldepth * EARTHTREE_EXPAND_INDENT - 2 );
				getglobal(frame:GetName().."Title"..i.."ButtonDisabledText"):SetWidth( getglobal(frame:GetName().."Title"..i):GetWidth() - getglobal(frame:GetName().."Title"..i.."Tag"):GetStringWidth() - ldepth * EARTHTREE_EXPAND_INDENT - 2 );
			else
				getglobal(frame:GetName().."Title"..i.."ButtonNormalText"):SetWidth( getglobal(frame:GetName().."Title"..i):GetWidth() - ldepth * EARTHTREE_EXPAND_INDENT - 2 );
				getglobal(frame:GetName().."Title"..i.."ButtonHighlightText"):SetWidth( getglobal(frame:GetName().."Title"..i):GetWidth() - ldepth * EARTHTREE_EXPAND_INDENT - 2 );
				getglobal(frame:GetName().."Title"..i.."ButtonDisabledText"):SetWidth( getglobal(frame:GetName().."Title"..i):GetWidth() - ldepth * EARTHTREE_EXPAND_INDENT - 2 );
			end
...
function EarthTree_CreateEntry(k,v,funcTable,parents)
	local entry = {};
	...
	elseif ( type(k) == "string" ) then 
		...
		entry.titleColor = EARTHTREE_COLOR_UKNOWN;
	end
	...
	if ( type(v) == "table" ) then
		...
		for k2,v2 in v do 
			...
			-- Add kids
			local kid = EarthTree_CreateEntry(k2,v2,funcTable,newParent);
...
--
--]]
function EarthTree_HandleAction (frame, id, action, ...)
	...
	elseif ( action == "ENTER_EXPAND" ) then 
		local item = EarthTree_GetItemByID(frame,id);
		if ( item.collapsed ) then 
			local message = EARTHTREE_EXPAND_INFO..EarthTree_GetNodeCount( item );
			EarthTree_SetTooltip(frame, message);
		end
		update = false;
...
function EarthTree_ExpandButton_OnLoad()
	...
	getglobal(this:GetName()):SetNormalTexture("Interface\\Buttons\\UI-PlusButton-Up");
...
function EarthTree_ExpandButton_OnMouseWheel(delta)
	local offset = FauxScrollFrame_GetOffset(getglobal(this:GetParent():GetParent():GetName().."ListScrollFrame"));
	local maxOffset = EarthTree_GetFullCount( this:GetParent():GetParent().activeTable, true) - this:GetParent():GetParent().titleCount;
	if ( type(offset) == "number" ) then 
		if ( offset - delta >= 0 and offset - delta <= maxOffset ) then
			FauxScrollFrame_SetOffset(getglobal(this:GetParent():GetParent():GetName().."ListScrollFrame"), offset - delta );
			EarthTree_UpdateFrame(this:GetParent():GetParent());
		elseif ( offset - delta > maxOffset ) then 
			if ( maxOffset < 0 ) then 
				offset = 0;
				maxOffset = 1;
			else
			...
			local scrollbar = getglobal(this:GetParent():GetParent():GetName().."ListScrollFrameScrollBar");
			local min,max = scrollbar:GetMinMaxValues();
			scrollbar:SetValue(offset/maxOffset*max);

			FauxScrollFrame_SetOffset(getglobal(this:GetParent():GetParent():GetName().."ListScrollFrame"), offset);
			EarthTree_UpdateFrame(this:GetParent():GetParent());
		end
	end
end
...
function EarthTree_Frame_OnMouseWheel(delta)
	local offset = FauxScrollFrame_GetOffset(getglobal(this:GetName().."ListScrollFrame"));
	local maxOffset = EarthTree_GetFullCount( this.activeTable, true) - this.titleCount ;
	
	if ( type(offset) == "number" ) then 
		if ( offset - delta >= 0 and offset - delta <= maxOffset ) then
			FauxScrollFrame_SetOffset(getglobal(this:GetName().."ListScrollFrame"), offset - delta );
			EarthTree_UpdateFrame(this);
		elseif ( offset - delta > maxOffset ) then 
			if ( maxOffset < 0 ) then 
				offset = 0;
			...
			local min,max = scrollbar:GetMinMaxValues();
			--Sea.io.printComma(min,max,offset,maxOffset);
			scrollbar:SetValue(offset/maxOffset*max);

			FauxScrollFrame_SetOffset(getglobal(this:GetName().."ListScrollFrame"), offset);
			EarthTree_UpdateFrame(this);
		end
	end
end
