...
function OneHitWonder_MoveForwardStart(p1)
	OneHitWonder_Saved_Binding_Pointers["MoveForwardStart"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_MoveForwardStop(p1)
	OneHitWonder_Saved_Binding_Pointers["MoveForwardStop"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_MoveBackwardStart(p1)
	OneHitWonder_Saved_Binding_Pointers["MoveBackwardStart"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_MoveBackwardStop(p1)
	OneHitWonder_Saved_Binding_Pointers["MoveBackwardStop"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_TurnLeftStart(p1)
	OneHitWonder_Saved_Binding_Pointers["TurnLeftStart"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_TurnLeftStop(p1)
	OneHitWonder_Saved_Binding_Pointers["TurnLeftStop"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_TurnRightStart(p1)
	OneHitWonder_Saved_Binding_Pointers["TurnRightStart"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_TurnRightStop(p1)
	OneHitWonder_Saved_Binding_Pointers["TurnRightStop"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_StrafeLeftStart(p1)
	OneHitWonder_Saved_Binding_Pointers["StrafeLeftStart"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_StrafeLeftStop(p1)
	OneHitWonder_Saved_Binding_Pointers["StrafeLeftStop"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_StrafeRightStart(p1)
	OneHitWonder_Saved_Binding_Pointers["StrafeRightStart"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_StrafeRightStop(p1)
	OneHitWonder_Saved_Binding_Pointers["StrafeRightStop"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_ActionButtonDown(p1)
	OneHitWonder_Saved_Binding_Pointers["ActionButtonDown"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_ActionButtonUp(p1)
	OneHitWonder_Saved_Binding_Pointers["ActionButtonUp"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_BonusActionButtonDown(p1)
	OneHitWonder_Saved_Binding_Pointers["BonusActionButtonDown"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_BonusActionButtonUp(p1)
	OneHitWonder_Saved_Binding_Pointers["BonusActionButtonUp"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_CameraOrSelectOrMoveStart(p1)
	OneHitWonder_Saved_Binding_Pointers["CameraOrSelectOrMoveStart"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_CameraOrSelectOrMoveStop(p1)
	OneHitWonder_Saved_Binding_Pointers["CameraOrSelectOrMoveStop"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_TurnOrActionStart(p1)
	OneHitWonder_Saved_Binding_Pointers["TurnOrActionStart"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_TurnOrActionStop(p1)
	OneHitWonder_Saved_Binding_Pointers["TurnOrActionStop"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_PitchUpStart(p1)
	OneHitWonder_Saved_Binding_Pointers["PitchUpStart"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_PitchUpStop(p1)
	OneHitWonder_Saved_Binding_Pointers["PitchUpStop"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_PitchDownStart(p1)
	OneHitWonder_Saved_Binding_Pointers["PitchDownStart"](p1);
	OneHitWonder_DoStuffContinuously();
end

function OneHitWonder_PitchDownStop(p1)
	OneHitWonder_Saved_Binding_Pointers["PitchDownStop"](p1);
	OneHitWonder_DoStuffContinuously();
end
	...
	elseif ( OneHitWonder_GetPlayerRace() == ONEHITWONDER_RACE_DWARF ) then
		...
		if ( OneHitWonder_IsSpellAvailable(spellId) ) and ( OneHitWonder_UseStoneform == 1 ) then
			...
			if ( debuffsByType[ONEHITWONDER_DEBUFF_TYPE_DISEASE] ) and ( table.getn(debuffsByType[ONEHITWONDER_DEBUFF_TYPE_DISEASE]) > 0 ) then
				local n = OneHitWonder_ClassesThatCanCure[ONEHITWONDER_DEBUFF_TYPE_DISEASE][OneHitWonder_GetPlayerClass()];
				local nId = OneHitWonder_GetSpellId(n);
				if ( not n ) or ( n <= -1 ) then
					shouldStoneform = true;
				else
					local ok = OneHitWonder_CastSpell(nId);
					if ( ok ) then
						SpellTargetUnit("player");
					end
					return ok;
...
function OneHitWonder_HoTInCombat()
	if ( PlayerFrame.inCombat == 1 ) and ( OneHitWonder_HoTTime() ) then
		return true;
	else
		return false;
	end
...
function OneHitWonder_UnitAliveEnemy(unit)
	if ( UnitExists(unit) ) then
		if ( not UnitIsDead(unit) ) and ( UnitCanAttack(unit, "player") ) then
			return true;
		else
			return false;
		end
