...
function FB_Command_DisplayMisc()
	...
	util:Print("For Show/Hide Grid, lock and text, group=<group> may be substituted for Button=<...>")
end

function FB_Command_DisplayAppearance()
	FBcmd:DisplayUsage("appearance")
	util:Print("For all commands, group=<group> may be substituted for Button=<...>")
end

function FB_Command_DisplayGroup()
	FBcmd:DisplayUsage("group")
	util:Print("The anchor button MUST be a part of the proposed group")
end

function FB_Command_DisplayMovement()
	FBcmd:DisplayUsage("movement")
	util:Print("Once grouped, only the anchor may be locked/unlocked")
	util:Print("MoveXXX only takes positive arguments currently")
end
...
-- Check basic usage
	...
	if FBState[buttonnum]["group"] ~= FB_GetButtonNum(button) then
		util:Echo("No such group", "green")
		return
	end
	FB_CircularGroup(args["group"], args["padding"])
	FB_Report(args["group"] .. " auto-arranged")
	FB_ResyncSettings()
	FB_ApplySettings()
	FBGroupData[args["group"]] = FB_GetBoundingButtons(args["group"])
end
...
-- Groups must have more than one members
	if buttons.n < 2 then 
		util:Echo("Error: Groups must have more than one member")
		return
	end
-- Anchor must also be a member of the set of buttons being made into a group
	...
	if not goodanchor then
		util:Echo("Error: Anchor must be a member of the buttons being formed into a group")
		return
	end
...
-- Check basic usage
	...
	for index = 1,buttons.n do
		if FBState[buttons[index]]["group"] == buttons[index] then
			FB_Report("Disbanding group " .. buttons[index])
...
-- Check basic usage
	...
	if (args["group"] == nil) then
		...
		local text = "The following groups are present:"
		for index = 1, FBNumButtons do
	...
	local index
	local text = "Buttons in group " .. args["group"] .. " :"
	for index = 1, buttons.n do
