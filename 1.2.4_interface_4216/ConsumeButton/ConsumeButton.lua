	...
	for bag = startBag, 4 do
		for slot = startSlot, GetContainerNumSlots(bag) do
			...
			if ( itemInfo.name ) and ( strlen(itemInfo.name) > 0 ) then
				if (( itemInfo.classification == CONSUMEBUTTON_CLASSIFICATION_FOOD ) or ( itemInfo.classification == CONSUMEBUTTON_CLASSIFICATION_DRINK ) ) then
					if ( not strfind(CONSUMEBUTTON_NAME_RECIPE_SUBSTRING, itemInfo.name) ) then
