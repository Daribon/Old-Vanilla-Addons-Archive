...
function FB_GetSpellDataByID(id, data)
  -- taken from cncfanatics' code. This function returns spell data we don't
  -- have in our tooltips. For 2.0, I suggest expanding the tooltip to its
  -- full glory.
  -- XXX: Please optimize me.
  -- TODO: Please optimize me.
	if(id < 1 or id > 120) then return(""); end;
	FlexBarTooltip:SetAction(id);
	spelldata = TheoryCraft_GetSpellDataByFrame(FlexBarTooltip);
	if(spelldata == nil)  then return(""); end;
	if(spelldata[data] == nil) then return(""); end;
	return(tonumber( string.format("%.".."0".."f", spelldata[data] ) ));
end
...
-- for the given button number, determine if the hotkey or text2 attributes are dynamic and set the text
	...
	elseif FBState[buttonnum]["hotkeytext"] ~= nil and string.sub(FBState[buttonnum]["hotkeytext"], 1, 4) == "%tc_" then
		text:SetText(FB_GetSpellDataByID(buttonnum, string.sub(FBState[buttonnum]["hotkeytext"], 5)));
	...
	elseif FBState[buttonnum]["text2"] ~= nil and string.sub(FBState[buttonnum]["text2"], 1, 4) == "%tc_" then
		text:SetText(FB_GetSpellDataByID(buttonnum, string.sub(FBState[buttonnum]["text2"], 5)));
	...
	elseif FBState[buttonnum]["text3"] ~= nil and string.sub(FBState[buttonnum]["text3"], 1, 4) == "%tc_" then
		text:SetText(FB_GetSpellDataByID(buttonnum, string.sub(FBState[buttonnum]["text3"], 5)));
