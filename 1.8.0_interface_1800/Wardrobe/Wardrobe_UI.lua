...
---------------------------------------------------------------------------------
function Wardrobe_HandleUIMenuTriggerClick()
...
function Wardrobe_ShowUIMenu()
    ...
    Wardrobe_UpdateOutfitAvailability_NonChronos();

    -- find out if we're wearing one of our outfits
    Wardrobe_ActiveOutfitList = Wardrobe_DetermineActiveOutfit_NonChronos();
...
function Wardrobe_CompleteShowingUIMenu()
    ...
    for i = 1, table.getn(Wardrobe_Config[WD_charID].Outfit) do
        if (Wardrobe_Config[WD_charID].Outfit[i].OutfitName ~= "" and (not Wardrobe_Config[WD_charID].Outfit[i].Virtual)) then
            ...
            button:SetText(Wardrobe_Config[WD_charID].Outfit[i].OutfitName);
            ...
            local buttonColor = Wardrobe_Config[WD_charID].Outfit[i].ButtonColor;
            ...
            elseif (Wardrobe_Config[WD_charID].Outfit[i].Available) then
            ...
            button.OutfitName = Wardrobe_Config[WD_charID].Outfit[i].OutfitName;
            button:Show();
        end
    end
...
function Wardrobe_Keybinding(outfitNum)
    if (outfitNum <= table.getn(Wardrobe_Config[WD_charID].Outfit)) then
        Wardrobe_WearOutfit(Wardrobe_Config[WD_charID].Outfit[outfitNum].OutfitName);
    end
end
...
function Wardrobe_PopupConfirm()
    ...
    elseif (Wardrobe_PopupFunction == "[Update]") then
        Wardrobe_NewWardrobeName = WardrobeNamePopupEditBox:GetText();
        Wardrobe_ShowWardrobeConfigurationScreen();
    elseif (Wardrobe_PopupFunction == "[Mounted]") then
        if (not Wardrobe_FoundOutfitName(WardrobeNamePopupEditBox:GetText()) or WardrobeNamePopupEditBox:GetText() == "") then
            WardrobePrint(WARDROBE_TXT_MOUNTEDNOTEXIST);
            UIErrorsFrame:AddMessage(WARDROBE_TXT_NOTEXISTERROR, 0.0, 1.0, 0.0, 1.0, UIERRORS_HOLD_TIME);
            Wardrobe_PopupFunction = "";
        else
            Wardrobe_SetMountedOutfit(WardrobeNamePopupEditBox:GetText());
            Wardrobe_PopupFunction = "";
        end
    end
...
function Wardrobe_ShowWardrobeConfigurationScreen()
    ...
    elseif (Wardrobe_PopupFunction == "[Update]") then
        ...
        for i = 1, table.getn(Wardrobe_Config[WD_charID].Outfit) do
            ...
            if (Wardrobe_Config[WD_charID].Outfit[i].OutfitName == Wardrobe_NewWardrobeName) then

                Wardrobe_CurrentOutfitButtonColor = Wardrobe_Config[WD_charID].Outfit[i].ButtonColor;
                
                -- for each item in the outfit, set the checkbox
                for j = 1, table.getn(Wardrobe_Config[WD_charID].Outfit[i].Item) do
                    if (not Wardrobe_Config[WD_charID].Outfit[i].Item[j].IsSlotUsed) then
                        Wardrobe_Config[WD_charID].Outfit[i].Item[j].IsSlotUsed = Wardrobe_Config.DefaultCheckboxState;
                    end
                    local IsSlotUsed = Wardrobe_Config[WD_charID].Outfit[i].Item[j].IsSlotUsed;
...
function Wardrobe_ShowMainMenu()
    ...
    for i = 1, table.getn(Wardrobe_Config[WD_charID].Outfit) do
        Wardrobe_Config[WD_charID].Outfit[i].Selected = false;
    end
        ...
        if (toggle) then
            ...
            for i = 1, WARDROBE_MAX_SCROLL_ENTRIES do
                ...
                if (frameName ~= "WardrobeMainMenuFrameEntry"..i) then
                    if (Wardrobe_Config[WD_charID].Outfit[fs.OutfitNum]) then
...
function Wardrobe_PopulateMainMenu(maintainSelected)
    ...
    for i = 1, 21 do
        getglobal("WardrobeMainMenuFrameEntry"..i.."Icon"):Hide();
    end
    ...
    while rowCount <= WARDROBE_MAX_SCROLL_ENTRIES do
        ...
        if (not Wardrobe_Config[WD_charID].Outfit[entryCount + offset]) then
            fs:SetText("");
            fs.r = 0;
            fs.g = 0;
            fs.b = 0;
            getglobal("WardrobeMainMenuFrameEntry"..rowCount.."Icon"):Hide();
            rowCount = rowCount + 1;
            
        -- if this isn't a virtual outfit
        elseif (not Wardrobe_Config[WD_charID].Outfit[entryCount + offset].Virtual) then
            WardrobeDebug("Wasn't Virtual.  Setting text to: "..Wardrobe_Config[WD_charID].Outfit[entryCount + offset].OutfitName);
            fs:SetText(Wardrobe_Config[WD_charID].Outfit[entryCount + offset].OutfitName);
            local buttonColor = Wardrobe_Config[WD_charID].Outfit[entryCount + offset].ButtonColor;
            ...
            if (not maintainSelected) then
                --Wardrobe_Config[WD_charID].Outfit[entryCount + offset].Selected = false;
            end;
            
            -- if this outfit is selected, highlight it
            if (Wardrobe_Config[WD_charID].Outfit[entryCount + offset].Selected) then
            ...
            if (Wardrobe_Config[WD_charID].Outfit[entryCount + offset].Special == "mounted") then
                -- set the texture
                getglobal("WardrobeMainMenuFrameEntry"..rowCount.."Icon"):SetNormalTexture("Interface\\AddOns\\Wardrobe\\Images\\Paw");
                getglobal("WardrobeMainMenuFrameEntry"..rowCount.."Icon"):Show();

            -- check to see if we should show the plaguelands icon
            elseif (Wardrobe_Config[WD_charID].Outfit[entryCount + offset].Special == "plague") then
                -- set the texture
                getglobal("WardrobeMainMenuFrameEntry"..rowCount.."Icon"):SetNormalTexture("Interface\\AddOns\\Wardrobe\\Images\\Sword");
                getglobal("WardrobeMainMenuFrameEntry"..rowCount.."Icon"):Show();

            -- check to see if we should show the while-eating icon
            elseif (Wardrobe_Config[WD_charID].Outfit[entryCount + offset].Special == "chowing") then
                -- set the texture
                getglobal("WardrobeMainMenuFrameEntry"..rowCount.."Icon"):SetNormalTexture("Interface\\AddOns\\Wardrobe\\Images\\Food");
                getglobal("WardrobeMainMenuFrameEntry"..rowCount.."Icon"):Show();
    ...
    FauxScrollFrame_Update(WardrobeSortScrollFrame, table.getn(Wardrobe_Config[WD_charID].Outfit), WARDROBE_MAX_SCROLL_ENTRIES, WARDROBE_MAX_SCROLL_ENTRIES);
...
function Wardrobe_WardrobeMainMenuFrameEntry_OnLeave()
    ...
    if (thenum) then
        if (Wardrobe_Config[WD_charID].Outfit[fs.OutfitNum]) then
            if (not Wardrobe_Config[WD_charID].Outfit[fs.OutfitNum].Selected) then
                fs:SetTextColor(fs.r, fs.g, fs.b);
            end
        end
    end    
...
function Wardrobe_WardrobeMainMenuFrameEntry_OnClick()
    ...
    if (fs) then
        if (fs.OutfitNum) then
            if (Wardrobe_Config[WD_charID].Outfit[fs.OutfitNum] ~= nil) then
                if (not Wardrobe_Config[WD_charID].Outfit[fs.OutfitNum].Selected) then
                    Wardrobe_Config[WD_charID].Outfit[fs.OutfitNum].Selected = true;
                    Wardrobe_SetSelectionColor(true, frameName.."Outfit", fs);
                else
                    Wardrobe_Config[WD_charID].Outfit[fs.OutfitNum].Selected = false;
                end
            end    
        end    
    end    
...
function Wardrobe_EditOutfitButtonClick()
    ...
    if (selectedOutfit) then
        Wardrobe_NewWardrobeName = Wardrobe_Config[WD_charID].Outfit[selectedOutfit].OutfitName;
...
---------------------------------------------------------------------------------
function Wardrobe_MountButtonClick()
    local outfitNum = Wardrobe_FindSelectedOutfit();
    local alreadyUsingThisOutfitForMounting = false;
    
    if (outfitNum) then
    
        if (Wardrobe_Config[WD_charID].Outfit[outfitNum].Special == "mounted") then
            UIErrorsFrame:AddMessage(Wardrobe_Config[WD_charID].Outfit[outfitNum].OutfitName.." "..WARDROBE_TXT_NOLONGERWORNMOUNTERR, 0.0, 1.0, 0.0, 1.0, UIERRORS_HOLD_TIME);
            alreadyUsingThisOutfitForMounting = true;
        end
        for i = 1, table.getn(Wardrobe_Config[WD_charID].Outfit) do
            if (Wardrobe_Config[WD_charID].Outfit[i].Special == "mounted") then Wardrobe_Config[WD_charID].Outfit[i].Special = ""; end
        end
        if (not alreadyUsingThisOutfitForMounting) then
            Wardrobe_Config[WD_charID].Outfit[outfitNum].Special = "mounted";
            UIErrorsFrame:AddMessage(Wardrobe_Config[WD_charID].Outfit[outfitNum].OutfitName.." "..WARDROBE_TXT_WORNWHENMOUNTERR, 0.0, 1.0, 0.0, 1.0, UIERRORS_HOLD_TIME);
        end
...
---------------------------------------------------------------------------------
-- when clicking the plaguelands button
---------------------------------------------------------------------------------
function Wardrobe_PlaguelandsButtonClick()
    local outfitNum = Wardrobe_FindSelectedOutfit();
    local alreadyUsingThisOutfitForPlaguelands = false;
    
    if (outfitNum) then
    
        if (Wardrobe_Config[WD_charID].Outfit[outfitNum].Special == "plague") then
            UIErrorsFrame:AddMessage(Wardrobe_Config[WD_charID].Outfit[outfitNum].OutfitName.." "..WARDROBE_TXT_NOLONGERWORNPLAGUEERR, 0.0, 1.0, 0.0, 1.0, UIERRORS_HOLD_TIME);
            alreadyUsingThisOutfitForPlaguelands = true;
        end
        for i = 1, table.getn(Wardrobe_Config[WD_charID].Outfit) do
            if (Wardrobe_Config[WD_charID].Outfit[i].Special == "plague") then Wardrobe_Config[WD_charID].Outfit[i].Special = ""; end
        end
        if (not alreadyUsingThisOutfitForPlaguelands) then
            Wardrobe_Config[WD_charID].Outfit[outfitNum].Special = "plague";
            UIErrorsFrame:AddMessage(Wardrobe_Config[WD_charID].Outfit[outfitNum].OutfitName.." "..WARDROBE_TXT_WORNPLAGUEERR, 0.0, 1.0, 0.0, 1.0, UIERRORS_HOLD_TIME);
        end
...
---------------------------------------------------------------------------------
-- when clicking the while eatings/drinking button
---------------------------------------------------------------------------------
function Wardrobe_ChowingButtonClick()
    local outfitNum = Wardrobe_FindSelectedOutfit();
    local alreadyUsingThisOutfitForChowing = false;
    
    if (outfitNum) then
    
        if (Wardrobe_Config[WD_charID].Outfit[outfitNum].Special == "chowing") then
            UIErrorsFrame:AddMessage(Wardrobe_Config[WD_charID].Outfit[outfitNum].OutfitName.." "..WARDROBE_TXT_NOLONGERWORNEATERR, 0.0, 1.0, 0.0, 1.0, UIERRORS_HOLD_TIME);
            alreadyUsingThisOutfitForChowing = true;
        end
        for i = 1, table.getn(Wardrobe_Config[WD_charID].Outfit) do
            if (Wardrobe_Config[WD_charID].Outfit[i].Special == "chowing") then Wardrobe_Config[WD_charID].Outfit[i].Special = ""; end
        end
        if (not alreadyUsingThisOutfitForChowing) then
            Wardrobe_Config[WD_charID].Outfit[outfitNum].Special = "chowing";
            UIErrorsFrame:AddMessage(Wardrobe_Config[WD_charID].Outfit[outfitNum].OutfitName.." "..WARDROBE_TXT_WORNEATERR, 0.0, 1.0, 0.0, 1.0, UIERRORS_HOLD_TIME);
        end
...
function Wardrobe_DeleteOutfit_OnClick()
    ...
    if (selectedOutfit) then
        ...
        WardrobeNamePopupEditBox:SetText(Wardrobe_Config[WD_charID].Outfit[selectedOutfit].OutfitName);
        WardrobePopupConfirm:Show();
        Wardrobe_ToggleMainMenuFrameVisibility(false);
        WardrobePopupConfirmText:SetText(WARDROBE_TXT_REALLYDELETEOUTFIT.."\n\n"..Wardrobe_Config[WD_charID].Outfit[selectedOutfit].OutfitName);
    else
...
function Wardrobe_ShowColorPickFrame()
    ...
    if (selectedOutfit or Wardrobe_EnteredColorPickerFromPaperdollFrame) then
        ...
        if (Wardrobe_EnteredColorPickerFromPaperdollFrame) then
            ...
            WardrobeColorPickFrameExampleText:SetText(Wardrobe_Config[WD_charID].Outfit[selectedOutfit].OutfitName);
            local colorNum = Wardrobe_Config[WD_charID].Outfit[selectedOutfit].ButtonColor;
...
function Wardrobe_SetSelectedOutfitColor(num)
    ...
    if (selectedOutfit) then
        Wardrobe_Config[WD_charID].Outfit[selectedOutfit].ButtonColor = num;        
        Wardrobe_PopulateMainMenu();
    end
end
...
function Wardrobe_ShowButtonTooltip(theText1, theText2, theText3, theText4, theText5)
    ...
    Wardrobe_MoneyToggle();
    if (Sea.wow.tooltip.clear) then
