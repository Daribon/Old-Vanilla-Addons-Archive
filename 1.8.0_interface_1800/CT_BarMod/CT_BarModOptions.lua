...
function CT_BarModOptions_Slider_OnValueChanged()
	...
	for i = 1, 12, 1 do
		...
		if ( prefix == "" ) then
			getglobal("BonusActionButton" .. i):SetScale(CT_BarModOptions_Options[UnitName("player")][this:GetID()]*UIParent:GetScale());
		end
	end
end
...
function CT_BarModOptions_RemoveSpaceBars(checked)
	...
	if ( CT_BottomBar_Enabled and not CT_BottomBar_Enabled[UnitName("player")] ) then
			CT_BottomBar_Enabled[UnitName("player")] = { };
	end
