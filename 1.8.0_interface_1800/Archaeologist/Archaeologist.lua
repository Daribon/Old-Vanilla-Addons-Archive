...
Known Bugs: Party Pet Buff/Debuff tooltips don't show.
	...
	$Date: 2005-10-15 05:42:08 -0500 (Sat, 15 Oct 2005) $
	...
	if (index) then
		...
		if (index == 1) then
			...
			for k, v in {"player", "pet", "party", "target"} do
				MCom.SlashCommandHandler(id, v.."hp on");
				MCom.SlashCommandHandler(id, v.."hp2 off");
				MCom.SlashCommandHandler(id, v.."mp on");
				MCom.SlashCommandHandler(id, v.."mp2 off");
				
				MCom.SlashCommandHandler(id, v.."hpinvert off");
				MCom.SlashCommandHandler(id, v.."mpinvert off");
				
				MCom.SlashCommandHandler(id, v.."hpswap on");
				MCom.SlashCommandHandler(id, v.."mpswap on");
			end
			
			MCom.SlashCommandHandler(id, "targethpswap off");
			MCom.SlashCommandHandler(id, "usehpvalue on");
			if (MobHealth_OnEvent) then
				MCom.SlashCommandHandler(id, "mobhealth on");
			end
		
		elseif (index == 2) then
			-- Values next to the Bars
			for k, v in {"player", "pet", "party", "target"} do
				MCom.SlashCommandHandler(id, v.."hp off");
				MCom.SlashCommandHandler(id, v.."hp2 on");
				MCom.SlashCommandHandler(id, v.."mp off");
				MCom.SlashCommandHandler(id, v.."mp2 on");
				
				MCom.SlashCommandHandler(id, v.."hpinvert off");
				MCom.SlashCommandHandler(id, v.."mpinvert off");
				
				MCom.SlashCommandHandler(id, v.."hpswap off");
				MCom.SlashCommandHandler(id, v.."mpswap off");
			end
			
			MCom.SlashCommandHandler(id, "targethpswap on");
			MCom.SlashCommandHandler(id, "usehpvalue on");
			if (MobHealth_OnEvent) then
				MCom.SlashCommandHandler(id, "mobhealth on");
			end
		...
		elseif (index == 3) then
			...
			MCom.SlashCommandHandler(id, "usehpvalue off");
...
function Archaeologist_SetPrefixPresets(index)
	if (index) then
		...
		if (index == 1) then
			-- All off
			for k, v in {"player", "pet", "party", "target"} do
				MCom.SlashCommandHandler(id, v.."hpnoprefix on");
				MCom.SlashCommandHandler(id, v.."mpnoprefix on");
			end
			
			MCom.SlashCommandHandler(id, "playerxpnoprefix on");
			MCom.SlashCommandHandler(id, "petxpnoprefix on");
		
		elseif (index == 2) then
			-- All on
			for k, v in {"player", "pet", "party", "target"} do
				MCom.SlashCommandHandler(id, v.."hpnoprefix off");
				MCom.SlashCommandHandler(id, v.."mpnoprefix off");
			end
			
			MCom.SlashCommandHandler(id, "playerxpnoprefix off");
			MCom.SlashCommandHandler(id, "petxpnoprefix off");
