...
Change Log:
	...
	$Id: MobileFrames.lua 2386 2005-09-06 23:28:25Z karlkfi $
	$Rev: 2386 $
	$LastChangedBy: karlkfi $
	$Date: 2005-09-06 18:28:25 -0500 (Tue, 06 Sep 2005) $
...
function MobileFrames_DefineSpecialFrames()
	MobileFrames_SpecialFrames = {
		...
		FramerateLabel = { cosmos = "COS_MOBILE_FRAMES_FRAMERATE", reset = function() MobileFrames_FramerateLabelReset() end, default = MobileFrames_EnabledByDefault, func = function(checked) MobileFrames_ToggleEnableGeneral("FramerateLabel", checked) end },
		...
		TicketStatusFrame = { cosmos = "COS_MOBILE_FRAMES_TICKETSTATUS", reset = MobileFrames_TicketStatus_Reset, default = MobileFrames_EnabledByDefault, func = function(checked) MobileFrames_AnchorToggleEnableGeneral("TicketStatusFrame", checked) end },
		...
		WorldStateScoreFrame = { cosmos = "COS_MOBILE_FRAMES_WORLDSTATESCORE", reset = function() MobileFrames_WorldStateScoreFrameReset() end, default = MobileFrames_EnabledByDefault, func = function(checked) MobileFrames_ToggleEnableGeneral("WorldStateScoreFrame", checked) end },
...
function MobileFrames_OnEvent(event)
	if event == "VARIABLES_LOADED" then
		...
		if (Eclipse_UIParent_ManageRightSideFrames) then
			Sea.util.unhook("UIParent_ManageRightSideFrames", "Eclipse_UIParent_ManageRightSideFrames", "after");
		end
...
function MobileFrames_UIParent_ManageRightSideFrames(setQWFPoint, setDFPoint, setGLFPoint)
	...
	elseif (Eclipse_UIParent_ManageRightSideFrames) then
		Eclipse_UIParent_ManageRightSideFrames();
	else
...
function MobileFrames_MobileMinimapButton_OnMouseUp(buttonFrame, clickFunction)
	...
	elseif (MouseIsOver(buttonFrame:GetParent())) then
		if (IsShiftKeyDown()) and ( arg1 == "RightButton" ) then
			MobileFrames_MobileMinimapButton_Reset(buttonFrame:GetParent());
		elseif (buttonFrame:IsEnabled() == 1) then
			clickFunction();
		end
	end
end
