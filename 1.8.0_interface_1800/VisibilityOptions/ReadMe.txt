...
--Change Log--
v1.58:
-Hopefully fixed a bug with load on demand addons that don't load frames until later, and in turn didn't get proper transparency from parent frames.
v1.57:
-Was hiding experience bar text with mainbar, instead of experience bar, fixed now
v1.56:
-Will now not allow pet bar, and shapeshift bar to be moved by the game if the user has manually moved them.
v1.55:
-Updated to MCom 1.49
-Fixed XP tick not hiding
v1.54:
-Now uses IsShown instead of IsVisible when available for better visibility state detection
-Revamped MainBar registration to use more appropriate methodology
-Experience bar is now a seperate setting from MainBar
v1.53:
-Fixed a potential error message that could occur when using VisibilityOptions without using BarOptions
v1.52:
-Updated to MCom 1.48
-Updated for 1.8 patch
-No longer needs BarOptions to hide the parts of the mainbar
v1.51:
...
-Improved options for buffs
-Added Target frame options
-Improved handling of position code for stance/shapeshift/stealth/aura bar
v1.45:
