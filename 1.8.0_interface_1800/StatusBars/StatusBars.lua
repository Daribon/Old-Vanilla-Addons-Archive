...
local bars = {
	...
	petHealth, petPower, petBuff, petDebuff
};
...
local petLogicals = { 'phealth', 'pmana', 'prage', 'penergy', 'pfocus', 'pbuff', 'pdebuff' };
...
local function sortBars(gbars, container)
	...
	table.foreachi(gbars,
		function(i, vars)
			if not vars.never and (vars.visible or vars.softEnable) then
...
local function fadeFrame(vars, visible)
	...
	if current ~= visible then
		...
		if visible then
			notifyingFade(vars, frame, 'IN', 0.2, frame:GetAlpha(), alpha);
		else
			notifyingFade(vars, frame, 'OUT', 1.5, frame:GetAlpha(), 0);
		end
...
local function updateBuffs(vars, display)
	local num = 0;
	for i = 1, MaxBuffs do
		local buff = vars.buffFunc(vars.unit, i);
		if buff then
			num = num + 1;
		end
		if display then
...
local function slashHandler(command)
	...
	if value == nil then
		...
		consolePrint('  phealth, pmana, prage, penergy, pfocus, pbuff, pdebuff');
