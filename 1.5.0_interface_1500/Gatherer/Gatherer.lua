-- Gatherer
-- Written by Chandora
GATHERER_VERSION="2.0.3";
-- Revision: $Id: Gatherer.lua,v 1.15 2005/06/11 02:59:46 islorgris Exp $
--
				...
				if ((allowed == true) and
					...
					for hPos, gatherInfo in gatherData do
						if ((gatherInfo.x) and (gatherInfo.y) and (gatherInfo.x>0) and (gatherInfo.y>0) and (lastUnused <= maxNotes)) then
							...
							--mainNote:SetFrameLevel(Gatherer_WorldMapPlayerFrameLevel - 1);
							-- GathererMapOverlayFrame:SetFrameLevel(Gatherer_WorldMapPlayerFrameLevel - 1);
							if ( Gatherer_TitleCase(specificType) == ORE_THORIUM or 
								( GatherConfig and GatherConfig.ToggleWorldNotes and GatherConfig.ToggleWorldNotes == 1)
							 ) then
