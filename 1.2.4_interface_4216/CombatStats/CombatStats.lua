...
function CombatStats_ChatCommandHandler(msg)
	...
	Print("endoffight [on | off] - 'Show end of figth information'");
...
function CombatStatsSaveSettings()
	RegisterForSave("CombatStats_Config");
end
...
function CombatStats_UpdateDetails(attackName)
	...
	if(attackName == "Defensive") then
		...
		if(specialAttacks[attackName].totalBlocked ~= 0) then
			sCombatStatsGeneralBlockedPercentLabel:SetText( format(NORMAL_FONT_COLOR_CODE.."[ "..WHITE_FONT_COLOR_CODE.. "%.1f%%" .. NORMAL_FONT_COLOR_CODE.." ]", (specialAttacks[attackName].totalBlocked / (specialAttacks[attackName].totalSwings) * 100)));
		else
			CombatStatsGeneralBlockedPercentLabel:SetText(NORMAL_FONT_COLOR_CODE.."[ "..WHITE_FONT_COLOR_CODE.."0.0%"..NORMAL_FONT_COLOR_CODE.." ]");
		end
