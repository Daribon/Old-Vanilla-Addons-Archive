AquiredMouseOverTargetTime = 0;
AquiredMouseOverTarget = nil;
...
function HealingEstimator_Init()
	...
	-- To keep cache of spell ids up to date
...
function HealingEstimator_OnEvent(event)
	...
	if ( event == "SPELLS_CHANGED" ) then
		HealingEstimator_SpellsChanged();
		return;
	end
	if ( event == "UPDATE_MOUSEOVER_UNIT" ) then
		local u = "mouseover";
		if (UnitExists(u)) and (UnitIsFriend("player",u)) then
			AquiredMouseOverTargetTime = GetTime();
			AquiredMouseOverTarget=HealingEstimator_GetGroupName(u);	-- Store it, it might change
			if (AquiredMouseOverTarget==nil) then
				-- No groupname found, use "mouseover"
				AquiredMouseOverTarget="mouseover"
			end
		end
		return;
	end
...
HealingEstimator_GroupNames = {
	raid = {};
	raidpet = {};
	party = {};
	partypet = {};
};
for i=1,40 do table.insert(HealingEstimator_GroupNames.raid, "raid"..i) table.insert(HealingEstimator_GroupNames.raidpet, "raidpet"..i) end
for i=1,4 do table.insert(HealingEstimator_GroupNames.party, "party"..i) table.insert(HealingEstimator_GroupNames.partypet, "partypet"..i) end
function HealingEstimator_GetGroupName(Unit)
	if ( not Unit ) then return Unit; end
	if (strfind(Unit,"raid")) or (strfind(Unit,"party")) then
		return Unit;
	end
	...
	elseif (UnitIsUnit(Unit,"pet")) then
		...
		if (Count~=0) then
			local c;
			local u;
			local petName = Group.."pet";
			for c=1,Count do
				u = HealingEstimator_GroupNames[Group][c];
				if (UnitIsUnit(Unit,u)) then
					-- Unit was found, return
					HealingEstimator_Print(DebugMsg,"Group target: "..u);
					return u;
				end
				u = HealingEstimator_GroupNames[petName][c];
				if (UnitIsUnit(Unit,u)) then
					-- Unit was found, return
					HealingEstimator_Print(DebugMsg,"Group target: "..u);
					return u;
...
function HealingEstimator_SetTargetData(Unit,HealAverage)
	...
	if (not Unit) or ( not UnitExists(Unit) ) or ( not UnitIsFriend("player", Unit) ) then
		return;
	end
...
function HealingEstimator_StartSpell(arg1)
	...
	if ((not SpellTarget) or (not UnitExists(SpellTarget)) or (not UnitName(SpellTarget))) then
