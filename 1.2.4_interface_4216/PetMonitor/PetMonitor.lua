--
--
-- PetMonitor 1.1 for Cosmos
-- Written by Arys 01-19-05
--
--	Displays Party Pet Health
--
...
PetMonitor_Vars.CombatInterval = .75;
...
PetMonitor_Vars.LastHPShout = 0;
PetMonitor_Vars.Cooldown = 5;
...
function PetMonitor_ReceiveChat(message, user, channel)
...
function PetMonitor_Initialize()	
	SLASH_PETMONITOR1 = "/petmon";
	SlashCmdList["PETMONITOR"] = function(msg)
		PetMonitor_OnSlashCommand(msg);
	end
	...
	if (PetMonitor_Vars.Broadcast or PetMonitor_Vars.Receive) then
		...
		if (PetMonitor_Vars.Receive) then
			...
			Sky.registerAlert({id="PM", func=PetMonitor_ReceiveChat});
		end
	else
		return;
	end
...
function PetMonitor_OnSlashCommand(msg)
	if (not msg) then
		return;
	end
	...
	if (command == "debug") then
		if ( PETMONITOR_DEBUG ) then
			PETMONITOR_DEBUG = false;			
			Sea.io.print("PetMonitor debug OFF");
		else
			PETMONITOR_DEBUG = true;
			Sea.io.print("PetMonitor debug ON");
		end
		PM_DEBUG = "PETMONITOR_DEBUG";
	end
...
function PetMonitor_BroadcastPetHealth(forced)
	...
	if ( UnitInParty("party1") ) then
		...
		Sky.sendAlert(sendMsg, SKY_PARTY, "PM");
...
function PetMonitor_OnScaleUpdate(toggle, value)
	...
	PetMonitor_SavedVars.PetFrameScale = value;
...
function PetMonitor_OnCombatIntervalUpdate(toggle, value)
	PetMonitor_Vars.CombatInterval = value;
end

function PetMonitor_OnSlowIntervalUpdate(toggle, value)
	PetMonitor_Vars.SlowInterval = value;
end

function PetMonitor_OnRegularIntervalUpdate(toggle, value)
	PetMonitor_Vars.RegularInterval = value;
end
...
function PetMonitor_RegisterCosmosOptions()
	if( Cosmos_RegisterConfiguration ) then
		...
		Cosmos_RegisterConfiguration("COS_PETMONITOR_THRESHOLD",
			"BOTH",					
			PETMONITOR_HEALTH,			 
