	...
	if ( CT_TimerFrame.update >= 1 ) then
		...
		for k, v in CT_Timer_CallFunctions do
			if ( type(v) == "function" ) then
				v(CT_TimerFrame.time, CT_TimerFrame.step);
			end
		end

		if ( CT_TimerFrame.time == 0 ) then
			DEFAULT_CHAT_FRAME:AddMessage(format(CT_TIMER_FINISHCOUNT, CT_TimerFrame.countfrom), 1, 0.5, 0);
...
function CT_Timer_Start(seconds)
	CT_TimerFrame.status = 1;
	if ( seconds ) then
		CT_TimerFrame.step = -1;
		CT_TimerFrame.countfrom = CT_Timer_GetTimeString(seconds);
	else
		CT_TimerFrame.step = 1;
	end
	if ( seconds ) then
		CT_TimerFrame.time = seconds;
	else
...
function CT_Timer_Pause(newStatus)
	if ( not newStatus and ( not CT_TimerFrame.status or CT_TimerFrame.status == 0 ) ) then
		newStatus = 1;
	...
	if ( newStatus ) then
		if ( not CT_TimerFrame.status ) then
			if ( CT_TimerFrame.time > 0 ) then
				CT_TimerFrame.countfrom = CT_Timer_GetTimeString(CT_TimerFrame.time);
				CT_TimerFrame.step = -1;
			else
				CT_TimerFrame.step = 1;
			end
...
function CT_Timer_SetTimerTime(num)
	...
	CT_TimerFrame.time = num;
	CT_Timer_SetTime(CT_TimerFrametime, CT_TimerFrame);
end
