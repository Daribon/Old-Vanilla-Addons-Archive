...
function TellTrack_Register_Sky()
	Sky.registerSlashCommand(
		{
			id="TellTrackEnable";
			commands = TellTrackEnableCommands;
			onExecute = TellTrack_Enable_ChatCommandHandler;
			helpText = TELLTRACK_CHAT_COMMAND_ENABLE_INFO;
		}
	);
	Sky.registerSlashCommand(
		{
			id="TellTrackInvert";
			commands = TellTrackInvertCommands;
			onExecute = TellTrack_Invert_ChatCommandHandler;
			helpText = TELLTRACK_CHAT_COMMAND_INVERT_INFO;
		}
	);
	Sky.registerSlashCommand(
		{
			id="TellTrackDontSaveList";
			commands = TellTrackDontSaveListCommands;
			onExecute = TellTrack_DontSaveList_ChatCommandHandler;
			helpText = TELLTRACK_CHAT_COMMAND_DONTSAVELIST_INFO;
		}
	);
	Sky.registerSlashCommand(
		{
			id="TellTrackAutoWhisperFrame";
			commands = TellTrackAutoWhisperFrameCommands;
			onExecute = TellTrack_AutoWhisperFrame_ChatCommandHandler;
			helpText = TELLTRACK_CHAT_COMMAND_AUTOWHISPERFRAME_INFO;
		}
	);
	Sky.registerSlashCommand(
		{
			id="TellTrackHideOtherWhispers";
			commands = TellTrackHideOtherWhispersCommands;
			onExecute = TellTrack_HideOtherWhispers_ChatCommandHandler;
			helpText = TELLTRACK_CHAT_COMMAND_HIDEOTHERWHISPERS_INFO;
		}
	);
	Sky.registerSlashCommand(
		{
			id="TellTrackTimeStamps";
			commands = TellTrackTimeStampsCommands;
			onExecute = TellTrack_TimeStamps_ChatCommandHandler;
			helpText = TELLTRACK_CHAT_COMMAND_TIMESTAMPS_INFO;
		}
	);
	Sky.registerSlashCommand(
		{
			id="TellTrackWhisperFirst";
			commands = TellTrackWhisperFirstCommands;
			onExecute = TellTrack_WhisperFirst_ChatCommandHandler;
			helpText = TELLTRACK_CHAT_COMMAND_WHISPERFIRST_INFO;
		}
	);
	TellTrack_Sky_Registered = 1;
end
...
function TellTrack_Register()
	...
	if (Sky) then
		if (not TellTrack_Sky_Registered) then
			TellTrack_Register_Sky();
		end
	else
		...
		SlashCmdList["TELLTRACKDONTSAVELIST"] = TellTrack_DontSaveList_ChatCommandHandler;
		SLASH_TELLTRACKDONTSAVELIST1 = "/telltrackdontsavelist";
		SLASH_TELLTRACKDONTSAVELIST2 = "/ttrackdontsavelist";
		RegisterForSave("TellTrack_DontSaveList");
		SlashCmdList["TELLTRACKAUTOWHISPERFRAME"] = TellTrack_AutoWhisperFrame_ChatCommandHandler;
		SLASH_TELLTRACKAUTOWHISPERFRAME1 = "/telltrackautowhisperframe";
		SLASH_TELLTRACKAUTOWHISPERFRAME2 = "/ttrackautowhisperframe";
		RegisterForSave("TellTrack_AutoWhisperFrame");
		SlashCmdList["TELLTRACKHIDEOTHERWHISPERS"] = TellTrack_HideOtherWhispers_ChatCommandHandler;
		SLASH_TELLTRACKHIDEOTHERWHISPERS1 = "/telltrackhidewhispers";
		SLASH_TELLTRACKHIDEOTHERWHISPERS2 = "/ttrackhidewhispers";
		RegisterForSave("TellTrack_HideOtherWhispers");
		SlashCmdList["TELLTRACKTIMESTAMPS"] = TellTrack_TimeStamps_ChatCommandHandler;
		SLASH_TELLTRACKTIMESTAMPS1 = "/telltracktimestamps";
		SLASH_TELLTRACKTIMESTAMPS2 = "/ttracktimestamps";
		RegisterForSave("TellTrack_TimeStamps");
		SlashCmdList["TELLTRACKWHISPERFIRST"] = TellTrack_WhisperFirst_ChatCommandHandler;
		SLASH_TELLTRACKWHISPERFIRST1 = "/telltrackwhisperfirst";
		SLASH_TELLTRACKWHISPERFIRST2 = "/ttrackwhisperfirst";
		RegisterForSave("TellTrack_TimeStamps");
...
function TellTrack_ChatCommandHandler(msg, func)
	if msg == nil then
		msg = "";
	end
...
function TellTrack_Enable_ChatCommandHandler(msg)
	TellTrack_ChatCommandHandler(msg, TellTrack_Toggle_Enabled);
end

-- Handles chat - e.g. slashcommands - inverting/normalizing the TellTrack list
function TellTrack_Invert_ChatCommandHandler(msg)
	TellTrack_ChatCommandHandler(msg, TellTrack_Toggle_Inverted);
end

-- Handles chat - e.g. slashcommands - saving/not saving the TellTrack list between sessions
function TellTrack_DontSaveList_ChatCommandHandler(msg)
	TellTrack_ChatCommandHandler(msg, TellTrack_Toggle_DontSaveList);
end

-- Handles chat - e.g. slashcommands - saving/not saving the TellTrack list between sessions
function TellTrack_AutoWhisperFrame_ChatCommandHandler(msg)
	TellTrack_ChatCommandHandler(msg, TellTrack_Toggle_AutoWhisperFrame);
end

-- Handles chat - e.g. slashcommands - Hiding whispers in frames other than the whisper frame
function TellTrack_HideOtherWhispers_ChatCommandHandler(msg)
	TellTrack_ChatCommandHandler(msg, TellTrack_Toggle_HideOtherWhispers);
end

-- Handles chat - e.g. slashcommands - Print time stamps on messages in the whisper frame
function TellTrack_TimeStamps_ChatCommandHandler(msg)
	TellTrack_ChatCommandHandler(msg, TellTrack_Toggle_TimeStamps);
end

-- Handles chat - e.g. slashcommands - First TellTrack name click will open a whisper, second will reveal their chat log.
function TellTrack_WhisperFirst_ChatCommandHandler(msg)
	TellTrack_ChatCommandHandler(msg, TellTrack_Toggle_WhisperFirst);
end
...
function TellTrack_OnEvent(event)
	...
	elseif ( event == "CHAT_MSG_WHISPER" ) or ( event == "CHAT_MSG_WHISPER_INFORM" ) then
		...
		--Hide Sky messages
		if (string.len(arg1) > 4) and (string.sub(arg1,1,4) == "<Sky") then
			return;
		end
		...
		table.insert(TellTrack_WhisperMessages, {type=type,name=arg2,msg=arg1,time=time, date=date()});
	end
end

function TellTrack_AddWhipser(chatFrame, text, name, type, time)
...
function TellTrack_ShowConversationWith(name, showAll)
	...
	for i, data in TellTrack_WhisperMessages do
		if (showAll) or (data.name == name) then
			if (ChatTimeStamps_LOCALTIME) then
				TellTrack_AddWhipser(WhisperFrame, data.msg, data.name, data.type, data.date);
			else
				TellTrack_AddWhipser(WhisperFrame, data.msg, data.name, data.type, data.time);
			end
			found = found + 1;
		end
	end
...
function TellTrackTextButton_ShowMenu()
	if (EarthMenu_MenuOpen) then
		...
		--EarthMenu_MenuOpen(info, this:GetName(), 25, 20, "MENU");
