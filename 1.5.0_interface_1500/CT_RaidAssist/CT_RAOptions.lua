CT_RA_VersionNumber = 1.153; -- Used for number comparisons

CT_RA_Version = "b" .. CT_RA_VersionNumber;
...
function CT_RAOptionsGroupButton_OnMouseDown(button)
	if ( button == "LeftButton" and CT_RAMenu_Options[CT_RAMenu_CurrSet]["SORTTYPE"] == "custom" ) then
		this:StartMoving();
...
function CT_RAOptions_Update()
	...
	for i=1, MAX_RAID_MEMBERS do
		...
		if ( i <= numRaidMembers ) then
			...
			if ( not CT_RA_CurrPositions[i] or CT_RAMenu_Options[CT_RAMenu_CurrSet]["SORTTYPE"] ~= "custom" ) then

				CT_RA_CurrPositions[i] = subgroup;

				if ( CT_RAMenu_Options[CT_RAMenu_CurrSet]["SORTTYPE"] == "custom" and getglobal("CT_RAOptionsGroup" .. subgroup).nextIndex > 5 ) then
					for y = 1, 8, 1 do
