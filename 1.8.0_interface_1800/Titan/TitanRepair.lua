...
function TitanRepair_GetStatus(index)
	...
	local hasItem, _, repairCost = TDTooltip:SetInventoryItem("player", id);
	TDTooltip:Hide();
	...
	if ( hasItem ) then
		...
		for i = 1, 30 do
			local text = getglobal("TDTooltipTextLeft" .. i):GetText();
			if ( text ) then
	...
	for i = 1, table.getn(REPAIR_ITEM_STATUS) do
		...
		if ( str ) then
			if ( cost > 0 ) then
				...
				if ( TitanGetVar(TITAN_REPAIR_ID,"ShowUndamaged") ) then
					out = out .. str .. "\n";
				end
			end
		end
