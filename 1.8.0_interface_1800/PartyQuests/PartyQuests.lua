...
--	$Author: AlexYoshi $
--	$Rev: 2251 $
--	$Date: 2005-08-08 03:29:31 -0500 (Mon, 08 Aug 2005) $
--]]
...
PARTYQUESTS_REQUEST_SHARE = "REQ_SHARE";
...
PartyQuests_QuestSettings.common = {sortMethod="level";showDetailedTooltip=true;autoJoinSkyParty=true};
...
function PartyQuests_GetPlayerQuestTree()
	PartyQuests_MyCurrentLog = Sea.table.copy(Libram.requestHistory());
			...
			for i=1,table.getn(questList) do 
				...
				if ( type ( quest ) == "table" ) then 
					...
					if ( funcList ) then 
						...
						local onCheck = funcList.onCheck;
						if ( onCheck ) then 
							entry.check = true;
							if ( IsQuestWatched(quest.flatid) ) then
								entry.checked = true;
							end
							entry.onCheck = onCheck;
						end
...
function PartyQuests_ConvertPlayerQuestTreeToEnhancedTree(tree)
	...
	if ( tree ) then 
		local funcList = {
			...
			onCheck=PartyQuests_OnPlayerQuestCheck;
...
function PartyQuests_CreatePlayerTooltip(quest)
	...
	if ( type( quest ) == "table" and PartyQuests_QuestSettings.player.showDetailedTooltip ) then 
		if ( quest.id ) then 
			...
			if ( questInfo and questInfo.objectives and table.getn(questInfo.objectives) > 0 ) then
...
function PartyQuests_OnPlayerQuestCheck(checked, value)
	local id = nil;
	if ( type (value) == "table" ) then 
		id = value.id;
	else 
...
function PartyQuests_CreateMenu(mtype)
	...
	info[5] = { text = TEXT(PARTYQUESTS_MENU_AUTOJOIN), keepShownOnClick = 1 };
	info[5].func = function (checked) PartyQuests_QuestSettings.common.autoJoinSkyParty = checked; end
	info[5].checked = PartyQuests_QuestSettings.common.autoJoinSkyParty;
	
	info[6] = { text = TEXT(PARTYQUESTS_MENU_SHOWZONES), keepShownOnClick = 1 };
	info[6].func = function (checked) PartyQuests_LastUpdateTime=GetTime(); PartyQuests_SetZoneDisplay(checked,mtype); PartyQuests_LoadGui(); end
	
	if ( PartyQuests_QuestSettings[mtype].zonesOn ) then 
		info[6].checked = true;
	else
		info[6].checked = false;
	end
	
	info[7] = { text = TEXT(PARTYQUESTS_MENU_SHOWLEVEL), keepShownOnClick = 1 };
	info[7].func = function (checked) PartyQuests_LastUpdateTime=GetTime(); PartyQuests_SetLevelDisplay(checked,mtype); PartyQuests_LoadGui(); end
	
	if ( PartyQuests_QuestSettings[mtype].showLevel ~= nil) then 
		info[7].checked = PartyQuests_QuestSettings[mtype].showLevel;
	else
		info[7].checked = true;
	end
	

	info[8] = { text = TEXT(PARTYQUESTS_MENU_SHOWINDENT), keepShownOnClick = 1 };
	info[8].func = function (checked) PartyQuests_LastUpdateTime=GetTime(); PartyQuests_SetIndentDisplay(checked,mtype); PartyQuests_LoadGui(); end
	
	if ( PartyQuests_QuestSettings[mtype].indentOff ~= nil) then 
		info[8].checked = PartyQuests_QuestSettings[mtype].indentOff;
	else
		info[8].checked = false;
	end

	info[9] = { text = TEXT(PARTYQUESTS_MENU_SHOWCOMPLETEDBANNER), keepShownOnClick = 1 };
	info[9].func = function (checked) PartyQuests_LastUpdateTime=GetTime(); PartyQuests_SetCompletedBannerDisplay(checked,mtype); PartyQuests_LoadGui(); end
	
	if ( PartyQuests_QuestSettings[mtype].showCompletedBanner ~= nil) then 
		info[9].checked = PartyQuests_QuestSettings[mtype].showCompletedBanner;
	else
		info[9].checked = true;
	end
	

	info[10] = { text = TEXT(PARTYQUESTS_MENU_SHOWDETAILEDTIP), keepShownOnClick = 1 };
	info[10].func = function (checked) PartyQuests_LastUpdateTime=GetTime(); PartyQuests_SetDetailedTooltipDisplay(checked,mtype); PartyQuests_LoadGui(); end
	
	if ( PartyQuests_QuestSettings[mtype].showDetailedTooltip ~= nil) then 
		info[10].checked = PartyQuests_QuestSettings[mtype].showDetailedTooltip;
	else
		info[10].checked = true;
	end
	

	-- Submenu 
	info[11] = { text = TEXT(PARTYQUESTS_SORTING), hasArrow = 1, value = 1};
	info[11][1] = { text = TEXT(PARTYQUESTS_SORTING), isTitle = 1 };
	info[11][2] = { text = TEXT(PARTYQUESTS_SORT_LEVEL), keepShownOnClick = false, func = function() PartyQuests_ChooseSortMethod("level", mtype); PartyQuests_LastUpdateTime=GetTime();PartyQuests_LoadGui(); end };
	info[11][3] = { text = TEXT(PARTYQUESTS_SORT_TITLE), keepShownOnClick = false, func = function() PartyQuests_ChooseSortMethod("title", mtype); PartyQuests_LastUpdateTime=GetTime();PartyQuests_LoadGui(); end };
	info[11][4] = { text = TEXT(PARTYQUESTS_SORT_ZONE), keepShownOnClick = false, func = function() PartyQuests_ChooseSortMethod("zone", mtype); PartyQuests_LastUpdateTime=GetTime();PartyQuests_LoadGui(); end };
		...
		EarthFeature_AddButton ( 
			{
				...
				tooltip=PARTYQUESTS_BUTTON_TOOTIP;
				icon="Interface\\Icons\\INV_Misc_Book_04";
				callback=TogglePartyQuests;
			}
		);
	end
end
...
function PartyQuests_ReadMail()
	...
	--[[ changed to always send over SkyParty
	if ( count < 3 ) then 
		for k,v in PartyQuests_WhoNeedsMyLog do
...
function PartyQuests_HandleRequest(data, username, time)
	...
	elseif ( data.action == PARTYQUESTS_REQUEST_SHARE ) then 
		if ( data.questTitle and data.questLevel ) then 
			PartyQuests_ShareQuest(data.questTitle, data.questLevel);
		end
	end			
end
...
function PartyQuests_RequestNeededLogs()
	...
	if ( not Sky.isChannelActive(SKY_PARTY) ) then 
		if (UnitInParty("party1")) then
			if (PartyQuests_QuestSettings.common.autoJoinSkyParty) and (SkyChannelManager.getChannelCount) and (SkyChannelManager.getChannelCount() < 10) then
				--Sky Light
				JoinChannelByName(SKY_PARTY, nil, nil, true);
				Chronos.schedule(1, PartyQuests_RequestNeededLogs);
			elseif (PartyQuests_QuestSettings.common.autoJoinSkyParty) and (SkyChannelManager.getLiveChannelCount) and (SkyChannelManager.getLiveChannelCount() < SKY_MAX_CHANNELS) then
				--Sky
				JoinChannelByName(SKY_PARTY, nil, nil, true);
				Chronos.schedule(1, PartyQuests_RequestNeededLogs);
			elseif ( not PartyQuests_JoinPartyAlert ) then 
	...
	-- Debug only
	Sea.io.dprint(PQ_DEBUG, "Request Quest Logs from "..numParty.." party members...");
...
--[[ Request Quest Status From a Player ]]--
function PartyQuests_RequestQuestShareFromPlayer(player, questTitle, questLevel)
	local data = {};
	data.partyQuests = true;
...
function PartyQuests_RecordQuestDetails (username, questTitle, questLevel, details)
	...
	PartyQuests_CurrentQuest_SetFromCache(username, questTitle, questLevel, nil);
	PartyQuests_ShowCurrentQuest();
end
...
function PartyQuests_SendQuestStatusToPlayer(username, questTitle, questLevel )
	Sea.io.dprint(PQ_DEBUG, "Sending Quest Status To Player ", username);
...
function PartyQuests_SendQuestStatusToParty(questTitle, questLevel )
	Sea.io.dprint(PQ_DEBUG, "Sending Quest Status To Party");
	local id = PartyQuests_MatchQuestToPlayerQuestID(questTitle, questLevel );
	local details = PartyQuests_GetPlayerQuestInfo(id);
...
function PartyQuests_SendQuestDetailsToPlayer(username, questTitle, questLevel )
	Sea.io.dprint(PQ_DEBUG, "Sending Quest Details To Player ", username );
	local id = PartyQuests_MatchQuestToPlayerQuestID(questTitle, questLevel );
	local details = PartyQuests_GetPlayerQuestInfo(id);
...
function PartyQuests_ShareQuest(questTitle, questLevel)
	local id = PartyQuests_MatchQuestToPlayerQuestID(questTitle, questLevel );
	if (id) then
		PartyQuests_ShareQuestByID(id);
	end
end

function PartyQuests_ShareQuestByID(id)
...
function PartyQuests_ShowCurrentQuest()
	if ( PartyQuests_CurrentQuest and PartyQuests_CurrentQuest.visible ) then 
		if ( PartyQuests_CurrentQuest.quest) then 
			local id = PartyQuests_MatchQuestToPlayerQuestID(PartyQuests_CurrentQuest.quest.title, PartyQuests_CurrentQuest.quest.level);
			if ( not PartyQuests_CurrentQuest.quest.id ) or ( not id ) then 
				PartyQuestsLogFrameAbandonButton:Disable();
			else
				PartyQuestsLogFrameAbandonButton:Enable();
			end
			if ( PartyQuests_CurrentQuest.quest.pushable and GetNumPartyMembers() > 0) then
...
-- Help Icon
--------------------------------------------------
function PartyQuests_HelpIcon_OnLoad()
	this.onEnter = PartyQuests_HelpIcon_OnEnter;
	this.onLeave = PartyQuests_HelpIcon_OnLeave;
end
function PartyQuests_HelpIcon_OnEnter()
	EarthTooltip:SetOwner(this, "ANCHOR_RIGHT");
	EarthTooltip:SetText(PARTYQUESTS_HELP_TIP);
end	
function PartyQuests_HelpIcon_OnLeave()
	EarthTooltip:Hide();
end
...
function PartyQuests_Frame_Initialize()
	...
	PartyQuests_LoadGui();	
	PartyQuests_RegisterMeteorologist();
end

-- Register with Khaos
function PartyQuests_RegisterWithKhaos()
	if ( Khaos ) then
		Khaos.registerOptionSet(
			"quest",
			{
				...
				options={
					{
						id="ReplaceQuestLog";
						text=PQ_REPLACE_TEXT;
						difficulty=1;
						helptext=PQ_REPLACE_HELPTEXT;
						callback=PartyQuests_ReplaceDefaultQuestWindow;
						feedback=PartyQuests_ReplacedDefaultFeedback;
						check=true;
						type=K_TEXT;
						default={
							checked=true;
...
function PartyQuests_RegisterMeteorologist()
...
function PartyQuests_Frame_OnLoad()
	...
	if ( Khaos ) then 
		PartyQuests_RegisterWithKhaos();
	end
	
	-- Register with Sky for communication
...
function PartyQuests_Frame_OnEvent()
	...
	elseif ( event == "PARTY_MEMBERS_CHANGED" ) then
		if ( PartyQuestsLogFrame:IsVisible() and PartyQuests_CurrentQuest and PartyQuests_CurrentQuest.quest ) then
			if ( PartyQuests_CurrentQuest.quest.pushable and GetNumPartyMembers() > 0 ) then
				PartyQuestsLogFrameShareButton:Enable();
			else
				PartyQuestsLogFrameShareButton:Disable();
...
-- Hide
function PartyQuests_Frame_OnHide()
	UpdateMicroButtons();
...
function PartyQuests_LogFrame_Abandon_OnClick()
	local id = PartyQuests_MatchQuestToPlayerQuestID(PartyQuestsLog.questInfo.title, PartyQuestsLog.questInfo.level);
...
function PartyQuests_LogFrame_Share_OnClick()
	if ( PartyQuestsLog.questInfo ) then 
		if (PartyQuests_MatchQuestToPlayerQuestID(PartyQuestsLog.questInfo.title, PartyQuestsLog.questInfo.level)) then
			PartyQuests_ShareQuestByID(PartyQuestsLog.questInfo.id);
		else
			PartyQuests_RequestQuestShareFromPlayer(PartyQuestsLog.questInfo.username, PartyQuestsLog.questInfo.title, PartyQuestsLog.questInfo.level)
		end
	end
end
...
function PartyQuests_UpdateMicroButtons()
...
function PartyQuests_ReplaceDefaultQuestWindow ( state )
	if ( state.checked ) then 
		Sea.util.hook("ToggleQuestLog", "TogglePartyQuests", "replace");
		Sea.util.hook("UpdateMicroButtons", "PartyQuests_UpdateMicroButtons", "after");
	else
		Sea.util.unhook("ToggleQuestLog", "TogglePartyQuests", "replace");
		Sea.util.unhook("UpdateMicroButtons", "PartyQuests_UpdateMicroButtons", "after");
	end
end

function PartyQuests_ReplacedDefaultFeedback ( state ) 
	if ( state.checked ) then 
		return PQ_REPLACE_FEEDBACK_TRUE;
	else
		return PQ_REPLACE_FEEDBACK_FALSE;
	end
end
