...
function QuickMount_ShowUsage()
	QM_Print("/mountequip on | off | quiet | verbose | flightpoint [on/off] | config | status | load profileName | save profileName | delete profileName | profiles | detect\n");
end
...
function QuickMount_PickupInventoryItem(slot)
	...
	if CursorHasItem() then
		...
		if ( texture ) then
			local name = QuickMount_GetItemName(QM_NIL, slot)
...
function QuickMount_OnLoad()
	...
	if ( Sea and Sea.util and Sea.util.hook ) then
		Sea.util.hook("PickupContainerItem", "QuickMount_PickupContainerItem", "after");
		Sea.util.hook("PickupInventoryItem", "QuickMount_PickupInventoryItem", "after");
	else
