...
function ConsumeButton_Register_Cosmos()
	if ( ( Cosmos_UpdateValue ) and ( Cosmos_RegisterConfiguration ) and ( ConsumeButton_Cosmos_Registered == 0 ) ) then
...
function ConsumeButton_Generic_Toggle(toggle, variableName, CVarName, enableMessage, disableMessage, CosmosVarName)
	...
	if ( ConsumeButton_Cosmos_Registered == 1 ) then 
...
function ConsumeButton_GetItemNameFromLink(link)
	local name;
	if( not link ) then
		return nil;
	end
...
function ConsumeButton_GetItemName(bag, slot)
	...
	return ConsumeButton_GetItemNameFromLink(GetContainerItemLink(bag, slot));
end
...
function ConsumeButton_GetTotalNumberOfFoods()
	return ConsumeButton_GetTotalNumberOfType(CONSUMEBUTTON_CLASSIFICATION_FOOD);
end

function ConsumeButton_GetTotalNumberOfDrinks()
	return ConsumeButton_GetTotalNumberOfType(CONSUMEBUTTON_CLASSIFICATION_DRINK);
end
...
CONSUMEBUTTON_BUFF_ICON_WELLFED		= "Interface\\Icons\\";

function ConsumeButton_HasWellFedBuff()
	for i = 0, 15 do
		if ( GetPlayerBuffTexture(i) == CONSUMEBUTTON_BUFF_ICON_WELLFED )  then
			return true;
		end
	end
	return false;
...
function ConsumeButtonDisplayFrame_ChooseConsumables()
	ConsumeButton_Consumables_ChosenFood = nil;
	if ( not ConsumeButton_HasWellFedBuff() ) then
		for k, v in ConsumeButton_BuffFoodList do
			if ( ConsumeButton_Consumables[k] ) then
	...
	if ( not ConsumeButton_Consumables_ChosenFood ) then
		...
		for k, v in ConsumeButton_Consumables do
			if ( v == CONSUMEBUTTON_CLASSIFICATION_FOOD ) then
	...
	if ( not ConsumeButton_Consumables_ChosenDrink ) then
		...
		local minNumberIndex = nil;
		for k, v in ConsumeButton_Consumables do
			if ( v == CONSUMEBUTTON_CLASSIFICATION_DRINK ) then
				temp = ConsumeButton_GetTotalNumberOfConsumable(k);
				if ( temp < minNumber ) then
					minNumberIndex = k;
				end
...
function ConsumeButtonDisplayFrame_UpdateText()
	local drinkDescription = ConsumeButton_FormatDrinkText(ConsumeButtonDisplayFrame_GetDescriptionString(ConsumeButton_Consumables_ChosenDrink, CONSUMEBUTTON_CLASSIFICATION_DRINK));
	local foodDescription = ConsumeButton_FormatFoodText(ConsumeButtonDisplayFrame_GetDescriptionString(ConsumeButton_Consumables_ChosenFood, CONSUMEBUTTON_CLASSIFICATION_FOOD));
	ConsumeButtonDisplayFrameTextUpper:SetText(drinkDescription);
	ConsumeButtonDisplayFrameTextLower:SetText(foodDescription);
	ConsumeButtonDisplayFrameText:SetText(ConsumeButton_FormatDrinkText(ConsumeButtonDisplayFrame_GetDescriptionString(ConsumeButton_Consumables_ChosenDrink, CONSUMEBUTTON_CLASSIFICATION_DRINK, true)).." "..ConsumeButton_FormatFoodText(ConsumeButtonDisplayFrame_GetDescriptionString(ConsumeButton_Consumables_ChosenFood, CONSUMEBUTTON_CLASSIFICATION_FOOD, true)));
end

function ConsumeButtonDisplayFrame_IsExcluded(name, exclusionList)
	if ( not exclusionList ) then
		return false;
	end
	if ( not name ) then
		return true;
	end
	for k, v in exclusionList do
		if ( v == name ) then
			return true;
		end
	end
...
function ConsumeButtonDisplayFrame_ScanInventory(forced)
	...
	ConsumeButton_Consumables = {};
	ConsumeButton_Consumables_Positions = {};
	local foodClass = CONSUMEBUTTON_CLASSIFICATION_FOOD;
	local drinkClass = CONSUMEBUTTON_CLASSIFICATION_DRINK;
	local c, n = nil, nil;
	for bag = startBag, 4 do
		for slot = startSlot, GetContainerNumSlots(bag) do
			...
			if (itemInfo ) and ( itemInfo.name ) and ( strlen(itemInfo.name) > 0 ) then
				c = itemInfo.classification;
				n = itemInfo.name;
				if (( c == foodClass ) or ( c == drinkClass ) ) then
					if ( not strfind(CONSUMEBUTTON_NAME_RECIPE_SUBSTRING, n) ) 
						and ( not ConsumeButtonDisplayFrame_IsExcluded(n, ConsumeButton_Exclusion_List[c]) )
						then
						ConsumeButton_Consumables[n] = itemInfo.classification;
						if ( not ConsumeButton_Consumables_Positions[n] ) then
							ConsumeButton_Consumables_Positions[n] = {};
						end
						local element = {};
...
function ConsumeButton_GetByteValue(pValue)
...
function ConsumeButton_GetColorFormatString(a, r, g, b)
	...
	a = ConsumeButton_GetByteValue(a); r = ConsumeButton_GetByteValue(r); g = ConsumeButton_GetByteValue(g); b = ConsumeButton_GetByteValue(b);
