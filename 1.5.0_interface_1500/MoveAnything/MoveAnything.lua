...
MoveAnything_VariablesLoaded = false;
MoveAnything_PlayerNameValid = false;
...
MoveAnything_DefaultFrameList = {
	...
	{ "MirrorFrame1", "Breath/Fatigue Bar" },
...
function MoveAnything_FindMAFrame( moveFrameName )
	...
	for i = 1, MOVEANYTHING_MOVEATONETIME, 1 do
		...
		if( frame:IsVisible() and frame.tagged == getglobal( moveFrameName ) ) then
			return frame;
		end
	end
	return nil;
...
function MoveAnything_GetParentScaleRatio( frame )
	return MAGetScale( frame ) / MAGetScale( MAGetParent( frame ) );
end
...
function MoveAnything_Attach( moveFrame, tagFrame )
	...
	x = tagFrame:GetLeft() * MAGetScale( tagFrame ) / UIParent:GetScale();
	x = x - MAGetParent( tagFrame ):GetLeft() * MAGetScale( MAGetParent( tagFrame ) ) / UIParent:GetScale();
	y = tagFrame:GetBottom() * MAGetScale( tagFrame ) / UIParent:GetScale();
	y = y - MAGetParent( tagFrame ):GetBottom() * MAGetScale( MAGetParent( tagFrame ) ) / UIParent:GetScale();
	moveFrame:ClearAllPoints();
	moveFrame:SetPoint( "BOTTOMLEFT", MAGetParent( tagFrame ):GetName(), "BOTTOMLEFT", x, y );
	moveFrame:SetWidth( tagFrame:GetWidth() * MAGetScale( tagFrame ) / UIParent:GetScale() );
	moveFrame:SetHeight( tagFrame:GetHeight() * MAGetScale( tagFrame ) / UIParent:GetScale() );
...
function MoveAnything_SetLeftFrameLocation()
	...
	if( left ) then
		if( MoveAnything_IsFrameHooked( "UIPanelMover1" ) ) then
			...
			left:SetScale( MAGetScale( UIPanelMover1 ) );
		else
			left:SetScale( UIParent:GetScale() );
		end
	end
end
	...
	if( GetLeftFrame() ) then
		...
		if( center ) then
			if( MoveAnything_IsFrameHooked( "UIPanelMover2" ) ) then
				...
				center:SetScale( MAGetScale( UIPanelMover2 ) );
			else
				center:SetScale( UIParent:GetScale() );
			end
		end
	end
end
...
--TODO:
--DONE** Confirmation on Reset All
--DONE** Custom list of frames
--DONE** Resizing buttons
--DONE** Check global UI scale
--DONE** Loot roll frame
--DONE** Friendly operation with myAddons and other people who add buttons to their main menu
--DONE** Item buff frame and debuffs
--ChatFrameEditBox move
--DONE** Combo points
--DONE** Look at spacing on pet bar
--DONE** Look at player/pet/bottom bar etc when using per-character settings