...
function Dcr_OnUpdate(arg1)
    ...
    if (Dcr_CheckingPET) then
		...
		if (Dcr_CheckPet_Timer >= Dcr_CheckPet_Delay) then
		    ...
		    if (curr_DemonType) then 
		    	--Dcr_debug_bis ("Pet Type: " .. curr_DemonType);  
		    end;
	
		    if (last_DemonType ~= curr_DemonType) then
				if (curr_DemonType) then 
					--Dcr_debug_bis ("Pet name changed: " .. curr_DemonType); 
				else  
					--Dcr_debug_bis ("No more pet!"); 
				end;
...
function Dcr_Cure_Magic(Magic_Count, Unit)
    ...
    elseif (DCR_CAN_CURE_MAGIC and (not UnitCanAttack("player", Unit))) and ( not UnitIsCharmed(Unit) or UnitCanAttack("player", Unit) ) then
