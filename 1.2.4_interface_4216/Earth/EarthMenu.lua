...
function EarthMenu_ExpandArrowScriptedTemplate_OnClick ()
	if ( not this.onClick ) then 
		EarthMenu_ToggleDropDownMenu(this:GetParent():GetParent():GetID()+1, this:GetParent().value);	
...
function EarthMenu_ButtonScriptedTemplate_OnEnter()
	if ( not this.onEnter ) then
		if ( this.hasArrow ) then
			EarthMenu_ToggleDropDownMenu(this:GetParent():GetID() + 1, this.value);
		end
...
function EarthMenu_ListScriptedTemplate_OnUpdate()
...
function EarthMenu_ToggleDropDownMenu(level, value, dropDownFrame, anchorName, xOffset, yOffset)
	if ( not level ) then
		level = 1;
	end
	...
	if ( listFrame:IsVisible() and (EARTH_UIDROPDOWNMENU_OPEN_MENU == tempFrame:GetName()) ) then
		...
		if ( Chronos ) then 
			Chronos.scheduleByName("EarthMenuAutoHide".."EarthDropDownList"..level, EARTHMENU_HIDE_TIMEOUT, function() EarthMenu_HideFrame("EarthDropDownList", level); end);
		end
...
function EarthMenuButton_OpenColorPicker(button)
	...
	EARTH_UIDROPDOWNMENU_MENU_VALUE = button.value;
...
function EarthMenu_MenuOpen(menulist, anchor, x, y, mode)
	...
	EarthMenu_ToggleDropDownMenu(1, nil, EarthMenu, anchor, x, y);
end
