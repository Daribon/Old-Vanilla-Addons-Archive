...
--[[ The Workhorse ]]--
function EarthTable_HandleEvent(frame, id, action, ... ) 
	...
	elseif ( action == "CLICK_ROW" ) then
		item = EarthTable_GetDatabaseItemByID(frame,id);
	...
	elseif ( action == "ENTER_HEADER" ) then
		header = EarthTable_GetHeaderByID(frame,id);

		if ( header and header.tooltip ) then 
			EarthTable_SetTooltip(frame, headertooltip);
		end
		update = false;
