...
function BidHelper_SetButtonPos(pos)
	BidHelperMinimapButton:ClearAllPoints();
	BidHelperMinimapButton:SetPoint(
		"TOPLEFT",
		"Minimap",
		"TOPLEFT",
		55 - (75 * cos(pos)),
...
function BidHelper_SetMinimapPosition(pos, quiet)
	if ( not quiet ) then
		BidHelper_Print(string.format(BIDHELPER_CHAT_POSITION, pos));
	end
	if ( pos >= 0 ) and ( pos <= 360 ) then
		BidHelper_SetButtonPos(pos);
		BidHelper_Options.minimapButtonPosition = pos;
	end
end

function BidHelper_SetMinimapPosition_NoChat(pos)
	BidHelper_SetMinimapPosition(pos, true);
end
...
function BidHelper_Register_Cosmos()
	if ( ( Cosmos_UpdateValue ) and ( Cosmos_RegisterConfiguration ) and ( BidHelper_Cosmos_Registered == 0 ) ) then
		...
		Cosmos_RegisterConfiguration(
			"COS_BIDHELPER_POSITION",
			"SLIDER",
			TEXT(BIDHELPER_POSITION),
			TEXT(BIDHELPER_POSITION_INFO),
			BidHelper_SetMinimapPosition_NoChat,
			1,
			BidHelper_Options.minimapButtonPosition,
			0,
			360,
			"",
			1,
...
function BidHelper_Generic_Toggle(toggle, variableName, CVarName, enableMessage, disableMessage, CosmosVarName)
	...
	BidHelper_Register_Cosmos();
	if ( BidHelper_Cosmos_Registered == 1 ) then 
