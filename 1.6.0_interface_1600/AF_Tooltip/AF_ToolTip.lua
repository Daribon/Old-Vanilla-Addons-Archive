	...
	--Set our options frame refresh
		if (aftt_optionFrame:IsVisible()) then
			...
			elseif (AF_ToolTip[aftt_localUser]["Guild"] == 2) then
				...
				aftt_TextLeft9:SetText(aftt_color_chart_name);
				aftt_TextLeft10:SetText(aftt_color_chart_author);
		end
...
function aftt_format(unit)
	...
	-----------------------------------------------------------
		--detect if on same unit from previous refresh
			...
			if (aftt_updateFlag["Current"] == aftt_updateFlag["Previous"] and aftt_updateFlag["Previous"] ~= "none") then
				--do not reset flags, were on the same unit
			else
				aftt_updateFlag = {
					["EasterEgg"] = 0;
					["Guild"] = 0;
	...
	-----------------------------------------------------------
		if (AF_ToolTip[aftt_localUser]["ReactionText"] == 1) then
			GameTooltipTextLeft1:SetText("|cff" .. aftt_color_name .. UnitName(unit) .. " (" .. aftt_text_reaction .. ")|r")
		else
			GameTooltipTextLeft1:SetText("|cff" .. aftt_color_name .. UnitName(unit) .. "|r");
		end
	...
	--Append Easter Eggs---------------------------------------
	-----------------------------------------------------------
		if (aftt_updateFlag["EasterEgg"] == 0 and UnitName(unit) == "Iniko" and (GetCVar("realmName") == "Firetree" or GetCVar("realmName") == "Stormscale" or GetCVar("realmName") == "Arthas")) then
			GameTooltip:AddLine("|cff" .. aftt_color_name .. "Author of AF_ToolTip|r");
