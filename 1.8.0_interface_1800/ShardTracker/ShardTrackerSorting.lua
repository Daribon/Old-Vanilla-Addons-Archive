--[[
    ...
    Originally written by Ryu (ShardSort)
    Modified by Cragganmore
...
function ShardTracker_UpdateShardMoverCleanup()
    ...
    -- pick the furthest consecutive shard
    for i = ShardTracker_ShardMoverCleanupSlot + 1, GetContainerNumSlots(SHARDTRACKER_CONFIG.THE_SORT_BAG) do
        ...
        if (ShardTracker_ShouldBeSorted(itemName)) then
            ...
            foundAshard = true;
            swapSlot = i;
        elseif (foundAshard) then
            break;
        end
    end

    -- if we found a shard to swap with, move it    
    if (foundAshard) then
        PickupContainerItem(SHARDTRACKER_CONFIG.THE_SORT_BAG, swapSlot);
...
--
-----------------------------------------------------------------------------------
function ShardTracker_SortShards(showSortingMessage)

    if (UnitClass("player") == SHARDTRACKER_WARLOCK) then    
    
        if (showSortingMessage == nil) then showSortingMessage = true; end
        ...
        ShardTracker_ToggleSortFrame(true, showSortingMessage);
...
function ShardTracker_ToggleSortFrame(toggle, OKtoDisplay)
    if (toggle and OKtoDisplay) then
            ...
            ColorCycle_AlternateColors(
                {
                    ...
                    globalType  = "FontText";
                    color1      = "lightyellowgreen"; 
                    color2      = "lightbluepurple"; 
                    unique      = true;
                }
            );
        end
        ...
        if (ColorCycle_Exists) then
            ColorCycle_Remove("ShardTrackerSortText");
        end
...
function ShardTracker_ShouldBeSorted(itemName)
    ...
    if (itemName == SHARDTRACKER_SOULSHARD or 
        string.find(itemName, SHARDTRACKER_SOULSTONE, 1, true) or 
        string.find(itemName, SHARDTRACKER_HEALTHSTONE, 1, true) or 
        string.find(itemName, SHARDTRACKER_SOULSTONE, 1, true) or 
        string.find(itemName, SHARDTRACKER_FIRESTONE, 1, true) or 
        string.find(itemName, SHARDTRACKER_SPELLSTONE, 1, true)) then
        return true;
    else
        return false;
    end
...
function ShardTracker_SetShardBag(msg)
    ...
    if (UnitClass("player") == SHARDTRACKER_WARLOCK) then
        if (msg == "" or tonumber(msg) < 0 or tonumber(msg) > 4) then
            if ( ChatFrame1 ) then
                ShardTrackerPrint("Usage: /shardtracker bag [0-4]. Sets the default bag for shards.");
                ShardTrackerPrint("0 is the backpack, 1-4 are rightmost to leftmost bag slots.");
            end
            return;
        else
            SHARDTRACKER_CONFIG.THE_SORT_BAG = tonumber(msg);
            if ( ChatFrame1 ) then
                 ShardTrackerPrint("Default shard bag is now "..SHARDTRACKER_CONFIG.THE_SORT_BAG..".");
            end
        end
    else
        ShardTrackerPrint(ST_ONLYFORWARLOCKS);
