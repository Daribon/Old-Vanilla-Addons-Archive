--[[
	...
	Version: <%version%>
	Revision: $Id: localization.lua,v 1.7 2005/06/11 02:58:40 islorgris Exp $
...
elseif ( GetLocale() == "deDE" ) then
	...
	HERB_FIREBLOOM         ="feuerbl\195\188t"
	...
	function Gatherer_FindOreType(input)
		...
		-- fix for "rich" and "small" adjectives
		--oreTypeClass = string.gsub(string.gsub(input, "kleines ", ""), "reiches ", "");
		-- fix for ooze covered, pending
	...
	function Gatherer_FindTreasureType(input)
		...
		-- this block for the case where keyword is the first one
		i,j, treasType = string.find(input, "^([^ ]+)");
		if (treasType and (treasType == TREASURE_CHEST) or (treasType == TREASURE_BARREL) or (treasType == TREASURE_CRATE) or (treasType == TREASURE_BOX)  or (treasType == TREASURE_CASK) or (treasType == TREASURE_CLAM)) then
			return treasType;
		end
