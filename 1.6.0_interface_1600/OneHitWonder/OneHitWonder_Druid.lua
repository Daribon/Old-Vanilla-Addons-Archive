...
function OneHitWonder_SetupStuffContinuously_Druid()
	...
	OneHitWonder_AddStuffContinuously(ONEHITWONDER_SPELL_MARK_OF_THE_WILD_NAME, false, true, {effectName={ONEHITWONDER_SPELL_MARK_OF_THE_WILD_EFFECT, ONEHITWONDER_SPELL_GIFT_OF_THE_WILD_EFFECT}, useWhenHigherManaPercentage=95});
...
function OneHitWonder_ShouldTryToCastABuff_Druid(ignoreCloakEffects)
	local currentForm = OneHitWonder_Druid_GetCurrentShapeshiftform();
	if ( currentForm == ONEHITWONDER_DRUID_SHAPESHIFT_FORM_NORMAL ) then
		return true;
