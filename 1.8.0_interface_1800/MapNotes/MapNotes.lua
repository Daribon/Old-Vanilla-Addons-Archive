...
MapNotes_Const[MAPNOTES_ARATHIBASIN] = { scale = 0.045, xoffset = 0.41757282062541, 
                         yoffset = 0.33126468682991, xscale = 12897.3, yscale = 8638.1 };

function MapNotes_ConvertFromOldZoneShift()
   local temp = { };

   if( not MapNotes_ZoneShiftOld ) then
       MapNotes_StatusPrint("No old zoneshift defined.");
       return;
    end
	...
	elseif (event == "VARIABLES_LOADED") then
		...
		MapNotes_ImportCartoPlus(); -- for testin only, doesn't work correctly, because of the word wraps...
                ...
                if (Sea) then
				   ...
				   MapNotes_WorldMapButton_OnUpdate(elapsed);
                                   old_WorldMapButton_OnUpdate(elapsed);
                     		 end
...
function MapNotes_WorldMapButton_OnUpdate(elapsed)
   local currentZone;
   local continent = GetCurrentMapContinent();
