***********************************
Scrolling Combat Text Versions 3.5
***********************************
...
examples or try this site http://lua-users.org/wiki/PatternsTutorial. Its also possible you are trying 
to search for an event SCT is not using (very few).

How do I change the text for parry, block, etc...? - open up the localiztions.lua file and look 
for the event you want to change. Then change the text to whatever you like.

How do I get get text to scroll? I only see numbers! - Make sure the "Show Events as Message" 
option is unchecked. This is only if you want events to appear in the games default message area ("Not 
enough Mana", "You are too far away", etc...)

I don't like the message "ABCDEFG" but I can't find where to turn it off in the options! - remove 
or comment out the default custom events you don't want in sct_event_config.lua (notepad, etc...).

How do I change the fonts? - Doing this is easy, but I will not support it beyond telling you 
where to change it =) Look in sct.xml, on line 9 in the FontString element you will want to change the 
font it is pointing to.
