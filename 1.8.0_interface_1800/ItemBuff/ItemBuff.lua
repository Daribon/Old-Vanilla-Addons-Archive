		...
		if( hasItem ) then
			for index = 1, 15, 1 do
				...
				if( field and field:IsVisible() ) then
					...
					if( text and (string.find(text, SEARCH_STRING_MIN) or string.find(text, SEARCH_STRING_SEC)) ) then
						return 1;
					end
				end
			end
...
local function ItemBuff_UpdateWindow()
	...
	if( iVisible == 0 ) then
		...
		ItemBuffFrame:SetHeight(6 + iVisible * 42);
		ItemBuffFrame:SetWidth(48 + maxSize - 1);
		lOkayToShow = 1;
	end
end
		...
		for index = 1, 15, 1 do
			...
			if( field and field:IsVisible() ) then
				...
				if( text ) then
					...
					if( string.find(text, SEARCH_STRING_MIN) ) then
						label:SetTextColor(0.0, 1.0, 0.0);
						label:SetText(text);
						return;
					end
					
					iStart, iEnd, secs = string.find(text, SEARCH_STRING_SEC)
					if( secs ) then
						secs = secs + 0;
...
local function ItemBuff_Reset()
	...
	ItemBuffFrame:SetPoint("BOTTOMLEFT", "ChatFrame1", "TOPLEFT", -30, 30);
end
...
function ItemBuff_OnLoad()
	...
	if( DEFAULT_CHAT_FRAME ) then
		--DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00Xanadu: |r|cffffffffTelo's ItemBuff loaded.|r");
	end
end
...
function ItemBuff_OnEnter()
	...
	if( lOkayToShow and (not ItemBuffState or not ItemBuffState.Freeze) ) then
		ItemBuffBackdrop:Show();
		ItemBuffBackdrop:SetBackdropColor(0, 0, 0, 0.6);
	end
end
