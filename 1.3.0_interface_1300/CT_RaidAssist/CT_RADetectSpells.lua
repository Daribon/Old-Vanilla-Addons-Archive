...
function CT_RA_newUseAction(a1, a2, a3)
	...
	CT_RADetectSpells_ParseSpellTooltip(a1);
	if ( a3 and a3 == 1 ) then
	...
	if ( this.update ) then
		...
		if ( this.update <= 0 ) then
			...
			elseif ( not CT_RA_CastSpell and UnitExists("mouseover") and CT_RA_SpellBeingCast and not SpellIsTargeting() ) then
				...
				CT_RA_SpellStartCast(CT_RA_CastSpell);
			elseif ( GameTooltip:IsVisible() and CT_RA_SpellBeingCast and not SpellIsTargeting() ) then
...
function CT_RADetectSpells_ParseSpellTooltip(action)
	CT_RA_CastSpell = nil;
