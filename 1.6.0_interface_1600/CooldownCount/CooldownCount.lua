--[[
	...
	Thanks goes to Drex of the #ultimateuitesters channel for suggesting this!
...
CooldownCount_UltimateUI_Registered = 0;
CooldownCount_SideBarOffset = 0;
...
CooldownCount_ButtonNames = {
	"ActionButton", "BonusActionButton", "ShapeshiftButton", 
};
...
function CooldownCount_Register_UltimateUI()
	if ( ( UltimateUI_UpdateValue ) and ( UltimateUI_RegisterConfiguration ) and ( CooldownCount_UltimateUI_Registered == 0 ) ) then
		...
		UltimateUI_RegisterConfiguration(
			"UUI_COOLDOWNCOUNT_HEADER",
			"SEPARATOR",
			TEXT(COOLDOWNCOUNT_CONFIG_HEADER),
			TEXT(COOLDOWNCOUNT_CONFIG_HEADER_INFO)
		);
		UltimateUI_RegisterConfiguration(
			"UUI_COOLDOWNCOUNT_ENABLED",
			"CHECKBOX",
		...
		--[[
		UltimateUI_RegisterConfiguration(
			"UUI_COOLDOWNCOUNT_USELONGTIMERS",
			"CHECKBOX",
		...
		CooldownCount_UltimateUI_Registered = 1;
	end
end

-- registers the mod with the system, integrating it with slash commands and "master" AddOns
function CooldownCount_Register()
	if ( UltimateUI_RegisterConfiguration ) and ( UltimateUI_UpdateValue ) then
		CooldownCount_Register_UltimateUI();
	else
...
function CooldownCount_SetFlashSpeed(speed)
	if ( speed ) then
		...
		if ( CooldownCount_UltimateUI_Registered == 0 ) then
...
function CooldownCount_UpdateSideBars()
	...
	for i = 1, 12 do
		...
		button = getglobal(tmpSideBarButton);
		if ( button ) then
...
function CooldownCount_UpdateSideBarOffset(toggle, offset)
	CooldownCount_SetSideBarOffset(offset);
end
...
function CooldownCount_SetupSideBarFrames(onTop)
	...
	for i = 1, 12 do
		...
		button  = getglobal(sideBarButtonStr);
		if ( button ) then
