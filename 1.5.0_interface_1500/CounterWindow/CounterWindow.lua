...
Window_AutoOpenOnLoad_Window = "XPMegaCounterWindowFrame";

CounterWindow_Enabled = 0;
CounterWindow_AutoOpenOnLoad = 0;
...
function CounterWindow_Register_Cosmos()
	if ( ( Cosmos_UpdateValue ) and ( Cosmos_RegisterConfiguration ) and ( CounterWindow_Cosmos_Registered == 0 ) ) then
		...
		Cosmos_RegisterConfiguration(
			"COS_COUNTERWINDOW_AUTOOPENONLOAD",
			"CHECKBOX",
			TEXT(COUNTERWINDOW_AUTOOPENONLOAD),
			TEXT(COUNTERWINDOW_AUTOOPENONLOAD_INFO),
			CounterWindow_Toggle_AutoOpenOnLoad,
			CounterWindow_AutoOpenOnLoad
		);
		Cosmos_RegisterConfiguration(
...
function CounterWindow_Main_ChatCommandHandler(msg)
	...
	elseif ( strfind( commandName, "auto" ) ) then
		func = CounterWindow_Toggle_AutoOpenOnLoad;
...
function CounterWindow_OnEvent(event)
	if ( event == "VARIABLES_LOADED" ) then
		if ( CounterWindow_Cosmos_Registered == 0 ) then
			local value = getglobal("CounterWindow_Enabled");
			if (value == nil ) then
				-- defaults to off
				value = 0;
			end
			CounterWindow_Toggle_Enabled(value);
			value = getglobal("CounterWindow_AutoOpenOnLoad");
			if (value == nil ) then
				-- defaults to off
				value = 0;
			end
			CounterWindow_Toggle_AutoOpenOnLoad(value);
		else
			CounterWindow_AutoOpenOnLoad_LOADED = true;
		end
		if ( CounterWindow_AutoOpenOnLoad == 1 ) then
			local frame = getglobal(Window_AutoOpenOnLoad_Window);
			if ( frame ) then
				frame:Show();	
			end
		end
...
function CounterWindow_Generic_Toggle(toggle, variableName, CVarName, enableMessage, disableMessage, CosmosVarName)
	...
	if ( CounterWindow_Cosmos_Registered == 1 ) then 
...
function CounterWindow_Toggle_AutoOpenOnLoad(toggle)
	local newvalue = CounterWindow_Generic_Toggle(toggle, "CounterWindow_AutoOpenOnLoad", "COS_COUNTERWINDOW_AUTOOPENONLOAD_X", "COUNTERWINDOW_CHAT_AUTOOPENONLOAD_ENABLED", "COUNTERWINDOW_CHAT_AUTOOPENONLOAD_DISABLED");
	if ( CounterWindow_AutoOpenOnLoad_LOADED ) then
		CounterWindow_AutoOpenOnLoad_LOADED = false;
		local frame = getglobal(Window_AutoOpenOnLoad_Window);
		if ( frame ) then
			frame:Show();	
		end
