	...
	elseif DYKey and DYKey.On then
		...
		elseif event == "CHAT_MSG_COMBAT_PARTY_HITS" or event == "CHAT_MSG_COMBAT_PARTY_MISSES" then
			...
			if DYKey.Assist then
				...
				--[[if pname and not UnitExists("target") and DYVar.buff then
					if (UIOptionsFrameCheckButtons["ASSIST_ATTACK"].value == "0") then
						AssistUnit("party"..pname);
					else
						UIOptionsFrameCheckButtons["ASSIST_ATTACK"].value = "0";
						AssistUnit("party"..pname);
						UIOptionsFrameCheckButtons["ASSIST_ATTACK"].value = "1";
					end
				end]]
			end
...
--[[
function DefendYourself_Fetch_OnUpdate()
	if not Fetch_Frame then 
		DefendYourself_UnitSave(0);
	elseif Fetch_Done then 
		DefendYourself_UnitSave(1);
	end
end]]
...
function DefendYourself_Main_ChatCommandHandler(msg)
	...
	elseif strfind(msg[1], "on") then
		DefendYourself_Print("DefendYourself! is on"); DYKey.On = true;
	elseif strfind(msg[1], "off") then
		DefendYourself_Print("DefendYourself! is off"); DYKey.On = nil;
	else
...
function DefendYourself_Help()
		...
		DefendYourself_Print("TLHP is:"..dycol(DYKey.HP));
		...
		DefendYourself_Print("Use /dy help to see this, /dy on turns it on, /dy off turns it off, and /dy toggle turns it between the two. /dy menu handles the menu!");
end
...
function DefendYourself_GetBloody()
	...
	--(rage4 level indicator stolen from the doom comic. HERE COMES THE NIGHT TRAIN!)
	if not DYKey.blvl then DYKey.blvl = 1; end
	if DYKey.blvl == 1 then
		DYVar.hate = true;
