...
function OneHitWonder_SetupStuffContinously_Mage()
	...
	if ( OneHitWonder_Mage_ShouldBuffNonCastersIntellect == 1 ) then
		OneHitWonder_AddStuffContinously(ONEHITWONDER_SPELL_ARCANE_INTELLECT_NAME, false, true, { useWhenHigherManaPercentage=95 });
	else
		OneHitWonder_AddStuffContinously(ONEHITWONDER_SPELL_ARCANE_INTELLECT_NAME, false, true, {useWhenHigherManaPercentage=95, powerType = { ONEHITWONDER_POWERTYPE_MANA } });
		...
		OneHitWonder_AddStuffContinously(ONEHITWONDER_SPELL_ARCANE_INTELLECT_NAME, false, true, {useWhenHigherManaPercentage=95, onlyBuffClass = {ONEHITWONDER_CLASS_DRUID}});
	end
	...
	if ( ( OneHitWonder_Mage_ShouldBuffDampenMagicWhenNoHealer == 1 ) and ( OneHitWonder_GetNumberOfClassInParty(OneHitWonder_HealerClassesArray) <= 0 ) ) then
		OneHitWonder_AddStuffContinously(ONEHITWONDER_SPELL_DAMPEN_MAGIC_NAME, false, true, {invalidTarget={"target"},useWhenHigherManaPercentage=50, validTarget={"player","party1","party2","party3","party4","pet"}});
	end
end
...
function OneHitWonder_DoStuffContinously_Mage()
