...
function RestReminder_ShouldPopupWarning()
	local level = UnitLevel("player");
	if ( not IsResting() ) and ( level < 60 ) then
		if ( level < 59 ) or ( GetXPExhaustion() < ( UnitXPMax("player") - UnitXP("player") ) ) then
			return true;
		end
	end
	return false;
...
function RestReminder_StaticPopup_Show(which, text_arg1, text_arg2)
	if ( ( RestReminder_Enabled == 1 ) and ( RestReminder_IsRightPopup(which) ) ) then
		if ( RestReminder_ShouldPopupWarning() ) then
			RestReminder_Warning(TEXT(RESTREMINDER_WARNING));
		end
	end
	return RestReminder_Saved_StaticPopup_Show(which, text_arg1, text_arg2);
end
