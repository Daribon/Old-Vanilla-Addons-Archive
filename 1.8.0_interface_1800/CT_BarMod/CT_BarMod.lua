...
function CT_ActionButton_HideGrid(button)
	...
	if ( CT_ShowGrid == 1 and isException ) then return; end
	btn.showgrid = 0;
...
function gridfunction(modId)
	...
	local i;
	if ( val == "off" ) then
		CT_Print(BARMOD_OFF_BUTTONGRID, 1, 1, 0);
		CT_ShowGrid = 1;
		if ( CT_BottomBar_Enabled and CT_BottomBar_HiddenFrames[UnitName("player")]["BarHotbar"] and CT_BottomBar_Enabled[UnitName("player")] ) then
			return;
		end
		for i=1, 12, 1 do
			...
			getglobal("MultiBarBottomLeftButton" .. i):Show();
			getglobal("MultiBarBottomRightButton" .. i):Show();
			getglobal("MultiBarLeftButton" .. i):Show();
			getglobal("MultiBarRightButton" .. i):Show();
		...
		CT_Print(BARMOD_ON_BUTTONGRID, 1, 1, 0);
		CT_ShowGrid = 0;
		for i=1, 12, 1 do
...
CT_RegisterMod(BARMOD_MODNAME_MOVEPARTYFRAME, BARMOD_ONOFFTOGGLE, 5, "Interface\\Icons\\Spell_Holy_MindSooth", BARMOD_TOOLTIP_MOVEPARTYFRAME, "off", nil, movefunction, CT_CheckPartyMove);
...
function CT_updateActionButtons()
	for i = 1, 12, 1 do
		...
		--[[local ab = getglobal("ActionButton" .. i);
		if ( not HasAction(ActionButton_GetPagedID(ab)) or CT_ShowGrid == 1 ) then
			if ( 
				( CT_ShowGrid == 0 and not HasAction(CT_ActionButton_GetPagedID(ab)) ) or 
...
function CT_newActionButton_ShowGrid(button)
	...
	if ( 
		( strsub(button:GetName(), 1, 12) == "ActionButton" and GetBonusBarOffset() > 0 ) or
		...
		button:Hide();
	elseif ( strsub(button:GetName(), 1, 17) == "BonusActionButton" and GetBonusBarOffset() == 0 ) then
		button:Hide();
	else
		button:Show();
	end
...
function CT_BarMod_UpdateCooldownCount()
	local cdText, numLines = "", 0;
	for b = 1, 7, 1 do
		local prefix = "CT" .. b .. "_";
		if ( b == 1 ) then
			prefix = "CT_";
		...
		for i = 1, 12, 1 do
			if ( CT_CDCount == 1 ) then
				...
				if ( CT_BarMod_Cooldowns[name] and CT_BarMod_Cooldowns[name][3] > 0 ) then
					...
					if ( strlen(cdText) > 0 ) then
						cdText = cdText .. "\n";
					end
					cdText = cdText .. ( CT_BarMod_Cooldowns[name][4] or "Unknown" ) .. ": |c00FFFFFF" .. form .. "|r";
					numLines = numLines + 1;
	...
	CT_CDCFrameText:SetText(cdText);
	CT_CDCFrame:SetWidth(CT_CDCFrameText:GetStringWidth()+15);
	CT_CDCFrame:SetHeight(15*numLines+25);
end
