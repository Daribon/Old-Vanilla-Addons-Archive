...
function AddOnHelper_IsValueInList(soughtValue, list)
	if ( not value ) or ( not list ) then
		return false;
	end
	local tvalue = type(soughtValue);
	local tlist = type(list);
	if ( tvalue == "table" ) and ( tlist == "table" ) then
		if ( table.getn(soughtValue) <= 0 ) then
			return false;
		end
		local found = false;
		for k, v in soughtValue do
			found = false;
	...
	if ( tvalue == "table" ) and ( tlist ~= "table" ) then
		soughtValue, list = list, soughtValue;
	elseif ( tvalue ~= "table" ) and ( tlist ~= "table" ) then
		if ( soughtValue == list ) then
			return true;
		else
			return false;
		end
