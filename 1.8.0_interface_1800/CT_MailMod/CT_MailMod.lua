...
function CT_newPickupContainerItem(bag, item, special)
	...
	elseif ( IsAltKeyDown() and not CursorHasItem() and not TradeFrame:IsVisible() and UnitExists("target") and CheckInteractDistance("target", 2) ) then
		InitiateTrade("target");
		CT_Mail_addItem = { bag, item };
...
function CT_Mail_newTradeFrameShow()
	...
	if ( CT_Mail_addItem and not CursorHasItem() ) then
