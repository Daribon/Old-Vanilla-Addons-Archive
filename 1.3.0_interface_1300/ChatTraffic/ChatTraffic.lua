...
function ChatTraffic_Schedule(...)
	if ( not arg ) then
		arg = {};
	end
	if ( Cosmos_Schedule ) then
		Cosmos_Schedule(unpack(arg));
	elseif ( Chronos ) and ( Chronos.schedule ) then
		Chronos.schedule(unpack(arg));
	else
		return false;
	end
end

function ChatTraffic_ScheduleByName(...)
	if ( not arg ) then
		arg = {};
	end
	if ( Cosmos_ScheduleByName ) then
		Cosmos_ScheduleByName(unpack(arg));
	elseif ( Chronos ) and ( Chronos.scheduleByName ) then
		Chronos.scheduleByName(unpack(arg));
	else
		return false;
	end
end
...
function ChatTraffic_SendChatMessage(message, ...)
	if ( ( ChatTraffic_Enabled == 1 ) and ( not ChatTraffic_ExemptFromTrafficLimitation(message) ) ) then
		...
		if ( currentTrafficLeft < messageLen ) then
			ChatTraffic_Schedule(CHATTRAFFIC_TRAFFIC_DELAY, ChatTraffic_SendChatMessage, message, unpack(arg));
			return;
		else
			ChatTraffic_AddTrafficSent(message);
		end
