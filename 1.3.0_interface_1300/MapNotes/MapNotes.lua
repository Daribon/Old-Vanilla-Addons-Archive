...
Date: 31. Jan 2005 15:17 GMT + 1
Version: 0.5.5

Website: www.wowwiki.com/Map_Notes
em@il: mapnotes@metelmann.name
...
Additional Note: Main idea: CTmod, requested on cosmosui.org
                 ...
                 sarf    5/19/05 - Fixed so that Sky is not required. There were hidden dependencies before.
                                 - Updated the code to allow compatibility with old Cosmos channel code

                 Marsman 3/27/05 - Changed Player to Player notes to use Sky
                                 - Added code to show Raid Member positions
...
RegisterForSave("MapNotes_Options");
RegisterForSave("MapNotes_Data");
RegisterForSave("MapNotes_MiniNote_Data");
RegisterForSave("MapNotes_Lines");
MapNotes_Options = {};
MapNotes_Data = {};
MapNotes_Data[1] = {};
MapNotes_Data[2] = {};
...
MapNotes_CosmosIsRegistered = false;
...
function MapNotes_OnLoad()
	...
	if ( ( Cosmos_RegisterConfiguration ) and ( MapNotes_CosmosIsRegistered == false ) ) then
		-- Override the Thottbot tloc (deactivated)
		if ( ThottbotReplace_ReplaceIt ) then ThottbotReplace_ReplaceIt(); end
		if ( Cosmos_RegisterChatCommand ) then
		local MapNotesEnableCommands = {"/mapnote", "/MapNote", "/MAPNOTE"};
		Cosmos_RegisterChatCommand (
			"MAPNOTES_ENABLE_COMMANDS",
			MapNotesEnableCommands,
			MapNotes_GetNoteBySlashCommand,
			MAPNOTES_CHAT_COMMAND_ENABLE_INFO
			);
		local MapNotesOneNoteCommands = {"/onenote", "/allowonenote", "/aon"};
		Cosmos_RegisterChatCommand (
			"MAPNOTES_ONEOTE_COMMANDS",
			MapNotesOneNoteCommands,
			MapNotes_OneNote,
			MAPNOTES_CHAT_COMMAND_ONENOTE_INFO
			);
		local MapNotesMiniNoteCommands = {"/nextmininote", "/nextMiniNote", "/nmn"};
		Cosmos_RegisterChatCommand (
			"MAPNOTES_MININOTE_COMMANDS",
			MapNotesMiniNoteCommands,
			MapNotes_NextMiniNote,
			MAPNOTES_CHAT_COMMAND_MININOTE_INFO
			);
		local MapNotesMiniNoteOnlyCommands = {"/nextmininoteonly", "/nextMiniNoteOnly", "/nmno"};
		Cosmos_RegisterChatCommand (
			"MAPNOTES_MININOTEONLY_COMMANDS",
			MapNotesMiniNoteOnlyCommands,
			MapNotes_NextMiniNoteOnly,
			MAPNOTES_CHAT_COMMAND_MININOTEONLY_INFO
			);
		local MapNotesMiniNoteOffCommands = {"/mininoteoff", "/MiniNoteOff", "/mno"};
		Cosmos_RegisterChatCommand (
			"MAPNOTES_MININOTEOFF_COMMANDS",
			MapNotesMiniNoteOffCommands,
			MapNotes_ClearMiniNote,
			MAPNOTES_CHAT_COMMAND_MININOTEOFF_INFO
			);
		local MapNotesmntlocCommands = {"/mntloc"};
		Cosmos_RegisterChatCommand (
			"MAPNOTES_MNTLOC_COMMANDS",
			MapNotesmntlocCommands,
			MapNotes_mntloc,
			MAPNOTES_CHAT_COMMAND_MNTLOC_INFO
			);
		local MapNotesQuicknoteCommands = {"/quicknote", "/qnote"};
		Cosmos_RegisterChatCommand (
			"MAPNOTES_QUICKNOTE_COMMANDS",
			MapNotesQuicknoteCommands,
			MapNotes_Quicknote,
			MAPNOTES_CHAT_COMMAND_QUICKNOTE
			);
		local MapNotesQuicktlocCommands = {"/quicktloc", "/qtloc"};
		Cosmos_RegisterChatCommand (
			"MAPNOTES_QUICKTLOC_COMMANDS",
			MapNotesQuicktlocCommands,
			MapNotes_Quicktloc,
			MAPNOTES_CHAT_COMMAND_QUICKTLOC
			);
		end
		if (Sky ) then
			Sky.registerAlert ( {id="MN",func=MapNotes_GetNoteFromChat,description="MapNotes"});
		elseif ( Cosmos_RegisterChatCommand ) and ( Cosmos_RegisterChatWatch ) then
--  sarf 5/19/05 added fallback code if Sky is not present
			Cosmos_RegisterChatWatch ( "MAPNOTESPARTY", {CHANNEL_PARTY}, function (type, info, arg1, arg2, arg3, arg4) if (strsub(arg1, 1, 6) == "<MapN>") then MapNotes_GetNoteFromChat(arg1, arg2); return 0; end end );
			Cosmos_RegisterChatWatch ( "MAPNOTESWHISPER", {"WHISPER"}, function (type, info, arg1, arg2, arg3, arg4) if (strsub(arg1, 1, 6) == "<MapN>") then MapNotes_GetNoteFromChat(arg1, arg2); return 0; end end );
			Cosmos_RegisterChatWatch ( "MAPNOTESWHISPER_INFORM", {"WHISPER_INFORM"}, function (type, info, arg1, arg2, arg3, arg4) if (strsub(arg1, 1, 6) == "<MapN>") then return 0; end end );
		end
		MapNotes_CosmosIsRegistered = true;
	else
		SLASH_MAPNOTE1 = "/mapnote";
		SLASH_MAPNOTE2 = "/MapNote";
		SLASH_MAPNOTE3 = "/MAPNOTE";
		SlashCmdList["MAPNOTE"] = function(msg)
			MapNotes_GetNoteBySlashCommand(msg);
		end
		SLASH_ONENOTE1 = "/onenote";
		SLASH_ONENOTE2 = "/allowonenote";
		SLASH_ONENOTE3 = "/aon";
		SlashCmdList["ONENOTE"] = function(msg)
			MapNotes_OneNote(msg);
		end
		SLASH_MININOTE1 = "/nextmininote";
		SLASH_MININOTE2 = "/nextMiniNote";
		SLASH_MININOTE3 = "/nmn";
		SlashCmdList["MININOTE"] = function(msg)
			MapNotes_NextMiniNote(msg);
		end
		SLASH_MININOTEONLY1 = "/nextmininoteonly";
		SLASH_MININOTEONLY2 = "/nextMiniNoteOnly";
		SLASH_MININOTEONLY3 = "/nmno";
		SlashCmdList["MININOTEONLY"] = function(msg)
			MapNotes_NextMiniNoteOnly(msg);
		end
		SLASH_MININOTEOFF1 = "/mininoteoff";
		SLASH_MININOTEOFF2 = "/MiniNoteOff";
		SLASH_MININOTEOFF3 = "/mno";
		SlashCmdList["MININOTEOFF"] = function(msg)
			MapNotes_ClearMiniNote();
		end
		SLASH_MNTLOC1 = "/mntloc";
		SlashCmdList["MNTLOC"] = function(msg)
			MapNotes_mntloc(msg);
		end
		SLASH_QUICKNOTE1 = "/quicknote";
		SLASH_QUICKNOTE2 = "/qnote";
		SlashCmdList["QUICKNOTE"] = function(msg)
			MapNotes_Quicknote(msg);
		end
		SLASH_QUICKTLOC1 = "/quicktloc";
		SLASH_QUICKTLOC2 = "/qtloc";
		SlashCmdList["QUICKTLOC"] = function(msg)
			MapNotes_Quicktloc(msg);
		end
		MapNotes_CosmosIsRegistered = false;
...
-- don't know how to get these in one function (haven't tried often)
...
function MapNotes_GetZone() --because of the bug in SetToCurrentZone();
	for i=1, 2, 1 do
		...
		for k, value in MapNotes_ZoneNames[i] do
			if (MapNotes_ZoneNames[i][j] == GetZoneText()) then
				return i, j;
			end
			j = j + 1;
		end
...
function MapNotes_MiniNote_OnUpdate(arg1)
	if ( MapNotes_MiniNote_Data.xPos ~= nil ) then
		...
		if( MiniNotePOI.TimeSinceLastUpdate > MapNotes_Mininote_UpdateRate ) then
			local x, y = GetPlayerMapPosition("player");
			if ((x == 0 and y == 0 and (MapNotes_ZoneShift[GetCurrentMapContinent()][GetCurrentMapZone()] ~= 0 or GetCurrentMapContinent() ~= MapNotes_MiniNote_Data.continent)) or GetCurrentMapContinent() == 0) then SetMapToCurrentZone(); x, y = GetPlayerMapPosition("player"); end
			if (x ~= 0 and y ~= 0 and MapNotes_Const[MapNotes_MiniNote_Data.continent][MapNotes_MiniNote_Data.zone].scale ~= 0) then
				local xscale = MapNotes_Const[MapNotes_MiniNote_Data.continent][Minimap:GetZoom()].xscale;
				local yscale = MapNotes_Const[MapNotes_MiniNote_Data.continent][Minimap:GetZoom()].yscale;
				if (MapNotes_MiniNote_IsInCity) then
					xscale = xscale * MapNotes_Const[2][Minimap:GetZoom()].cityscale;
					yscale = yscale * MapNotes_Const[2][Minimap:GetZoom()].cityscale;
				end
				local xpos = MapNotes_MiniNote_Data.xPos * MapNotes_Const[MapNotes_MiniNote_Data.continent][MapNotes_MiniNote_Data.zone].scale + MapNotes_Const[MapNotes_MiniNote_Data.continent][MapNotes_MiniNote_Data.zone].xoffset;
				local ypos = MapNotes_MiniNote_Data.yPos * MapNotes_Const[MapNotes_MiniNote_Data.continent][MapNotes_MiniNote_Data.zone].scale + MapNotes_Const[MapNotes_MiniNote_Data.continent][MapNotes_MiniNote_Data.zone].yoffset;
				if (MapNotes_ZoneShift[GetCurrentMapContinent()][GetCurrentMapZone()] ~= 0) then
					x = x * MapNotes_Const[GetCurrentMapContinent()][MapNotes_ZoneShift[GetCurrentMapContinent()][GetCurrentMapZone()]].scale + MapNotes_Const[GetCurrentMapContinent()][MapNotes_ZoneShift[GetCurrentMapContinent()][GetCurrentMapZone()]].xoffset;
					y = y * MapNotes_Const[GetCurrentMapContinent()][MapNotes_ZoneShift[GetCurrentMapContinent()][GetCurrentMapZone()]].scale + MapNotes_Const[GetCurrentMapContinent()][MapNotes_ZoneShift[GetCurrentMapContinent()][GetCurrentMapZone()]].yoffset;
				end
...
function MapNotes_OpenEditForExistingNote(id)
	...
	MapNotes_TempData_Creator = MapNotes_Data[continent][zone][MapNotes_TempData_Id].creator;
	MapNotes_TempData_xPos = MapNotes_Data[continent][zone][MapNotes_TempData_Id].xPos;
	MapNotes_TempData_yPos = MapNotes_Data[continent][zone][MapNotes_TempData_Id].yPos;
	MapNotes_Edit_SetIcon(MapNotes_Data[continent][zone][MapNotes_TempData_Id].icon);
	MapNotes_Edit_SetTextColor(MapNotes_Data[continent][zone][MapNotes_TempData_Id].ncol);
	MapNotes_Edit_SetInfo1Color(MapNotes_Data[continent][zone][MapNotes_TempData_Id].in1c);
	MapNotes_Edit_SetInfo2Color(MapNotes_Data[continent][zone][MapNotes_TempData_Id].in2c);
	TitleWideEditBox:SetText(MapNotes_Data[continent][zone][MapNotes_TempData_Id].name);
	Info1WideEditBox:SetText(MapNotes_Data[continent][zone][MapNotes_TempData_Id].inf1);
	Info2WideEditBox:SetText(MapNotes_Data[continent][zone][MapNotes_TempData_Id].inf2);
...
--  sarf 5/19/05 changed so that a function is used to evaluate whether notes can be sent to party or not.
		if (MapNotes_CanSendNotesToParty()) then
...
-- <MapN> c<1> z<1> x<0.123123> y<0.123123> t<> i1<> i2<> cr<> i<8> tc<3> i1c<5> i2c<6>
	...
	local xPos = floor(MapNotes_Data[continent][zone][MapNotes_TempData_Id].xPos * 1000000)/1000000; --cut to six digits behind the 0
	local yPos = floor(MapNotes_Data[continent][zone][MapNotes_TempData_Id].yPos * 1000000)/1000000;
	text = text.." x<"..xPos.."> y<"..yPos..">";
	text = text.." t<"..MapNotes_EliminateUsedChars(MapNotes_Data[continent][zone][MapNotes_TempData_Id].name)..">";
	text = text.." i1<"..MapNotes_EliminateUsedChars(MapNotes_Data[continent][zone][MapNotes_TempData_Id].inf1)..">";
	text = text.." i2<"..MapNotes_EliminateUsedChars(MapNotes_Data[continent][zone][MapNotes_TempData_Id].inf2)..">";
	if ( not MapNotes_Data[continent][zone][MapNotes_TempData_Id].creator ) then 
		MapNotes_Data[continent][zone][MapNotes_TempData_Id].creator = UnitName("player");
	end
	text = text.." cr<"..MapNotes_Data[continent][zone][MapNotes_TempData_Id].creator..">";
	text = text.." i<"..MapNotes_Data[continent][zone][MapNotes_TempData_Id].icon..">";
	text = text.." tf<"..MapNotes_Data[continent][zone][MapNotes_TempData_Id].ncol..">";
	text = text.." i1f<"..MapNotes_Data[continent][zone][MapNotes_TempData_Id].in1c..">";
	text = text.." i2f<"..MapNotes_Data[continent][zone][MapNotes_TempData_Id].in2c..">";
	return text;
end
...
function MapNotes_CanSendNotesToParty()
	if ( Sky ) or ( Cosmos_SendPartyMessage ) then
		return true;
	else
		return false;
	end
...
--  sarf 5/19/05 added fallback code if Sky is not present
	...
	elseif (type == 2) then
		...
		elseif ( Cosmos_SendPartyMessage ) then
--  sarf 5/19/05 added fallback code if Sky is not present
			Cosmos_SendPartyMessage(MapNotes_GenerateSendString(1));
		end
		MapNotes_HideFrames();
	end
end
...
function MapNotes_SetAsMiniNote(id)
	...
	elseif (id == -1) then
		...
		MapNotes_MiniNote_Data.xPos = MapNotes_Data[continent][zone][id].xPos;
		MapNotes_MiniNote_Data.yPos = MapNotes_Data[continent][zone][id].yPos;
		MapNotes_MiniNote_Data.name = MapNotes_Data[continent][zone][id].name;
		MapNotes_MiniNote_Data.color = MapNotes_Data[continent][zone][id].ncol;
		MapNotes_MiniNote_Data.icon = MapNotes_Data[continent][zone][id].icon;
		MiniNotePOITexture:SetTexture("Interface\\AddOns\\MapNotes\\POIIcons\\Icon"..MapNotes_MiniNote_Data.icon);
		MiniNotePOI:Show();
	end
end
...
function MapNotes_WriteNote()
	...
	local continent = GetCurrentMapContinent();
	MapNotes_Data[continent][zone][MapNotes_TempData_Id] = {};
	MapNotes_Data[continent][zone][MapNotes_TempData_Id].name = TitleWideEditBox:GetText();
	MapNotes_Data[continent][zone][MapNotes_TempData_Id].ncol = MapNotes_TempData_TextColor;
	MapNotes_Data[continent][zone][MapNotes_TempData_Id].inf1 = Info1WideEditBox:GetText();
	MapNotes_Data[continent][zone][MapNotes_TempData_Id].in1c = MapNotes_TempData_Info1Color;
	MapNotes_Data[continent][zone][MapNotes_TempData_Id].inf2 = Info2WideEditBox:GetText();
	MapNotes_Data[continent][zone][MapNotes_TempData_Id].in2c = MapNotes_TempData_Info2Color;
	MapNotes_Data[continent][zone][MapNotes_TempData_Id].creator = MapNotes_TempData_Creator;
	MapNotes_Data[continent][zone][MapNotes_TempData_Id].icon = MapNotes_TempData_Icon;
	MapNotes_Data[continent][zone][MapNotes_TempData_Id].xPos = MapNotes_TempData_xPos;
	MapNotes_Data[continent][zone][MapNotes_TempData_Id].yPos = MapNotes_TempData_yPos;
...
function MapNotes_OnEnter(id)
	if ((not MapNotesPOIMenuFrame:IsVisible()) and (not MapNotesNewMenuFrame:IsVisible()) and MapNotes_BlockingFrame()) then
		...
		WorldMapTooltip:SetText(MapNotes_Data[continent][zone][id].name, MapNotes_Colors[cNr].r, MapNotes_Colors[cNr].g, MapNotes_Colors[cNr].b);
		if ((MapNotes_Data[continent][zone][id].inf1 ~= nil) and (MapNotes_Data[continent][zone][id].inf1 ~= "")) then
			...
			WorldMapTooltip:AddLine(MapNotes_Data[continent][zone][id].inf1, MapNotes_Colors[cNr].r, MapNotes_Colors[cNr].g, MapNotes_Colors[cNr].b);
		end
		if ((MapNotes_Data[continent][zone][id].inf2 ~= nil) and (MapNotes_Data[continent][zone][id].inf2 ~= "")) then
			...
			WorldMapTooltip:AddLine(MapNotes_Data[continent][zone][id].inf2, MapNotes_Colors[cNr].r, MapNotes_Colors[cNr].g, MapNotes_Colors[cNr].b);
		end
		WorldMapTooltip:AddDoubleLine(MAPNOTES_CREATEDBY, MapNotes_Data[continent][zone][id].creator, 0.49, 0.39, 0.0, 0.49, 0.39, 0.0);
...
function MapNotes_Note_OnClick(arg1, id)
	...
	if ( MapNotes_LastLineClick.GUIactive ) then
		...
		local zone = MapNotes_ZoneShift[GetCurrentMapContinent()][GetCurrentMapZone()];
		local continent = GetCurrentMapContinent();
		local ax = MapNotes_Data[continent][zone][id].xPos;
		local ay = MapNotes_Data[continent][zone][id].yPos;
...
function MapNotes_SetPartyNote(xPos, yPos)
	...
	MapNotes_PartyNoteData.xPos = xPos;
	MapNotes_PartyNoteData.yPos = yPos;
	if (MapNotes_CosmosIsRegistered) then 
		local msg = "<MapN> c<"..continent.."> z<"..zone.."> x<"..xPos.."> y<"..yPos.."> p<1>";
		if ( Sky ) then
			Sky.sendAlert(msg, SKY_PARTY, "MN"); 
		elseif( Cosmos_SendPartyMessage ) then
--  sarf 5/19/05 added fallback code if Sky is not present
...
function MapNotes_WorldMapButton_OnClick(mouseButton, button)
	...
	if ( mouseButton == "LeftButton" ) then
		...
		local adjustedY = (centerY + (height/2) - y) / height;
		local adjustedX = (x - (centerX - (width/2))) / width;
-- Map Notes Begin Insert [ 2/ 5]
		local oldzone = MapNotes_ZoneShift[GetCurrentMapContinent()][GetCurrentMapZone()];
		if (MapNotes_BlockingFrame()) then
-- Map Notes End Insert [ 2/ 5]
			ProcessMapClick(adjustedX, adjustedY);
-- Map Notes Begin Insert [ 3/ 5]
		end
		if (MapNotes_ZoneShift[GetCurrentMapContinent()][GetCurrentMapZone()] ~= 0 and oldzone == MapNotes_ZoneShift[GetCurrentMapContinent()][GetCurrentMapZone()] and IsShiftKeyDown()) then
			MapNotes_SetPartyNote(adjustedX, adjustedY);
		elseif (MapNotes_ZoneShift[GetCurrentMapContinent()][GetCurrentMapZone()] ~= 0 and oldzone == MapNotes_ZoneShift[GetCurrentMapContinent()][GetCurrentMapZone()] and GetTime() < MapNotes_LastClick.time + 0.5 and abs(MapNotes_LastClick.yPos - adjustedY) < 0.01 and abs(MapNotes_LastClick.xPos - adjustedX) < 0.005) then
			MapNotesButtonNewNote:Enable();
			MapNotes_ShowNewFrame(adjustedX, adjustedY);
		else
			MapNotes_HideFrames();
...
-- Map Notes Begin Change [ 4/ 8]
		WorldMapMapNotesPlayer:Hide();
-- Map Notes End Change [ 4/ 8]
	else
		playerX = playerX * WorldMapDetailFrame:GetWidth();
		playerY = -playerY * WorldMapDetailFrame:GetHeight();
		
-- Map Notes Begin Change [ 5/ 8]
		WorldMapMapNotesPlayer:SetPoint("CENTER", "WorldMapDetailFrame", "TOPLEFT", playerX, playerY);
		WorldMapMapNotesPlayer:Show();
-- Map Notes End Change [ 5/ 8]
	end
-- Map Notes Begin Insert [ 6/ 8]
	WorldMapPlayer:Hide();
-- Map Notes End Insert [ 6/ 8]
	--Position groupmates
...
-- Marsman 3/27/05 - Added Raid Party Positions
	--Position raid groupmates
...
-- Map Notes Begin Insert [ 7/ 8]
	--MapNotes Display Notes...
	local zone = MapNotes_ZoneShift[GetCurrentMapContinent()][GetCurrentMapZone()];
	local continent = GetCurrentMapContinent();
	if (zone ~= 0 and MapNotes_Options.shownotes) then
		local n = 1;
		for i, value in MapNotes_Data[continent][zone] do
			local temp = getglobal("MapNotesPOI"..i);
				temp:SetPoint("CENTER", "WorldMapDetailFrame", "TOPLEFT", (MapNotes_Data[continent][zone][i].xPos)*WorldMapButton:GetWidth(), -(MapNotes_Data[continent][zone][i].yPos)*WorldMapButton:GetHeight());
				getglobal("MapNotesPOI"..i.."Texture"):SetTexture("Interface\\AddOns\\MapNotes\\POIIcons\\Icon"..MapNotes_Data[continent][zone][i].icon);
		...
		if (MapNotes_Options[12] ~= "off" and lastnote ~= 0) then
			if (getglobal("MapNotesPOI"..lastnote):IsVisible()) then
				getglobal("MapNotesPOI"..lastnote.."Texture"):SetTexture("Interface\\AddOns\\MapNotes\\POIIcons\\Icon"..MapNotes_Data[continent][zone][lastnote].icon.."red");
			end
		end
		...
		local n = 1;
		for i, value in MapNotes_Lines[continent][zone] do
			MapNotes_DrawLine(n, MapNotes_Lines[continent][zone][n].x1, MapNotes_Lines[continent][zone][n].y1, MapNotes_Lines[continent][zone][n].x2, MapNotes_Lines[continent][zone][n].y2);
			n = n + 1;
		end
...
if (Sea) then
	...
	Sea.util.hook("WorldMapButton_OnClick", "MapNotes_WorldMapButton_OnClick", "replace");
	Sea.util.hook("Minimap_OnClick", "MapNotes_Minimap_OnClick", "replace");
	...
	WorldMapButton_OnUpdate = function() old_WorldMapButton_OnUpdate(); MapNotes_WorldMapButton_OnUpdate(); end
	WorldMapButton_OnClick = MapNotes_WorldMapButton_OnClick;
	Minimap_OnClick = MapNotes_Minimap_OnClick;
	old_ToggleWorldMap = ToggleWorldMap;
	ToggleWorldMap = function() old_ToggleWorldMap(); MapNotes_OnToggleWorldMap(); end
	old_WorldMapZoomOutButton_OnClick = WorldMapZoomOutButton_OnClick;
	WorldMapZoomOutButton_OnClick = function() old_WorldMapZoomOutButton_OnClick(); MapNotes_ZoomOutButton_OnClick(); end
end
