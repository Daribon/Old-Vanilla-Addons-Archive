--[[
	...
	Version: <%version%>
	Revision: $Id: GathererUI.lua,v 1.8 2005/06/08 06:42:55 islorgris Exp $
...
function GathererUI_InitializeOptions()
	...
	if (GatherConfig.Position == nil) then
		...
		-- LedMirage 6/19/2005 Start change [ 1 or 1]
		GatherConfig.Position = 189;
		-- LedMirage 6/19/2005 End change [ 1 or 1]		
	end
			...
			if ( idx_c ~= 0 and idx_z ~= 0) then
				...
				for myItems, rec_gatheritem in rec_zone do
					...
					if (gathererFixItems == 1) then 
						fixedItemName = GathererUI_FixItemName(myItems, 0); 
					else
					...
					for idx_item, myGather in rec_gatheritem do
						...
						if (gathererFixItems == 1) then 
							NewGatherItems[idx_c][new_idx_z][fixedItemName][idx_item].icon = GathererUI_FixItemName(myGather.icon, 1);
						else
...
function GathererUI_FixItemName(itemName, isAnIcon)
	...
	if ( GetLocale() == "deDE" ) then
		...
		if ( isAnIcon == 1) then
			newItemName = string.gsub(itemName, "br\195\188hschlammbedecktes ", "");
			newItemName = string.gsub(newItemName, "reiches ", "");
			newItemName = string.gsub(newItemName, "kleines ", "");
			newItemName = string.gsub(newItemName, "riesen", "");
		end
		if ( itemName ~= newItemName ) then
			fixedItemCount = fixedItemCount + 1;
		end
