...
-- public functions	
	...
	getNameFromTooltip = function (tooltipName)
		local strings = DynamicData.util.getTooltipStrings(tooltipName, DynamicData.util.nameFromTooltipStrings);
...
-- private functions	
	...
	-- OnLoad()
	--
	--  sets up the util library. NOTE: This inserts an emulated Cosmos_Schedule if not present.
	--
	OnLoad = function()
		local needHandler = false;
		if ( not Cosmos_Schedule ) then
			Cosmos_Schedule = DynamicData.util.scheduleFunction;
			needHandler = true;
		end
		if ( not Cosmos_ScheduleByName ) then
			Cosmos_ScheduleByName = DynamicData.util.scheduleNamedFunction;
			needHandler = true;
		end
		if ( not Cosmos_AfterInit ) then
			Cosmos_AfterInit = DynamicData.util.scheduleAfterInitFunction;
			needHandler = true;
		end
		if ( needHandler ) then
