...
lColorSortTable["ffa335ee"] = 1;	-- epic, purple
lColorSortTable["ff0070dd"] = 2;	-- rare, blue
lColorSortTable["ff1eff00"] = 3;	-- uncommon, green
lColorSortTable["ffffffff"] = 4;	-- common, white
lColorSortTable["ff9d9d9d"] = 5;	-- poor, gray
...
lTypeAndSubtypeToSkill[TYPE_ARMOR][SUBTYPE_ARMOR_PLATE] = "Plate";
...
LOOTLINK_VERSION = 315;	-- version 3.15
...
--@todo Telo: temporary to build polearms, crossbows, and fist weapons types correctly
		or (LootLink_GetDataVersion() < 100 and
			((LL_SearchData(value, "ty") == TYPE_WEAPON and not LL_SearchData(value, "su"))
			 or (LL_SearchData(value, "ty") == TYPE_SHIELD)
			 or (LL_SearchData(value, "ty") == TYPE_MISC)))
	   ) then
			...
			if( _type ) then
				if( subtype ) then
					...
					if( type(entry) == "table" ) then
						...
						if( location ) then
							if( location == 3 ) then
								rank = ud.skills[entry[1]];
							else
								rank = ud.skills[entry[0]];
							end
							if( not rank ) then
								return nil;
							end
...
--[[
	if( ItemLinks[elem1] and not ItemLinks[elem1].d ) then
		LootLink_BuildSearchData(elem1, ItemLinks[elem1]);
	end
	if( ItemLinks[elem2] and not ItemLinks[elem2].d ) then
...
-- ======================================================
-- Start of Ultimate UI Menu Registration
-- ======================================================
   if ( UltimateUI_RegisterButton ) then
      UltimateUI_RegisterButton (
		"Lootlink",
		"Show",
		"An in-game browseable item database.",
		"Interface\\Icons\\Ability_Warrior_OffensiveStance",
		ToggleLootLink
	);
   end
-- ======================================================
-- End of Ultimate UI Menu Registration
-- ======================================================
...
function LootLink_SetItemRef(link, button)
	lOriginal_SetItemRef(link, button);
...
function LootLink_OnEvent()
	...
	elseif( event == "MERCHANT_SHOW" ) then
		...
		local name = LootLink_ProcessLinks(arg1, nil);
		if( name and ItemLinks[name] ) then
...
function LootLink_OnUpdate(elapsed)
	lCheckTimer = lCheckTimer + elapsed;
	if( lCheckTimer >= 0.2 ) then
...
--					end
				...
				if( ItemLinks[name] and not LootLink_CheckItemServerRaw(ItemLinks[name], lServerIndex) ) then
...
-- Replaces uncolored links in a message with colored ones, if possible, then adds the message to the given frame
function LootLink_ProcessLootMsg(msg, frame, info)
	...
	for item, name in string.gfind(msg, "|Hitem:(%d+:%d+:%d+:%d+)|h%[(.-)%]|h") do
		if( item and name and ItemLinks[name] ) then
			...
			msg = string.gsub(msg, LootLink_EscapePattern(link), "|c"..ItemLinks[name].c..link);
		end
	end
