...
function OneHitWonder_UnitHasGainedSpell(unitName, spellName)
	local funcName = format("OneHitWonder_UnitHasGainedSpell_%s", OneHitWonder_GetPlayerClass());
...
function OneHitWonder_Chat_OnEvent(event)
	if ( OneHitWonder_IsEnabled() ) then
		if ( strsub(event, 1, 8) == "CHAT_MSG" ) then
			...
			if ( ( OneHitWonder_HasTarget() ) and ( UnitCanAttack("target", "player") ) ) then
...
function OneHitWonder_ShowQueue()
	...
	if ( queueNumber <= 0 ) then
		...
		( actionQueueId == ONEHITWONDER_ACTIONID_SPELL_ITEM ) ) then
			...
			local spellName = OneHitWonder_GetSpellNameFromSpellId(spellId);
			if ( OneHitWonder_ShouldShowQueueFull ) then
				mainText = format("%s Queued [%d]", spellName,queueNumber);
			else
				mainText = format("Queue: %d left", queueNumber);
			end
		elseif ( actionQueueId == ONEHITWONDER_ACTIONID_ACTION_SELF ) then
			...
			local actionName = OneHitWonder_GetActionNameFromActionId(actionId);
			
			if ( OneHitWonder_ShouldShowQueueFull ) then
				mainText = format("%s Queued [%d]", actionName, queueNumber);
			else
				mainText = format("Queue: %d left", queueNumber);
			end
		else
...
function OneHitWonder_OnLoad()
	...
	RegisterForSave("OneHitWonder_ShouldShowQueueFull");
...
function OneHitWonder_IsPlayerOnMount()
	local effectInfos = DynamicData.effect.getEffectInfos("player");
	local isMounted = false;
	if ( effectInfos ) and ( effectInfos.buffs ) then
		for k, v in effectInfos.buffs do
			if ( strfind(v.texture, ONEHITWONDER_BUFF_TEXTURE_MOUNT) ) then
				isMounted = true;
				break;
			end
		end
...
function OneHitWonder_DoBuffs(destroyTarget, specialOptions, buffList)
	...
	for k, v in tempList do
		ok = false;
		if ( ( UnitExists(v) ) and ( ( v ~= "target" ) or ( UnitCanCooperate("player", v) ) ) and ( not UnitIsDead(v) ) and ( not UnitIsGhost(v) ) ) then
			ok = true;
...
function OneHitWonder_CastBuff(castBuffName, v, unit, destroyTarget, specialOptions)
	...
	if ( ok ) then
		if ( v ) then
			if ( ( v.isBuff ) and ( OneHitWonder_HasUnitEffect(unit, nil, effectName) ) ) then
				ok = false;
...
function OneHitWonder_BuffUnit(unit, destroyTarget, specialOptions)
	...
	if ( OneHitWonder_ShouldKeepBuffsUpParty == 0 ) and ( strfind(unit, "party") ) then
		return false;
	end
	if ( getn(OneHitWonder_BuffsToKeepUp) <= 0 ) then
		return false;
	end
	if ( OneHitWonder_IsPlayerDead() ) then
		return false;
	...
	for k, v in OneHitWonder_BuffsToKeepUp do
		...
		if ( type(v.buffName) == "table" ) then
			...
			for key, value in actualBuffs do
				...
				if ( ( buffId > -1 ) and ( not OneHitWonder_HasUnitEffect(unit, nil, effectName) ) ) then
					if (hasAnyBuff) then
...
function OneHitWonder_DoStuffContinouslyBuffOld()
		for k, v in OneHitWonder_BuffsToKeepUp do
...
function OneHitWonder_DoStuffContinously()
	...
	local isDead = OneHitWonder_IsPlayerDead();
	local isMounted = OneHitWonder_IsPlayerOnMount();
	local isTaxi = OneHitWonder_IsPlayerOnTaxi();
	if ( isDead ) or ( isMounted ) or ( isTaxi ) then
		return;
	end
	...
	local func = getglobal(format("OneHitWonder_DoStuffContinously_%s", OneHitWonder_GetPlayerClass()));
	if ( func ) then
		if ( func() ) then
			return;
		end
...
function OneHitWonder_CheckFriendlies()
	local curTime = GetTime();
	if ( OneHitWonder_CheckFriendlies_Time ) and ( ( curTime - OneHitWonder_CheckFriendlies_Time ) < 2 ) then
		Cosmos_Schedule(1, OneHitWonder_CheckFriendlies);
		return;
	end
	OneHitWonder_CheckEffect("player");
	OneHitWonder_CheckEffect("pet");
