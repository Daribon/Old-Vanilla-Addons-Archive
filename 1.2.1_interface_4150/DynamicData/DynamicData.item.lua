...
-- public functions	
	...
	getItemInfoByType = function (itemName) 
		...
		local items = {};
		for bagNumber, bag in DynamicData.item.itemsByBag do
...
-- protected functions
	...
	doUpdateItems = function (bag) 
		...
		if ( ( bag ) and ( DynamicData.item.player ) ) then
			...
			if ( not DynamicData.item.itemScanInProgress ) then
				...
				DynamicData.item.doScans(nil, nil, bag);
				--Cosmos_ScheduleByName("DynamicData_Item_doScans", 0.1, DynamicData.item.doScans);
			end
	...
	doScans = function (scanLists, currentIndex, bag)
		...
		if ( not currentIndex ) then
			...
			Cosmos_ScheduleByName(scheduleName, 0.1, DynamicData.item.doScans, scanLists, 1, bag);
			return;
		else
			-- prevents old scans from continuing.
			Cosmos_ScheduleByName(scheduleName, 0.1, DynamicData.item.doScans);
		end
		...
		if ( DynamicData.item.scanItems(scanType) ) then
			...
			Cosmos_ScheduleByName(scheduleName, 0.1, DynamicData.item.doScans, scanLists, currentIndex, bag);
			return;
		else
			if ( scanLists == DynamicData.item.scanList ) then
				--DynamicData.item.scanList = {};
...
-- private functions	
	...
	OnLoad = function ()
		...
		Cosmos_AfterInit(DynamicData.item.doUpdateItems);
		Cosmos_AfterInit(Cosmos_Schedule, 30, DynamicData.item.doUpdateItems);
		Cosmos_AfterInit(Cosmos_Schedule, 60, DynamicData.item.doUpdateItems);
		Cosmos_AfterInit(Cosmos_Schedule, 90, DynamicData.item.doUpdateItems);
	end;
	...
	scanItem = function(bag, slot, scanType)
		...
		if ( bag > -1 ) then
			...
			if ( ( scanType and DYNAMICDATA_ITEM_SCAN_TYPE_TOOLTIP ) > 0 ) then
				if ( not element.previousElement ) or ( element.previousElement.name ~= element.name ) or ( element.previousElement.cooldown.start ~= 0 ) or ( element.cooldown.start ~= 0 ) then
