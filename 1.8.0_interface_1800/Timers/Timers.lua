...
TIMERS_DEFAULT_TRIGGER_LIST         = {
                                        ...
                                        {
                                            time = 18,
                                            trigger = "Uralter Kernhund stirbt."
                                        },
										["firelord"] =
                                        {
                                            time = 149,
                                            trigger = "Firelord dies."
                                        },
										["feuerlord"] =
                                        {
                                            time = 149,
                                            trigger = "Feuerlord stirbt."
                                        },
										["annihilator"] =
                                        {
                                            time = 149,
                                            trigger = "Lava Annihilator dies."
                                        },
										["vernichter"] =
                                        {
                                            time = 149,
                                            trigger = "Lavavernichter stirbt."
                                        },
										["lavawoger"] =
                                        {
                                            time = 27,
                                            trigger = "Lavawoger stirbt."
                                        },
                                        ["surger"] =
                                        {
                                            time = 27,
...
function TimersFrame_OnLoad()
	...
	Timex:AddStartupFunc(TimersFrame_OnAfterInit);
	number = 1;
end
	...
	if ((text ~= nil) and TimerTriggers_IsThresholdExpired()) then
		for index, value in state.triggerList do
			if (value.trigger == text) then
				...
				TimersFrame_OnMessageBoxAccept(number);
				number = number + 1;
				break;
			end
		end
...
function TimerTriggers_SetThreshold(threshold)
	...
	Timers_Print("Trigger threshold is now "..threshold.." seconds.");
end
