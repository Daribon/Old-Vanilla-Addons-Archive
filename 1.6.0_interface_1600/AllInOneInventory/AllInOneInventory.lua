...
-- Constants
ALLINONEINVENTORYFRAME_UPDATE_DELAY = 0.5;
...
AllInOneInventoryScriptFrame_ScheduledNamedStuff = {};
AllInOneInventoryScriptFrame_ScheduledStuff = {};



function AIOI_Schedule_Own(time, func, p1, p2, p3, p4, p5, p6, p7, p8, p9)
	local arr = {};
	arr.when = GetTime()+time;
	arr.func = func;
	if ( p1 ~= null ) or ( p2 ~= null ) or ( p3 ~= null ) or ( p4 ~= null ) or ( p5 ~= null ) or ( p6 ~= null ) or ( p7 ~= null ) or ( p8 ~= null ) or ( p9 ~= null ) then
		arr.args = {p1, p2, p3, p4, p5, p6, p7, p8, p9};
	end
	table.insert(AllInOneInventoryScriptFrame_ScheduledStuff, arr);
end
...
function AIOI_ScheduleByName_Own(name, time, func, p1, p2, p3, p4, p5, p6, p7, p8, p9)
	...
	AllInOneInventoryScriptFrame_ScheduledNamedStuff[name] = arr;
end
...
function AllInOneInventoryScriptFrame_OnLoad()
	...
	if ( Chronos ) and ( Chronos.scheduleByName ) then
		...
		AllInOneInventoryScriptFrame:Hide();
	...
	if ( func == AIOI_ScheduleByName_Own ) then
		AllInOneInventoryItemsFrame_DoUpdate();
		AllInOneInventoryItems_UpdateFrame(true);
	end
...
function AllInOneInventoryFrame_OnInventoryUpdate(bag, scanType)
	...
	local fixTime = ALLINONEINVENTORYFRAME_UPDATE_DELAY;
...
function AllInOneInventory_ModifyItemTooltip(bag, slot, tooltipName)
	...
	if ( tooltipName == "GameTooltip" ) then
		if ( DropRate_OnShow ) then
			DropRate_OnShow();
		end
		if ( MonkeyQuest_SearchTooltip ) then
...
function AllInOneInventoryScriptFrame_OnUpdate(elapsed)
	local curTime = GetTime();
	local removed = true;
	while (removed) do
		removed = false;
		for k, v in AllInOneInventoryScriptFrame_ScheduledNamedStuff do
			if ( v.when >= curTime ) then
				...
				AllInOneInventoryScriptFrame_ScheduledNamedStuff[k] = nil;
				removed = true;
				break;
			end
		end
	...
	while (removed) do
		removed = false;
		for k, v in AllInOneInventoryScriptFrame_ScheduledStuff do
			if ( v.when >= curTime ) then
				if ( v.args ) then
					v.func(unpack(v.args));
				else
				...
				table.remove(AllInOneInventoryScriptFrame_ScheduledStuff, k);
				removed = true;
				break;
			end
		end
