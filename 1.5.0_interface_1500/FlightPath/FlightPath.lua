...
function FP_PlayerEnteringWorld()
   ...
   -- Hook any global WoW functions we are interested in
...
function FP_EventFrame_OnUpdate()
   ...
   if (FlightPath_Config.Enabled) then
      ...
      if(ctime ~= LastTime) then
         ...
         -- Do periodic processing (once per second)

         FP_ShowInFlightCountdown();
...
function FP_ShowInFlightCountdown()
   ...
   if(TimeRemaining > 0) then
      TimeRemaining = TimeRemaining - 1;  -- Decrease in flight time remaining
   end
...
function FP_ReportBindingKey()
   if(GetBindingKey("FPDIALOG")) then
      FPCHAT("The FlightPath dialog is bound to key "..KeyBindingFrame_GetLocalizedName(GetBindingKey("FPDIALOG"), "KEY_"));
   end
end
...
function FP_WorldMapButton_OnUpdate()

   Original_WorldMapButton_OnUpdate(); -- Call the original owner of the hook so we don't break other mods
